...
Begin VB.Form frmBLAdjustBal 
   ...
   Begin EditLib.fpDateTime fptxtToday 
      ...
      TabIndex        =   19
      TabStop         =   0   'False
      Tag             =   "This field defaults to today's date but can be edited."
      Top             =   1200
      Width           =   1740
      _Version        =   196608
      _ExtentX        =   3069
...
Private Sub cmdExit_Click()
  ...
  DoEvents
  MainLog ("Adjust customer balance screen exited.")
...
10:
    If fpcurrTotAmt.DoubleValue = 0 Then
  ...
  If GCustNum > 0 Then
    If EmpInLicProcess(CStr(GCustNum)) = True And Exist("artmppst.dat") Then
      ...
      Else
        ...
        MainLog ("User warned that continuing to adjust the balance of customer # " + CStr(GCustNum) + " would delete the 'attmppst.dat' and the 'artmplic.dat' files and the user elected to continue to post the balance adjustment anyway.")
      End If
    End If
  End If
30:
...
'        MainLog ("User warned that continuing to edit customer # " + CStr(GCustNum) + " would delete the 'attmppen.dat' file and the user elected to post the balance adjustment anyway.")
'      End If
'    End If
'  End If
40:
  If QPTrim$(fptxtName.Text) = "" Then
    ...
    frmBLMessageBoxJr.Show vbModal
    MainLog ("Adjustment post aborted because the business name was not valid.")
    Close
    Exit Sub
  End If
50:
  ...
  Else
    Unload frmBLMessageBoxJrWOpts
    MainLog ("User warned they were posting adjustment entries for customer # " + QPTrim$(fptxtAccount.Text) + " - " + QPTrim$(fptxtName.Text) + " and elected to continue the post.")
  End If
60:
  If fpcurrIssFeeAmt.DoubleValue < 0 Then
    ...
    MainLog ("Adjustment post aborted because of a negative issuance fee entry.")
    Close
    Exit Sub
  End If
70:
  If fpcurrPenAmt.DoubleValue < 0 Then
    ...
    MainLog ("Adjustment post aborted because of a negative penalty entry.")
    Close
    Exit Sub
  End If
80:
  For x = 0 To 4
    If fpcurrLicAmtDet(x).DoubleValue < 0 Then
      ...
      MainLog ("Adjustment post aborted because of a negative entry for category # " + CStr(x + 1) + ".")
      Close
      Exit Sub
    End If
  Next x
...
'  End If
  ...
  If optOverBill.Value = True Then
    If fpcurrPenBal.DoubleValue <= 0 And fpcurrPenAmt.DoubleValue > 0 Then
      ...
      fpcurrPenAmt.SetFocus
      MainLog ("Adjustment post aborted because the penalty adjustment amount would cause the penalty balance to be negative.")
      Close
      Exit Sub
    ElseIf fpcurrPenBal.DoubleValue - fpcurrPenAmt.DoubleValue < 0 Then
      ...
      fpcurrPenAmt.BackColor = &H80000005
      fpcurrPenAmt.SetFocus
      MainLog ("Adjustment post aborted because the issuance fee adjustment amount would cause the issuance fee balance to be negative.")
      Close
      Exit Sub
    End If
  End If
...
114:
    Adj$ = Adj$ + QPTrim$(fptxtDesc.Text)
    ARTransRec(1).TransDesc = QPTrim$(fptxtDesc.Text) 'Adj$
...
Private Sub Form_Load()
  ...
  MainLog ("Adjust customer balance screen opened.")
  DeletedFlag = False
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      KillFile "adjustbalance.dat"
      ClearInUse PWcnt
      MainLog ("BusinessLicense.exe terminated via menu bar on frmBLAdjustBal.")
      Call Terminate
      End
    End If
  End If
...
'  cmdExit.ToolTipText = "Press Escape to leave this screen and return to the Customer Maintenance menu."
  ...
  If GCustNum > 0 And GCustNum <= NumOfCustRecs Then
    'NOT OK to adjust if customer is included in an unposted license fee file
    If EmpInLicProcess(CStr(GCustNum)) = True And Exist("artmppst.dat") Then
   ...
   'OK to adjust if customer is involved in an unposted penalty file
    If EmpInPenProcess(CStr(GCustNum)) = True Then
      Get CHandle, GCustNum, CustRec
      frmBLMessageBoxJr.Label1.Caption = QPTrim$(CustRec.CustName) + " is currently involved in an unposted penalty assessment file."
      frmBLMessageBoxJr.Label1.Top = 800
      frmBLMessageBoxJr.Label1.Height = 1000
      frmBLMessageBoxJr.Show vbModal
    End If
    
    'OK to adjust if customer is in an unposted payment file
    If EmpInPayProcess(CStr(GCustNum)) = True Then
      Get CHandle, GCustNum, CustRec
      frmBLMessageBoxJr.Label1.Caption = QPTrim$(CustRec.CustName) + " is currently involved in an unposted payment file."
      frmBLMessageBoxJr.Label1.Height = 1000
      frmBLMessageBoxJr.Label1.Top = 800
      frmBLMessageBoxJr.Show vbModal
    End If
...
Private Sub LogSaves()
  ...
  If TempLicBal <> CustRec.LicBal Then
    MainLog ("For " + QPTrim$(CustRec.CustName) + ": " + "License balance has changed from " + QPTrim$(Using("$##,###,##0.00", TempLicBal)) + " to " + QPTrim$(Using("$##,###,##0.00", CustRec.LicBal)) + " in frmBLAdjustBal.")
  End If

  If TempPenBal <> CustRec.PenBal Then
    MainLog ("For " + QPTrim$(CustRec.CustName) + ": " + "Penalty balance has changed from " + QPTrim$(Using("$##,###,##0.00", TempPenBal)) + " to " + QPTrim$(Using("$##,###,##0.00", CustRec.PenBal)) + " in frmBLAdjustBal.")
  End If

  If TempIssBal <> CustRec.IssuanceBal Then
    MainLog ("For " + QPTrim$(CustRec.CustName) + ": " + "Issuance Fee balance has changed from " + QPTrim$(Using("$##,###,##0.00", TempIssBal)) + " to " + QPTrim$(Using("$##,###,##0.00", CustRec.IssuanceBal)) + " in frmBLAdjustBal.")
  End If

  If TempTotBal <> CustRec.AcctBal Then
    MainLog ("For " + QPTrim$(CustRec.CustName) + ": " + "Total balance has changed from " + QPTrim$(Using("$##,###,##0.00", TempTotBal)) + " to " + QPTrim$(Using("$##,###,##0.00", CustRec.AcctBal)) + " in frmBLAdjustBal.")
  End If
  ...
  If TempLicAmt <> SavedLicAmt Then
    ...
    For x = 0 To 4
      If ThisFee(x) <> TempLicDet(x) Then
        MainLog ("For " + QPTrim$(CustRec.CustName) + ": " + "Total balance for license #" + CStr(x) + " has changed from " + QPTrim$(Using("$##,###,##0.00", TempLicDet(x))) + " to " + QPTrim$(Using("$##,###,##0.00", ThisFee(x))) + " in frmBLAdjustBal.")
      End If
    Next x
  End If
