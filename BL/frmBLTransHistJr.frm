...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      ClearInUse PWcnt
      MainLog ("BusinessLicense.exe terminated via menu bar on frmBLTransHistJr.")
      Call Terminate
      End
    End If
  End If
...
Public Sub LoadMe()
  ...
  For x = TransCnt To 1 Step -1
    Get TransHandle, TransArray(x), TransRec
      ...
      End If
      If UpDown = "Down" Then
        fpList.AddItem "  " + MakeRegDate(TransRec.TransDate) + Chr(9) + TransDesc + Chr(9) + Using("$#,###,##0.00", -TransRec.TransAmount) + Chr(9) + Using("$#,###,##0.00", TransRec.BalanceAfterTrans) + Chr(9) + CStr(TransArray(x))
      Else
        fpList.AddItem "  " + MakeRegDate(TransRec.TransDate) + Chr(9) + TransDesc + Chr(9) + Using("$#,###,##0.00", TransRec.TransAmount) + Chr(9) + Using("$#,###,##0.00", TransRec.BalanceAfterTrans) + Chr(9) + CStr(TransArray(x))
      End If
  Next x
