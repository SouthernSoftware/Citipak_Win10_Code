...
'  End If
  DCLog "OUT: Decal Delete Payment" + " Oper:" + Oper$
  Unload Me
  DoEvents
End Sub
  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        DCLog "Closed via PaymentDelete by " + PWUser$ + " operator-" + Oper$
        CitiTerminate
      End If
    End If
  End If
...
Private Sub Form_Load()
  ...
  DCLog " IN Oper " + Oper$ + ": Decal Delete Payment"
  Me.HelpContextID = hlpDeleteDecal
  LoadPayList
End Sub
...
Private Sub LoadPayList()
  ...
  Oper$ = Str$(OperNum)
  PayFileName$ = DCPath$ + "DCPAY" + QPTrim(Oper$) + ".DAT"
  TPayFileName$ = DCPath$ + "DCPAY" + QPTrim(Oper$) + ".$$$"
 ...
 ' ReDim Picked(1 To TotalRecs&) As Integer
  ...
  Open PayFileName$ For Random Shared As PHandle Len = DCPayRecLen
  NumOfRecs& = LOF(PHandle) \ DCPayRecLen
  'TotalRecs& = NumOfRecs
  If NumOfRecs& > 0 Then
    For cnt& = 1 To NumOfRecs&
      Get PHandle, cnt&, DCPaymentRec(1)
      tmp$ = QPTrim(DCPaymentRec(1).CustName) & Chr$(9) & "Payment" & Chr$(9) & Using$(fmt$, Str$(DCPaymentRec(1).Amount)) & Chr$(9) & " " & Chr$(9) & cnt&
...
Private Sub DotheDelete()
  Dim FntSize As Integer, DCvFile As Integer, NumOfVRecs As Long
  Dim PCnt As Integer, NumPicked As Integer, PFile As Integer, VehRecord As Long
  Dim PayFileName As String, DCPayRecLen As Integer, TFile As Integer
  Dim TPayFileName As String, cnt As Integer, DCVehReclen As Integer
  ReDim DCPaymentRec(1) As DCEditPaymentRecType
  ReDim DCVRec(1) As DCVehType

  PayFileName$ = DCPath$ + "DCPAY" + QPTrim$(Str$(OperNum)) + ".DAT"
  TPayFileName$ = DCPath$ + "DCPAY" + QPTrim$(Str$(OperNum)) + ".$$$"
  DCPayRecLen = Len(DCPaymentRec(1))
  
  ReDim MsgText(0 To 5) As String
  For PCnt = 1 To fplstPayments.ListCount
    If fplstPayments.Selected(PCnt - 1) Then
  ...
  ElseIf NumPicked > 0 Then
    If MsgBox("Are You Sure You Wish to Continue With Deletion?", vbYesNo, "Delete") = vbYes Then
      PFile = FreeFile
      Open PayFileName$ For Random Shared As PFile Len = DCPayRecLen
      TFile = FreeFile
      Open TPayFileName$ For Random Shared As TFile Len = DCPayRecLen
      For PCnt = 1 To fplstPayments.ListCount
        If Not fplstPayments.Selected(PCnt - 1) Then
          fplstPayments.col = 4
          fplstPayments.Row = PCnt - 1
          cnt = QPTrim(fplstPayments.ColList)
          Get PFile, cnt, DCPaymentRec(1)
          Put TFile, , DCPaymentRec(1)
      ...
      DCLog "Delete Decal " + Str(NumPicked) + " Payment(s)" + " Oper:" + Oper$
    Else
      Exit Sub
    End If
  End If
...
UpdateVehRecord:
  ...
  VehRecord = DCPaymentRec(1).VehRecord
  If VehRecord <= 0 Or VehRecord > NumOfVRecs Then
    Close DCvFile
  Else
