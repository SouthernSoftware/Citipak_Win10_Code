...
Begin VB.Form frmMtrCoorEdit 
   ...
   Begin fpBtnAtlLibCtl.fpBtn fpCmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmMtrCoorEdit.frx":08CA
   End
   Begin fpBtnAtlLibCtl.fpBtn CmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmMtrCoorEdit.frx":0AA6
   End
   Begin fpBtnAtlLibCtl.fpBtn fpCmdInfo 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmMtrCoorEdit.frx":0C82
   End
   Begin EditLib.fpDoubleSingle fpLatitude 
      BeginProperty DataFormat 
         Type            =   1
   ...
   End
End
Attribute VB_Name = "frmMtrCoorEdit"
...
Dim BeenDone As Boolean, Answer As Integer
...
Private Sub Form_Activate()
  If Val(fpCustRecNo) > 0 And Not BeenDone Then
    ...
    LabelAcctNo.Caption = fpCustRecNo
    loadCustrec
    DoEvents
  End If
...
Private Sub cmdExit_Click()
    ...
    Case False:  '0= exit
      'go ahead and exit
    Case Else
      Exit Sub 'stay right where you are
    End Select
  CustAcct = 0
  fpCustRecNo = 0
  BeenDone = False
  ...
  If codeopt = 0 Then
    ...
    frmUBCustMenu.Show
  End If
  UBLog "OUT: UTIL EditLatLng" + " Oper:" + PWUser
  Unload Me
  DoEvents
End Sub
...
Private Sub fpCmdSave_Click()
 DeActivateControls frmMtrCoorEdit
  If MsgBox("Are you sure you wish to save these coordinates?", vbYesNo, "Save Transaction") = vbYes Then
      ...
      UBLog "OUT: UTIL MtrCoorEdit"
      Unload Me
      DoEvents
    End If
   ActivateControls frmMtrCoorEdit
End Sub


Private Sub fpLatitude_ChangeMode(Index As Integer, EditMode As Integer)
  EditMode = True
End Sub

Private Sub fpLatitude_KeyDown(Index As Integer, KeyCode As Integer, Shift As Integer)
  If KeyCode = vbKeyDown Or KeyCode = vbKeyReturn Then
    fpLongitude(Index).SetFocus
    KeyCode = 0
  ElseIf KeyCode = vbKeyUp Then
    If Index > 0 Then
      fpLongitude(Index - 1).SetFocus
    Else
      fpCmdSave.SetFocus
    End If
    KeyCode = 0
  End If
End Sub
Private Sub fpLongitude_ChangeMode(Index As Integer, EditMode As Integer)
  EditMode = True
End Sub

Private Sub fpLongitude_KeyDown(Index As Integer, KeyCode As Integer, Shift As Integer)
  If KeyCode = vbKeyDown Or KeyCode = vbKeyReturn Then
    If Index < 6 Then
      fpLatitude(Index + 1).SetFocus
    Else
      fpCmdSave.SetFocus
    End If
    KeyCode = 0
  ElseIf KeyCode = vbKeyUp Then
    fpLatitude(Index).SetFocus
    KeyCode = 0
  End If
End Sub
...
'  End If
  ...
  Close CustFile
  LabelAcctNo.Caption = Str$(CustAcct&)
 ...
 ' fpSearch = QPTrim$(UBCustRec(1).SEARCH)
  fpCustName = QPTrim$(UBCustRec(1).CustName)
  'LblInfo.Caption = QPTrim$(tmpCustRec.CustName)
 ' fptxtAddr1 = QPTrim$(UBCustRec(1).ADDR1)
 ' fptxtAddr2 = QPTrim$(UBCustRec(1).ADDR2)
  fpServAddr = QPTrim$(UBCustRec(1).ServAddr)
'  fpCity = QPTrim$(tmpCustRec.CITY)
'  fpState = QPTrim$(tmpCustRec.STATE)
'  fpZip = QPTrim$(tmpCustRec.ZIPCODE)

  For cnt = 0 To 6
    fpMtrSerial(cnt) = QPTrim$(UBCustRec(1).LocMeters(cnt + 1).MtrNum)
    fpMtrIDNO(cnt).Text = QPTrim$(UBCustRec(1).LocMeters(cnt + 1).MtrIDNO)
    fpLatitude(cnt) = UBCustRec(1).LocMeters(cnt + 1).MtrLat
    fpLongitude(cnt) = UBCustRec(1).LocMeters(cnt + 1).MtrLng
  Next
    fpLatitude(0).SetFocus
    fpLongitude(0).SetFocus
    fpLatitude(0).SetFocus
  Exit Sub
SkiptoHere:
...
Private Function CheckSaveCustFile%()
  Dim Changed As Boolean, cnt As Integer
  ...
  If CustAcct& > 0 Then
    ...
    Get #UBHandle, CustAcct&, chkCustRec
    Close UBHandle
    For cnt = 0 To 6
      If chkCustRec.LocMeters(cnt + 1).MtrLat <> Val(fpLatitude(cnt)) Then
        Changed = True
        GoTo DoneCustChk
        Exit For
      End If
      If chkCustRec.LocMeters(cnt + 1).MtrLng <> Val(fpLongitude(cnt)) Then
        Changed = True
        GoTo DoneCustChk
        Exit For
      End If
...
Private Sub SaveTransaction()
  ...
  Dim ReindexFlag As Boolean, cnt As Integer
  ...
  If CustAcct > 0 Then
    UBHandle = FreeFile
    Open UBCustFile For Random Shared As UBHandle Len = CustRecLen
    Get #UBHandle, CustAcct&, tmpCustRec(1)
    Close UBHandle
    LSet tmpCustRec(2) = tmpCustRec(1) 'copy for reindex comparison check below
  End If

  For cnt = 0 To 6
    tmpCustRec(1).LocMeters(cnt + 1).MtrLat = fpLatitude(cnt)
    tmpCustRec(1).LocMeters(cnt + 1).MtrLng = fpLongitude(cnt)
  ...
  Put #UBHandle, CustAcct&, tmpCustRec(1)
  Close UBHandle
  UBLog "Saved MtrLatlng for Acct: " + Str(CustAcct&) + "," + QPTrim$(fpStatus.Text) + "," + QPTrim$(fpCustName.Text) + "," + QPTrim$(fpBook.Text) + "-" + QPTrim$(fpSeqNumb.Text)
