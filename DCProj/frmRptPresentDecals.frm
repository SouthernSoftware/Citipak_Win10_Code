...
Begin VB.Form frmRptPresentDecals 
   BackColor       =   &H008F8265&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Present Decal Listing"
   ClientHeight    =   8640
   ClientLeft      =   36
   ClientTop       =   540
   ClientWidth     =   12192
   Icon            =   "frmRptPresentDecals.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   8640
   ...
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptPresentDecals.frx":08CA
   End
   Begin LpLib.fpCombo fpcboPrintOrder 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptPresentDecals.frx":0D14
   End
   Begin LpLib.fpCombo fpDecalCat 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptPresentDecals.frx":1153
   End
   ...
   Begin VB.Label Label1 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "Print Present Decal List"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   13.8
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmRptPresentDecals"
  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        DCLog "Closed via RptDecalListing by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpPresentDecal
End Sub
...
Private Sub PDecalListing()
  ...
  FrmShowPctComp.Label1 = "Creating Present Decal List"
  ...
  ReportFile$ = "DCPrDcls.PRN"  'Report File Name
  FF$ = Chr$(12)
  MaxLines = 58
  Linecnt = 0
  CustCnt = 0
...
SkipRecord:
  Next cnt
  GoSub PrintTheEnding
  If RptType = 1 Then Print #RptHandle, Chr$(18);   ' oki 320 10 cpi
  Close         'Close all open files now

  Header$ = "Present Decal Listing"
  If RptType = 1 Then
    ViewPrint ReportFile$, Header$
    Kill ReportFile$
  Else
    Load frmLoadingRpt
    frmLoadingRpt.setwherefrom frmRptPresentDecals
    ARptPresentDecals.txtDate = Now
    ARptPresentDecals.txtTown = TOWNNAME$
    ARptPresentDecals.Catfield = "Category -" + Category$
    ARptPresentDecals.Title = "Present Decal Listing"
    ARptPresentDecals.totcust = CustCnt
    ARptPresentDecals.GetName ReportFile$
    ARptPresentDecals.startrpt
  End If
ExitPrint:
  Exit Sub
ProcessThisOneCust:
    If RptType = 1 Then
      ...
      Print #RptHandle, DCCustREc(1).BILLNAME; Tab(60); "Acct # "; Str(AcctNo&)
      Print #RptHandle, RTrim$(DCCustREc(1).ADDRESS1)
      Print #RptHandle, RTrim$(DCCustREc(1).ADDRESS2)
      Print #RptHandle, RTrim$(DCCustREc(1).City); ", "; DCCustREc(1).State; " "; DCCustREc(1).ZIPCODE
      Print #RptHandle, "Vehicle Listing  "
      Print #RptHandle, "Make/Model"; Tab(26); "Tag #"; Tab(41); "VIN #"; Tab(56); "Sticker"; Tab(65); "Expire Date"
      Linecnt = Linecnt + 7
    End If
    CustCnt = CustCnt + 1
    Dothiscust = False
...
ProcessThisOne:
  If Dothiscust Then GoSub ProcessThisOneCust
    If RptType = 1 Then
      Print #RptHandle, Left$(DCVRec(1).makemodel, 20);
      Print #RptHandle, Tab(22); Left$(DCVRec(1).StateTag, 12);
      Print #RptHandle, Tab(35); Left$(DCVRec(1).Desc, 20);
      Print #RptHandle, Tab(56); Left$(DCVRec(1).Sticker, 9);
      Print #RptHandle, Tab(65); Num2Date$(DCVRec(1).ExpireDate)
      Linecnt = Linecnt + 1
    Else
      ToPrint$ = Str(AcctNo&) + "~" + QPTrim$(DCCustREc(1).BILLNAME)
      ToPrint$ = ToPrint$ + "~" + RTrim$(DCCustREc(1).ADDRESS1) + "~" + RTrim$(DCCustREc(1).ADDRESS2)
      ToPrint$ = ToPrint$ + "~" + RTrim$(DCCustREc(1).City) + "~" + DCCustREc(1).State + "~" + QPTrim$(DCCustREc(1).ZIPCODE)
      ToPrint$ = ToPrint$ + "~" + RTrim$(DCVRec(1).makemodel) + "~" + QPTrim$(DCVRec(1).StateTag)
      ToPrint$ = ToPrint$ + "~" + RTrim$(DCVRec(1).Desc) + "~" + RTrim$(DCVRec(1).Sticker) + "~" + Num2Date$(DCVRec(1).ExpireDate)
...
PrintTheHeader:
 If RptType <> 0 Then
  Page = Page + 1
  Print #RptHandle, Tab(22); "Va. Decal System : Present Decal Listing"
...
ERRORSTUFF:
  Unload FrmShowPctComp
  Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "RptPresentDecals", "Calc Report", Erl)
    Case emrExitProc:
      Resume Proc_Exit
    Case emrResume:
      Resume
