  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        UBLog "Closed via RptTransSummary by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
DefineType:
  ...
  Case 1, 101  'Bills col 1 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 1) = Round#(RevTotals(RCnt, 1) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 4, 104 'Payments col 2 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 2) = Round#(RevTotals(RCnt, 2) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 5, 105 'Applied Dep col 3 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 3) = Round#(RevTotals(RCnt, 3) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 6 'Penalties col 4 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 4) = Round#(RevTotals(RCnt, 4) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 7, 107 'Dep Payment col 5 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 5) = Round#(RevTotals(RCnt, 5) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 8  'Draft Payment col 6 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 6) = Round#(RevTotals(RCnt, 6) + (UBTransRec(1).RevAmt(RCnt) * -1))
    Next
'''''??????'    Amount# = UBTransRec(1).Transamt * -1
  Case 9, 109  'Refund Dep col 7 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 7) = Round#(RevTotals(RCnt, 7) + Abs(UBTransRec(1).RevAmt(RCnt)))
    Next
'''''???????'    Amount# = Abs(UBTransRec(1).Transamt)
  Case 11, 111 'Up Adj Bill col 8 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 8) = Round#(RevTotals(RCnt, 8) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 12, 112  'Down Adj Bill col 9 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 9) = Round#(RevTotals(RCnt, 9) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 33   'Over Pay Adj col 10 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 10) = Round#(RevTotals(RCnt, 10) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 37   'Dep Credit Rem col 11 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 11) = Round#(RevTotals(RCnt, 11) + UBTransRec(1).RevAmt(RCnt))
    Next
  Case 39
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 11) = Round#(RevTotals(RCnt, 11) + UBTransRec(1).RevAmt(RCnt))
  ...
  Case Else '99 or any other type col 12 in matrix
    For RCnt = 1 To MaxRevenue
      RevTotals(RCnt, 12) = Round#(RevTotals(RCnt, 12) + UBTransRec(1).RevAmt(RCnt))
    Next
  End Select
  Return
