VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} ARptNOMtrNum 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "No Serial Number Report"
   ClientHeight    =   4380
   ClientLeft      =   36
   ClientTop       =   264
   ClientWidth     =   9036
   Icon            =   "ARptPOEdits.dsx":0000
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   _ExtentX        =   15939
   _ExtentY        =   7726
   SectionData     =   "ARptPOEdits.dsx":08CA
End
Attribute VB_Name = "ARptNOMtrNum"
...
'    ' or user defined.
    ...
    Fields("PONum").Value = arr(2)
    Fields("Amount").Value = arr(3)
    Fields("Dept").Value = arr(4)
    Fields("AcctNum").Value = arr(5)
    Fields("AcctAmt").Value = arr(6)
    Fields("Over").Value = arr(7)
...
Private Sub ActiveReport_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
  ...
  If Shift = 4 Then
    ...
    If KeyCode = vbKeyE Then
      ...
      DoEvents
      MsgBox "File - PONApprv.xls, created in the Citipak Directory.", vbOKOnly
    End If
    If KeyCode = vbKeyT Then
      ...
      Screen.MousePointer = vbDefault
      DoEvents
      MsgBox "File - PONApprv.txt, created in the Citipak Directory.", vbOKOnly
    End If
  End If
...
Private Sub ExportReport(x As Integer)
  ...
  Select Case x
    Case 1   '"Excel"
        ...
        oEXL.FileName = outfile & "PONApprv.xls"
        oEXL.Export Me.Pages
    Case 2   '"Text"
        Set oTXT = New ActiveReportsTextExport.ARExportText
        oTXT.FileName = outfile & "PONApprv.txt"
        oTXT.PageDelimiter = ";"
        oTXT.TextDelimiter = ","
