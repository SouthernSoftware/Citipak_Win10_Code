...
Begin VB.Form frmRptOwner 
   ...
   Begin LpLib.fpCombo fpDecalCat 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptOwner.frx":08CA
   End
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptOwner.frx":0C45
   End
   Begin LpLib.fpCombo fpcboPrintOrder 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptOwner.frx":0F73
   End
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpOwnerReport
End Sub
...
Private Sub OwnerListing()
  ...
  FrmShowPctComp.Label1 = "Creating Owner List"
...
ProcessThisOne:
  If Dothiscust Then GoSub ProcessThisOneCust
    If RptType = 1 Then
      ...
      Linecnt = Linecnt + 1
    Else
     '''''' If AcctNo& = 142 Then Stop
      ToPrint$ = Str(AcctNo&) + "~" + QPTrim$(DCCustREc(1).BILLNAME)
     '''ToPrint$ = Str(VehRecord) + "~" + Str(DCVRec(1).MasterRecord)
...
ERRORSTUFF:
  Unload FrmShowPctComp
  Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "RptOwner", "Calc Report", Erl)
    Case emrExitProc:
      Resume Proc_Exit
    Case emrResume:
      Resume
...
'     '''''' If AcctNo& = 142 Then Stop
'      '''ToPrint$ = Str(AcctNo&) + "~" + QPTrim$(DCCustRec(1).BILLNAME)
'      ToPrint$ = Str(cnt) + "~" + Str(DCVREc(1).MasterRecord)
'      ToPrint$ = ToPrint$ + "~" + " " + "~" + " "
'      ToPrint$ = ToPrint$ + "~" + Str$(DCVREc(1).NextRec)
