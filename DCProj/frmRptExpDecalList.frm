...
Begin VB.Form frmRptExpDecalList 
   BackColor       =   &H008F8265&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Expired Decal Listing"
   ClientHeight    =   8640
   ClientLeft      =   36
   ClientTop       =   540
   ClientWidth     =   12192
   Icon            =   "frmRptExpDecalList.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   8640
   ...
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptExpDecalList.frx":08CA
   End
   Begin LpLib.fpCombo fpcboPrintOrder 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptExpDecalList.frx":0BF8
   End
   Begin LpLib.fpCombo fpDecalCat 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptExpDecalList.frx":0F1B
   End
   ...
   Begin VB.Label Label1 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "Print Expired Decal List"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   13.8
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmRptExpDecalList"
  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        DCLog "Closed via RptExpDecalList by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
Private Sub Form_Load()
  ...
  txtExpDate.Text = Format(Now, "mm/dd/yyyy")
  FillCatCMBOAll fpDecalCat
  fpDecalCat.ListIndex = 0
  Me.HelpContextID = hlpExpiredDecal
End Sub
...
Private Sub ExpDecalListing()
  ...
  Dim ReportFile As String, DCVehReclen As Integer, ExpDate As Integer
  ...
  FrmShowPctComp.Label1 = "Creating Decal List"
  ...
  Case Else
  End Select
  ReportFile$ = "DCEXPLIK.PRN"  'Report File Name
  FF$ = Chr$(12)
  MaxLines = 53
  Linecnt = 0
  CustCnt = 0
        ...
        If RTrim$(DCVRec(1).DecalCat) = CatSel Or CatSel = "ALL" Then
          If DCVRec(1).Active <> "N" Then
           If DCVRec(1).ExpireDate <= ExpDate Then
            If RptType = 1 Then
              Print #RptHandle, Str(AcctNo&); Tab(10); QPTrim$(DCCustRec(1).BILLNAME);
            ...
            Else
             ...
             ToPrint$ = ToPrint$ + "~" + LTrim$(DCVRec(1).Sticker)
             ToPrint$ = ToPrint$ + "~" + Num2Date$(DCVRec(1).ExpireDate)
  ...
  Else
    Load frmLoadingRpt
    frmLoadingRpt.setwherefrom frmRptExpDecalList
    ARptDecalList.txtDate = Now
    ARptDecalList.txtTown = TOWNNAME$
    ARptDecalList.Catfield = "Category -" + Category$
    ARptDecalList.Title = "Expired Decal Listing"
    ARptDecalList.totcust = CustCnt
    ARptDecalList.GetName ReportFile$
    ARptDecalList.startrpt
  End If
ExitPrint:
  Exit Sub
...
If RptType = 1 Then
  ...
  Print #RptHandle, "Category "; Category$; Tab(50); "Expiration Date: "; Num2Date$(ExpDate)
...
ERRORSTUFF:
  Unload FrmShowPctComp
  Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "RptExpDecalList", "Calc Report", Erl)
    Case emrExitProc:
      Resume Proc_Exit
    Case emrResume:
      Resume
