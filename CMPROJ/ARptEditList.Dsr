VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} ARptEditList 
   ...
   _ExtentX        =   15939
   _ExtentY        =   7726
   SectionData     =   "ARptEditList.dsx":08CA
End
Attribute VB_Name = "ARptEditList"
...
Public Sub GetName(RName As String, SName As String)
  ...
  SubFile$ = SName$
End Sub
Public Sub GetName2(RName As String)
  ReportFile$ = RName$
End Sub
...
'    ' or user defined.
    Fields("Date").Value = arr(0)
    Fields("Desc").Value = arr(1)
    Fields("Ref").Value = arr(2)
    Fields("AcctNo").Value = arr(3)
    Fields("AcctName").Value = arr(4)
...
Private Sub ActiveReport_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
  ...
  If Shift = 4 Then
    ...
    If KeyCode = vbKeyE Then
      ...
      DoEvents
      MsgBox "File - Editrpt.xls, created in the Citipak Directory.", vbOKOnly
    End If
    If KeyCode = vbKeyT Then
      ...
      Screen.MousePointer = vbDefault
      DoEvents
      MsgBox "File - Editrpt.txt, created in the Citipak Directory.", vbOKOnly
    End If
  End If
...
Private Sub ExportReport(x As Integer)
  ...
  Select Case x
    Case 1   '"Excel"
        ...
        oEXL.FileName = outfile & "Editrpt.xls"
        oEXL.Export Me.Pages
    Case 2   '"Text"
        Set oTXT = New ActiveReportsTextExport.ARExportText
        oTXT.FileName = outfile & "Editrpt.txt"
        oTXT.PageDelimiter = ";"
        oTXT.TextDelimiter = ","
...
Private Sub ReportFooter_Format()
  If RType <> 2 Then
    Set Me.SubReport1.object = New ARSubFundSummary
  Else
