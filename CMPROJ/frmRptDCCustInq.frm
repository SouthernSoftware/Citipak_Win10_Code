...
Begin VB.Form frmRptDCCustInq 
   BackColor       =   &H008F8265&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Decal Customer Inquiry"
   ClientHeight    =   8640
   ClientLeft      =   36
   ClientTop       =   540
   ClientWidth     =   12192
   Icon            =   "frmRptDCCustInq.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin EditLib.fpLongInteger fpVehOne 
      Height          =   300
      Left            =   1056
      TabIndex        =   44
      TabStop         =   0   'False
   ...
   Begin fpBtnAtlLibCtl.fpBtn fpCmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmRptDCCustInq.frx":08CA
   End
   Begin fpBtnAtlLibCtl.fpBtn fpCmdVehicles 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmRptDCCustInq.frx":0AA6
   End
   Begin fpBtnAtlLibCtl.fpBtn fpCmdTranHist 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmRptDCCustInq.frx":0C85
   End
   Begin fpBtnAtlLibCtl.fpBtn fpCmdPrintInfo 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmRptDCCustInq.frx":0E66
   End
   Begin VB.Line Line4 
   ...
   End
End
Attribute VB_Name = "frmRptDCCustInq"
...
Private Sub fpcmdPrintinfo_Click()
  If fpCustRecNo > 0 Then
    ...
    If rptopt = 1 Then
    'do the graphics
     PrintDCCustInfo fpCustRecNo, 1
    ElseIf rptopt = 2 Then
    'do the text
      PrintDCCustInfo fpCustRecNo, 2
    End If
   ActivateControls Me
  Else
    ActivateControls Me
...
startloop:
  ...
  If VehRec > 0 Then
    frmRptVehInq.fpVehRecNo = VehRec
    frmRptVehInq.Caption = "Vehicle Information  - " & QPTrim$(fpCustName)
    frmRptVehInq.fpCustRecNo = fpCustRecNo
    'frmCustVehicles.SetVehScreen
    frmRptVehInq.Show 1
  Else
    MsgBox "No Vehicles to Display.", vbOKOnly, "No Vehicles Found"
  End If
...
Private Sub fpCmdTranHist_Click()
  ...
  If fpCustRecNo > 0 Then
    ...
    If DCCustREc(1).FirstTrans > 0 Then
      'DeActivateControls Me
      DisplayDCCustTransList fpCustRecNo
      'ActivateControls Me
    Else
...
Private Sub LoadCustInfo()
  ...
  fpAcctBalance = Using$("$#,###.##", DCCustREc(1).AcctBal)
  ...
  If fpVehOne > 0 Then
    fpVehonfile = "Yes"
    fpCmdVehicles.Enabled = True
  Else
    fpVehonfile = "No"
    fpCmdVehicles.Enabled = False
  End If
End Sub
Private Sub Gettheveh()
  Dim NumOfVRecs As Long, DCvFile As Integer, DCVehReclen As Integer
  ReDim DCVRec(1) As DCVehType
startloop:
  If fpVehOne > 0 Then
