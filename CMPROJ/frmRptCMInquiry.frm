...
Begin VB.Form frmRptCMInquiry 
   ...
   Begin LpLib.fpCombo fpcboSource 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCMInquiry.frx":08CA
   End
   ...
   End
End
Attribute VB_Name = "frmRptCMInquiry"
  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        CMLog "Closed via RptCMInquiry by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
 ...
 ' ReDim TRList(1 To 1) As FLen2
  ...
  If NumOfRecs& > 0 Then
    For cnt& = 1 To NumOfRecs&
      ...
      totPay = 0
      If FromDate <= CMTrRec(1).TransDate And ThruDate >= CMTrRec(1).TransDate Then
      GoSub CheckForMatch
      If GotOne Then
        Build$ = Space(100)
        MatchCnt& = MatchCnt& + 1
        ...
        Case 1, 201
          ...
          If CMTrRec(1).TransSource = 201 Then
            TRType$ = "Void Misc"
          End If
        Case 20 To 29, 224, 227
          ...
          If CMTrRec(1).TransSource = 224 Then
            TRType$ = "Void Util"
          ElseIf CMTrRec(1).TransSource = 227 Then
            TRType$ = "Void UDep"
          End If
        Case 30 To 39, 131, 231, 161, 261, 171, 271
          TRType$ = " Tax"
          Change# = Round#((CMTrRec(1).TransCheck + CMTrRec(1).TransCash) - CMTrRec(1).TransAmount) 'CMTrRec(1).TransAmtOwed)
          Select Case CMTrRec(1).TransSource
            Case 231, 261, 271
            TRType$ = "Void Tax"
          End Select
        Case 40 To 49, 141, 241
          TRType$ = " Lic"
          Change# = Round#((CMTrRec(1).TransCheck + CMTrRec(1).TransCash) - CMTrRec(1).TransAmount) 'CMTrRec(1).TransAmtOwed)
          If CMTrRec(1).TransSource = 241 Then
            TRType$ = "Void Lic"
          End If
        Case 50 To 59, 151, 251
          TRType$ = "Decl"
          If CMTrRec(1).TransSource = 251 Then
            TRType$ = "Void Decl"
          End If
        Case Else
        ...
        frmCMDispList.fpCMList.InsertRow = Build$
...
'        End If
  ...
  If MatchCnt& = 0 Then
   ...
   'ActivateControls Me
    ...
    Exit Sub
  Else
    frmCMDispList.Wheretogo frmRptCMInquiry, frmRptCMInquiry, 2 'code 2 for list
