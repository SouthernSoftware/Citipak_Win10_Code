...
Begin VB.Form frmVATaxPaymentEntry 
   ...
   Caption         =   "CM Real Property Tax Payment Entry"
   ClientHeight    =   8730
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   11640
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdCash 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPaymentEntry.frx":0F7F
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdCheck 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPaymentEntry.frx":115A
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdCharge 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPaymentEntry.frx":1336
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdDist 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPaymentEntry.frx":1513
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPaymentEntry.frx":16EE
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPaymentEntry.frx":18CA
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdInfo 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPaymentEntry.frx":1AA6
   End
   Begin fpBtnAtlLibCtl.fpBtn fpcmdDrawer 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPaymentEntry.frx":1C81
   End
   ...
   Begin VB.Label Label8 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "CM Tax Real Payment  Entry"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   13.5
         Charset         =   0
...
Option Explicit
  ...
  Dim FirstBillRec As Long, Lunney As Boolean
  ...
  Dim TempBillList() As VARealPayListType
...
Public Sub cmdBills_Click()
  ...
  Dim ThisCust As Long, luncnt As Long, Laugh As Long
  DoEvents
...
'  End If
  ...
  If VAGetCustRealBalance(GCustNum, -1) = 0 Then
...
'  If EditFlag = True Then Exit Sub 'critical
  ...
  ThisCust = 0
  If BillCnt > 0 Or Exist(VATempRealBillRecs) Then 'BillCnt is a temporary value representing
...
Private Sub cmdDist_Click()
  ...
  Dim TPayRec As VARealPayListType
  Dim PayRec As VARealPayListType
...
'  WhatsLeft = OldRound(CDbl(fpCurrTotRecd.Value) + CDbl(fpCurrDisc.Value))
  ...
  'the bill list
    ...
    Next x
    Close PHandle
  ElseIf Exist(VATempRealBillRecs) Then
    ReDim BillTrans(1 To 1) As Long
    ReDim BillDate(1 To 1) As Integer
    BillCnt = 0
...
Private Sub cmdExit_Click()
  ...
  KillFile VATempRealBillRecs 'TempBillRecs is the filename for the temporary file
...
''this is diff from nctax
  If fpLongAcctNum <= 0 And GCustNum <= 0 Then
    Exit Sub
  End If
...
'
  ...
  If SavePay = True Then
    ...
    End If
    Oper$ = CStr(OperNum)
      KillFile ("CMXRCPR" + Oper$ + ".DAT")
      KillFile ("CMXRLOP" + Oper$ + ".DAT")
...
Dothesave:
  ...
  TaxPayRec.TotPaid = OldRound(TaxPayRec.TotPaid + CDbl(fpCurrRevOpt2Paid.Value) + CDbl(fpCurrRevOpt3Paid)) + CDbl(fpCurrPrePay.Value)
  TaxPayRec.PrePayAmt = CDbl(fpCurrPrePay.Value)
  TaxPayRec.CustPin = TaxCustRec.PIN
  TaxPayRec.BillType = "R"
    Put PayHandle, 1, TaxPayRec
  
'  KillFile VATempRealBillRecs 'get rid of all temporary files and records in
...
'  End If
  ...
  DontExit = False
'  Call VASavemsg(900, "This real tax payment has been saved successfully.")
  PostEmTax
  BillCnt = 0
  ReDim BillTrans(0 To 0) As Long
...
Private Sub fpCurrDisc_LostFocus()
  ...
  If CDbl(fpCurrDisc.Value) > MaxDisc Then
    Call VATaxMsg(800, "The maximum discount allowed for this customer is " + QPTrim$(Using$("$##,##0.00", MaxDisc)) + ". The program will reset the discount to the maximum allowed.")
    fpCurrDisc = MaxDisc
...
Public Sub Clearscreen()
  ...
  GCustNum = 0
  GPayNum = 0
  ReDim TempBillList(1 To 1) As VARealPayListType
  TempBillListCnt = 0
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPaymentEntry", "VACheck4ValidCustNum", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Public Sub EnterEditChk()
  ...
  Select Case VABegBalCheck(CustNum, ONum$, ThisRec, ThisBillType)
    Case 1 'normal first time transaction for this customer
      ...
      FindStr = FindVACustInBatchFile(CustNum, "R")
      If FindStr <> "0" Then
...
Private Sub LoadHerUpWOEdit()
  ...
  DiscYN = False
  ThisBalance = VAGetCustRealBalance(GCustNum, -1)
  ...
  ElseIf ThisBalance < 0 Then
    Call VATaxMsg(900, "This customer has a balance of -" + QPTrim$(Using$("$##,##0.00", Abs(ThisBalance))) + ".")
...
Private Sub LoadAmtOwed()
  ...
  Call GetMaxDisc
  If MaxDisc > 0 Then
    If VAGetCustRealBalance(GCustNum, ThisTaxYear) > 0 Then
      ...
      Else
        ...
        Call VATaxMsg(900, "The total real amount owed including the discount will be " + QPTrim$(Using$("$##,##0.00", Dif)) + ".")
      End If
    Else
...
Private Function VACheck4Discounts() As Integer
  ...
  Dim x As Integer
  Dim TaxYear As Integer
  Dim ListRec As VARealPayListType
  ...
  If PayTally = CDbl(fpCurrTotOwed.Value) Then
    If VAGetCustRealBalance(GCustNum, TaxYear) > PayTally Then 'if true than old balance exists
  ...
  If InSave = True And CDbl(fpCurrTotWDisc.Value) < CDbl(fpCurrTotOwed.Value) Then
    If VAGetCustRealBalance(GCustNum, TaxYear) > 0 Then 'if true than old balance exists
      DiscOK = 2 'discount allowed but warn that some old balance is outstanding
    Else
      DiscOK = 1 'discount allowed...no warnings necessary
    End If
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPaymentEntry", "VACheck4Discounts", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub UPDateListOfPayments()
  ...
  If BillCnt = 0 And VAGetCustRealBalance(GCustNum, -1) <= 0 Then 'customer
...
Private Function AddUpPaidCol() As Double
  ...
  ThisAdd = OldRound(ThisAdd + CDbl(fpCurrPenPaid.Value))
...
Private Sub ApplyDiscount()
  ...
  TXLog ("frmVATaxPaymentEntry: Customer, " + fptxtName.Text + ", is eligible for a discount of " + QPTrim$(Using$("$#,##0.00", ThisDiscAmt)) + " and the user allowed the discount to apply.")
...
Private Sub Distribute(WhatsLeft As Double)
  ...
  End If
    ...
    If CDbl(fpCurrAmtOwed) < CDbl(fpCurrCashPd) + CDbl(fpCurrChkChrgPd) + CDbl(fpCurrDisc) Then '1/25/07
      Call VATaxMsg(900, "Overpayments are not allowed when applying discounts.")
      fpCurrDisc.SetFocus
      Exit Sub
    End If
  End If
...
'  PrePayAmt = CDbl(fpCurrPrePay.Value)
  ...
  ReDim Preserve TempBillList(1 To BillCnt) As VARealPayListType
  TempBillListCnt = 0
  For x = 1 To BillCnt
 ...
 'WhatsLeft = OldRound(WhatsLeft - TempBillList(x).DiscAmt)'comment out 1/19/07 per BOB
...
Private Sub RemoveDiscount()
  Dim x As Integer
'  Dim TempPayRec As VARealPayListType
'  Dim THandle As Integer
...
Private Sub OverRideDist()
  ...
  If CDbl(fpCurrAdvColOwed.Value) > CDbl(fpCurrAdvColPaid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      TXLog ("WARNING: User warned that overriding the automatic adv/collect payment of " + QPTrim$(Using("$##,##0.00", TempAdvColPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new adv/collect amount of " + fpCurrAdvColPaid.Text + ".")
    End If
  End If
  ...
  If CDbl(fpCurrLateListOwed.Value) > CDbl(fpCurrLateListPaid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      TXLog ("WARNING: User warned that overriding the automatic late list payment of " + QPTrim$(Using("$##,##0.00", TempLateListPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new late list amount of " + fpCurrLateListPaid.Text + ".")
    End If
  End If
  ...
  If CDbl(fpCurrPenOwed.Value) > CDbl(fpCurrPenPaid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      TXLog ("WARNING: User warned that overriding the automatic penalty payment of " + QPTrim$(Using("$##,##0.00", TempLateListPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new late list amount of " + fpCurrLateListPaid.Text + ".")
    End If
  End If
  ...
  If CDbl(fpCurrPrincOwed.Value) > OldRound(CDbl(fpCurrPrincPaid.Value) + CDbl(fpCurrDisc.Value)) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      TXLog ("WARNING: User warned that overriding the automatic principle payment of " + QPTrim$(Using("$##,##0.00", TempPrincPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new interest amount of " + fpCurrPrincPaid.Text + ".")
    End If
  End If
  ...
  If CDbl(fpCurrRevOpt1Owed.Value) > CDbl(fpCurrRevOpt1Paid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      TXLog ("WARNING: User warned that overriding the automatic " + OptRev + " payment of " + QPTrim$(Using("$##,##0.00", TempRev1Paid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new interest amount of " + fpCurrRevOpt1Paid.Text + ".")
    End If
  End If
...
Next2:
  ...
  If CDbl(fpCurrRevOpt2Owed.Value) > CDbl(fpCurrRevOpt2Paid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      TXLog ("WARNING: User warned that overriding the automatic " + OptRev + " payment of " + QPTrim$(Using("$##,##0.00", TempRev2Paid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new interest amount of " + fpCurrRevOpt2Paid.Text + ".")
    End If
  End If
...
Next3:
  ...
  If CDbl(fpCurrRevOpt3Owed.Value) > CDbl(fpCurrRevOpt3Paid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      TXLog ("WARNING: User warned that overriding the automatic " + OptRev + " payment of " + QPTrim$(Using("$##,##0.00", TempRev3Paid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new interest amount of " + fpCurrRevOpt3Paid.Text + ".")
    End If
  End If
...
Private Sub GetMaxDisc()
  ...
  MaxDisc = 0
'  If TaxMasterRec.DiscRXDate > Date2Num(fptxtPayDate.Text) Then
'    DiscYN = True
'  End If
  ...
  For x = 1 To BillCnt
    ...
    Get THandle, BillTrans(x), TaxTRec
      Balance = 0
'    TaxTRec.DiscXDate = Date2Num("12/31/2005") 'for test purposes only
    If TaxTRec.BillType = "R" And TaxTRec.TaxYear = ThisTaxYear And TaxTRec.DiscXDate > 0 And DiscRXDate <= TaxTRec.DiscXDate Then
...
Private Function Check4ValidPaidEntries() As Boolean '8/12/05
  ...
  If CDbl(fpCurrPrincPaid.Value) > CDbl(fpCurrPrincOwed.Value) Then
    Call VATaxMsg(800, "The amount entered for principle paid cannot be more than the amount owed for principle. Press F9 to correct this problem.")
  ...
  If CDbl(fpCurrAdvColPaid.Value) > CDbl(fpCurrAdvColOwed.Value) Then
    Call VATaxMsg(800, "The amount entered for adv/collect paid cannot be more than the amount owed for adv/collect. Press F9 to correct this problem.")
  ...
  If CDbl(fpCurrLateListPaid.Value) > CDbl(fpCurrLateListOwed.Value) Then
    Call VATaxMsg(800, "The amount entered for late listing paid cannot be more than the amount owed for late listing. Press F9 to correct this problem.")
    fpCurrLateListPaid.SetFocus
    Check4ValidPaidEntries = False
    Exit Function
  End If
...
Private Sub PostEmTax()
  ...
  Dim NumOfPayTranRecs As Long, CmNum As Long
  ...
  If Not Exist("CMXRCPR" + Oper$ + ".DAT") Then Exit Sub
  TXLog ("OpenTempRealPay CMXRCPR" + Oper$)
...
'      TotStillOwed# = 0
'
'      TotStillOwed# = OldRound(TaxTranRec.Revenue.Principle1 + TaxTranRec.Revenue.Principle2 + TaxTranRec.Revenue.Principle3 + TaxTranRec.Revenue.Principle4 + TaxTranRec.Revenue.Principle5 + TaxTranRec.Revenue.Penalty)
'      TotStillOwed# = OldRound(TotStillOwed# + TaxTranRec.Revenue.Interest + TaxTranRec.Revenue.Collection + TaxTranRec.Revenue.LateList + TaxTranRec.Revenue.RevOpt1 + TaxTranRec.Revenue.RevOpt2 + TaxTranRec.Revenue.RevOpt3)
'      TotStillOwed# = OldRound(TotStillOwed# - (TaxTranRec.Revenue.Principle1Pd + TaxTranRec.Revenue.Principle2Pd + TaxTranRec.Revenue.Principle3Pd + TaxTranRec.Revenue.Principle4Pd + TaxTranRec.Revenue.Principle5Pd + TaxTranRec.Revenue.PenaltyPd))
'      TotStillOwed# = OldRound(TotStillOwed# - TaxTranRec.Revenue.InterestPd)
'      TotStillOwed# = OldRound(TotStillOwed# - TaxTranRec.Revenue.CollectionPd)
'      TotStillOwed# = OldRound(TotStillOwed# - TaxTranRec.Revenue.LateListPd)
'      TotStillOwed# = OldRound(TotStillOwed# - TaxTranRec.Revenue.RevOpt1Pd)
'      TotStillOwed# = OldRound(TotStillOwed# - TaxTranRec.Revenue.RevOpt2Pd)
'      TotStillOwed# = OldRound(TotStillOwed# - TaxTranRec.Revenue.RevOpt3Pd)
'
'      If TotStillOwed# < PayListRec.TotPaid Then
'        GoTo ErrorNOGo
'      End If
...
SkipThisRec:
 ...
 'Next
  ...
  Close
  WasSaved = True
  Call VASavemsg(900, "Real payment posting has completed successfully.")
...
dotheCMsave:
  ...
  If Len(QPTrim$(TaxPaymentRec.Desc)) = 0 Then
    CMTrRec(1).TransDesc = "Tax R Billing Payment"
  Else
  ...
  CMTrRec(1).TransPad = ""
  CMTrRec(1).TransRevAmt(1) = TaxPaymentRec.PaidOwed(1).AmtPaid    'Princeple
  CMTrRec(1).TransRevAmt(2) = TaxPaymentRec.PaidOwed(2).AmtPaid    'Interest
  CMTrRec(1).TransRevAmt(3) = TaxPaymentRec.PaidOwed(3).AmtPaid    'CollectAd
  CMTrRec(1).TransRevAmt(4) = TaxPaymentRec.PaidOwed(4).AmtPaid    'LateList
  CMTrRec(1).TransRevAmt(5) = TaxPaymentRec.PaidOwed(5).AmtPaid    'Penalty
  CMTrRec(1).TransRevAmt(6) = TaxPaymentRec.PaidOwed(6).AmtPaid    'Opt1
  CMTrRec(1).TransRevAmt(7) = TaxPaymentRec.PaidOwed(7).AmtPaid    'Opt2
  CMTrRec(1).TransRevAmt(8) = TaxPaymentRec.PaidOwed(8).AmtPaid    'Opt3
  CMTrRec(1).TransRevAmt(9) = TaxPaymentRec.DiscAmt
  CMTrRec(1).TransRevAmt(10) = TaxPaymentRec.PrePayAmt
  CMTrRec(1).TransRevAmt(11) = CDbl(TaxPaymentRec.NumPayRec) 'numofbills paid
