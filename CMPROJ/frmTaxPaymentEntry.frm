...
Begin VB.Form frmTaxPaymentEntry 
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdCash 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":0BC1
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdCheck 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":0D9C
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdCharge 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":0F78
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdLookup 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":1155
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdDist 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":1337
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":1512
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdBills 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":16EE
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":18CA
   End
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdInfo 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":1AA6
   End
   Begin fpBtnAtlLibCtl.fpBtn fpcmdDrawer 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxPaymentEntry.frx":1C81
   End
...
Option Explicit
  ...
  Dim TempBillList() As txPayListType
...
''      ThisCust = frmTaxPayEditList.fpListPay.ListIndex
...
Private Sub cmdInfo_Click()
  If fpLongAcctNum > 0 And GCustNum > 0 Then
     ...
     Dim AHandle As Integer
      One = 1
      AHandle = FreeFile
      Open "C:\CPWork\custinq.dat" For Output As AHandle
      Print #AHandle, One
      Close AHandle
      frmTaxCustInq.fptxtAcctNum = (fpLongAcctNum)
      frmTaxCustInq.Show
  End If
End Sub
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      ...
      'TXLog ("CitiTaxes.exe terminated via menu bar on frmTaxPaymentEntry.")
      Call CMTerminate
      End
    End If
  End If
...
Private Sub fpCurrPrePay_LostFocus()
  ...
  If CDbl(fpCurrPrePay.Value) > 0 And CDbl(fpCurrAmtOwed.Value) <> 0 Then
    If CDbl(fpCurrTotWDisc.Value) < CDbl(fpCurrAmtOwed.Value) Then
      ...
      Call TaxMsg(800, "Prepayment amounts can only be added if the total amounts owed are paid in full.")
...
Private Sub fpLongAcctNum_LostFocus()
  ...
  If Check4ValidCustNum(fpLongAcctNum.Value) = False Then
    ...
    GetNewCust = False
    Call Clearscreen
'    If fpcmbTenderType.Enabled = True Then
...
Public Sub Clearscreen()
  ...
  GCustNum = 0
  GPayNum = 0
  ReDim TempBillList(1 To 1) As txPayListType
  TempBillListCnt = 0
...
Private Sub LoadMe()
  ...
  DiscXDate = Date2Num(fptxtPayDate.Text)
  'DiscXDate = DiscXDate + 1
  
  OpenTaxSetUpFile TMHandle
  Get TMHandle, 1, TaxMasterRec
  Close TMHandle
  lblCurrTaxYr.Caption = "Current Tax Year: " + CStr(TaxMasterRec.TaxYear)
  TempPreYear = TaxMasterRec.TaxYear
...
LateListOK:
  ...
  ElseIf CDbl(fpCurrRevOpt1Owed.Value) > CDbl(fpCurrRevOpt1Paid.Value) Then
    ...
    ElseIf CDbl(fpCurrLateListPaid.Value) > CDbl(fpCurrLateListOwed.Value) Then
      ...
      fpCurrRevOpt1Paid.BackColor = &H8080FF
      fpCurrRevOpt1Owed.BackColor = &H8080FF
      frmTaxMsg.Label1.Caption = "The customer is attempting to overpay " + QPTrim$(fptxtRevOpt1.Text) + " while underpaying " + QPTrim$(fptxtRevOpt1.Text) + ". Please only allow overpayment for a revenue if all other revenue obligations have been met."
      frmTaxMsg.Label1.Top = 600
      frmTaxMsg.Show vbModal
      CheckOverPay = True
      Exit Function
...
Private Sub PrintReceipt()
  ...
  Oper$ = CStr(OperNum)
  PayRecpName2$ = UBPath$ + "TXVLD" + Oper$ + ".Rpt"
...
Private Sub ApplyDiscount()
  ...
  TXLog ("frmTaxPaymentEntry: Customer, " + fptxtName.Text + ", is eligible for a discount of " + QPTrim$(Using$("$#,##0.00", ThisDiscAmt)) + " and the user allowed the discount to apply.")
...
Private Sub Distribute(WhatsLeft As Double)
  ...
  If CDbl(fpCurrAmtOwed.Value) = 0 Then '8/12/05
    ...
    Else
      ...
      Call AddUpPaidCol
      Call LoadTempPayList
'    Call TaxMsg(900, "Distribution can only occur when the customer has a balance owed.")
      Exit Sub
    End If
  End If
...
'  PrePayAmt = CDbl(fpCurrPrePay.Value)
  ...
  ReDim Preserve TempBillList(1 To BillCnt) As txPayListType
  TempBillListCnt = 0
  For x = 1 To BillCnt
     ...
     If WhatsLeft >= PaySeq(x, 4) Then
       ...
       If TempBillList(x).DiscAmt > 0 Then
         If CDbl(fpCurrDisc.Value) = TempBillList(x).DiscAmt Then
           ...
           TempBillList(x).Principle1 = OldRound(TempBillList(x).Principle1 - TempBillList(x).DiscAmt)
         Else
           fpCurrPrincPaid = OldRound(CDbl(fpCurrPrincPaid.Value - TempBillList(x).DiscAmt)) 'CDbl(fpCurrDisc.Value))changed on 1/19/07 perBob
           TempBillList(x).Principle1 = OldRound(TempBillList(x).Principle1 - TempBillList(x).DiscAmt) 'CDbl(fpCurrDisc.Value))changed on 1/19/07 perBob
         End If
       End If
     ...
     Else
       ...
       If TempBillList(x).DiscAmt > 0 Then
         ...
         Else
           fpCurrPrincPaid = OldRound(CDbl(fpCurrPrincPaid.Value - TempBillList(x).DiscAmt)) 'CDbl(fpCurrDisc.Value))changed on 1/19/07 perBob
           TempBillList(x).Principle1 = OldRound(TempBillList(x).Principle1 - TempBillList(x).DiscAmt) 'CDbl(fpCurrDisc.Value))changed on 1/19/07 perBob
'           If TempBillList(x).DiscAmt = MaxDisc Then
'             TempBillList(x).DiscAmt = CDbl(fpCurrDisc.Value)
'           Else
'             ThisPct = OldRound(TempBillList(x).DiscAmt / CDbl(fpCurrDisc.Value))
'             ThisPct = ThisPct * 0.01
'             TempBillList(x).DiscAmt = OldRound(CDbl(fpCurrDisc.Value) * ThisPct)
'           End If
         End If
       End If
       TempBillList(x).TotPaid = OldRound(TempBillList(x).TotPaid + TempBillList(x).Principle1)
     ...
     If WhatsLeft >= PaySeq(x, 5) Then
       fpCurrRevOpt1Paid.Value = CDbl(fpCurrRevOpt1Paid.Value) + PaySeq(x, 5)
...
PlayedOut:
  ...
  'If MaxDisc > 0 And CDbl(fpCurrDisc.Value) = 0 Then
...
Private Sub OverRideDist()
  ...
  If CDbl(fpCurrPrincOwed.Value) > OldRound(CDbl(fpCurrPrincPaid.Value) + CDbl(fpCurrDisc.Value)) Then
    ...
    Else
      Unload frmTaxMsgWOpts
      fpCurrDisc.Value = 0
      TXLog ("WARNING: User warned that overriding the automatic principal payment of " + QPTrim$(Using("$##,##0.00", TempPrincPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new interest amount of " + fpCurrPrincPaid.Text + ".")
    End If
  End If
...
Private Sub GetMaxDisc()
  ...
  MaxDisc = 0
  'rem this out per BOB on 7-26-06
'  If TaxMasterRec.DiscXDate > Date2Num(fptxtPayDate.Text) Then
'    DiscYN = True
'  End If
...
'remarked out the following 4 lines on 2/8/07
...
Private Function Check4ValidPaidEntries() As Boolean '8/12/05
  ...
  If CDbl(fpCurrRevOpt1Paid.Value) > CDbl(fpCurrRevOpt1Owed.Value) Then
    Call TaxMsg(800, "The amount entered for " + fptxtRevOpt1 + " paid cannot be more than the amount owed for " + fptxtRevOpt1 + ". Press F9 to correct this problem.")
    fpCurrRevOpt1Paid.SetFocus
    Check4ValidPaidEntries = False
    Exit Function
  End If
  ...
  If CDbl(fpCurrRevOpt2Paid.Value) > CDbl(fpCurrRevOpt2Owed.Value) Then
    Call TaxMsg(800, "The amount entered for " + fptxtRevOpt2 + " paid cannot be more than the amount owed for " + fptxtRevOpt2 + ". Press F9 to correct this problem.")
    fpCurrRevOpt2Paid.SetFocus
    Check4ValidPaidEntries = False
    Exit Function
  End If
  ...
  If CDbl(fpCurrRevOpt3Paid.Value) > CDbl(fpCurrRevOpt3Owed.Value) Then
    Call TaxMsg(800, "The amount entered for " + fptxtRevOpt3 + " paid cannot be more than the amount owed for " + fptxtRevOpt3 + ". Press F9 to correct this problem.")
    fpCurrRevOpt3Paid.SetFocus
    Check4ValidPaidEntries = False
    Exit Function
  End If
...
Private Sub PostEmTax()
  ...
  Dim cnt&, TotalPaid#, tottotpaid#, TotStillOwed#
  Dim ThisListRec&
  Dim NextTransRec&
...
'Move the cmsave till after update tax files  GoSub dotheCMsave
...
'      TotalPaid# = OldRound#(PayListRec.DiscAmt + PayListRec.Principle1 + PayListRec.Interest1 + PayListRec.Collection + PayListRec.LateList)
      ...
      If TotalPaid# = 0 Then
        'TXLog (" PayRecNo " + Str$(ThisListRec&) + " for " + Oper$ + " sent to skip cause 0 totpaid")
        GoTo SkipThisRec
      End If
...
'      TaxTranRec.Revenue.Principle1Pd = OldRound#(TaxTranRec.Revenue.Principle1Pd + PayListRec.Principle1 + PayListRec.DiscAmt)
      ...
      TXLog ("TaxTrans update billrec " + Str$(PayListRec.BillRec) + " " + Str$(OperNum) + "," + Str$(TaxPaymentRec.CustAcct))

      NextTransRec& = (LOF(TaxTranHandle) \ Len(TaxTranRec)) + 1

      Put TaxTranHandle, NextTransRec&, PayTranRec
      TXLog ("TaxTrans newpayrec " + Str$(NextTransRec&) + " " + Str$(OperNum) + "," + Str$(TaxPaymentRec.CustAcct))
      TaxCustRec.LastTrans = NextTransRec&
      Put CHandle, TaxPaymentRec.CustAcct, TaxCustRec
      TXLog ("CustRec w/NextTransRec " + Str$(NextTransRec&) + " " + Str$(OperNum) + "," + Str$(TaxPaymentRec.CustAcct))
SkipThisRec:
...
PrePay:
  ...
  TotalPaid# = OldRound(TotalPaid# + PayListRec.OptRev1 + PayListRec.OptRev2 + PayListRec.OptRev3 + PayListRec.PrePayAmt)
  If TotalPaid# = 0 Then
    TXLog (" PayRecNo " + Str$(ThisListRec&) + " for " + Oper$ + " sent to skip cause 0 totpaid for prepay ")
    GoTo SkipThisRec
  End If
  'PayTranRec = the new record for tax transaction records
  PayTranRec.TransDate = TaxPaymentRec.payDate
  ...
  NextTransRec& = (LOF(TaxTranHandle) \ Len(TaxTranRec)) + 1
  Put TaxTranHandle, NextTransRec&, PayTranRec
  TXLog ("Taxtrans " + Str$(NextTransRec&) + " " + Str$(OperNum) + "," + Str$(TaxPaymentRec.CustAcct))
  TaxCustRec.LastTrans = NextTransRec&
  Put CHandle, TaxPaymentRec.CustAcct, TaxCustRec
  TXLog ("CustRec w/NextTransRec " + Str$(NextTransRec&) + " " + Str$(OperNum) + "," + Str$(TaxPaymentRec.CustAcct))
  Return
dotheCMsave:
  ...
  If Len(QPTrim$(TaxPaymentRec.Desc)) = 0 Then
    CMTrRec(1).TransDesc = "Tax Billing Payment"
  Else
  ...
  CMTrRec(1).TransPad = ""
  CMTrRec(1).TransRevAmt(1) = TaxPaymentRec.PaidOwed(1).AmtPaid    'Principle1Pd
  CMTrRec(1).TransRevAmt(2) = TaxPaymentRec.PaidOwed(2).AmtPaid    'InterestPd
  CMTrRec(1).TransRevAmt(3) = TaxPaymentRec.PaidOwed(3).AmtPaid    'CollectionPd
  CMTrRec(1).TransRevAmt(4) = TaxPaymentRec.PaidOwed(4).AmtPaid    'LateListPd
  CMTrRec(1).TransRevAmt(5) = TaxPaymentRec.PaidOwed(5).AmtPaid    'RevOpt1Pd
  CMTrRec(1).TransRevAmt(6) = TaxPaymentRec.PaidOwed(6).AmtPaid    'RevOpt2Pd
  CMTrRec(1).TransRevAmt(7) = TaxPaymentRec.PaidOwed(7).AmtPaid    'RevOpt3Pd
  CMTrRec(1).TransRevAmt(8) = TaxPaymentRec.DiscAmt
  CMTrRec(1).TransRevAmt(9) = TaxPaymentRec.PrePayAmt
  CMTrRec(1).TransRevAmt(10) = CDbl(TaxPaymentRec.NumPayRec) 'numofbills paid
...
Private Function GetNewTot()
  ...
  xx = OldRound(TaxOwed# + IntOwed# + ColOwed# + LLOwed# + RevOpt1# + RevOpt2# + RevOpt3#)
  GetNewTot = Val(xx)
  Close THandle
End Function
