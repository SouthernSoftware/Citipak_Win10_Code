...
Option Explicit
Public Sub NameEntryQSort(IdxBuff() As Struct, lLBound, lUBound)
  ...
  If lUBound <= lLBound Then Exit Sub 'GoTo Proc_Exit
    ...
    Do While (lngCurLow <= lngCurHigh)
      Do While IdxBuff(lngCurLow).who < Temp.who
        lngCurLow = lngCurLow + 1
        If lngCurLow = lUBound Then Exit Do
      Loop
      Do While Temp.who < IdxBuff(lngCurHigh).who
  ...
  'recurse if necessary
    If lLBound < lngCurHigh Then
      NameEntryQSort IdxBuff(), lLBound, lngCurHigh
    End If
  'recurse if necessary
    If lngCurLow < lUBound Then
      NameEntryQSort IdxBuff(), lngCurLow, lUBound
    End If
End Sub
