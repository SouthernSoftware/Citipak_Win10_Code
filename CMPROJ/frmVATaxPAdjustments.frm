...
Private Sub cmdBills_Click()
  ...
  If CLng(fpLongAcctNum.Value) <= 0 Then
    Call VATaxMsg(900, "Please supply a valid customer account number before accessing bills.")
    fpLongAcctNum.SetFocus
    Exit Sub
  End If
  
  If VACheck4ValidCustNum(CLng(fpLongAcctNum.Value)) = False Then
     Call VATaxMsg(800, "The customer account number entered cannot be found. Please enter a valid customer account number.")
     Close
     Exit Sub
  Else
  ...
  OpenVATaxCustFile TCHandle, NumOfTCRecs
  Get TCHandle, CLng(fpLongAcctNum.Value), TaxCustRec
  Close TCHandle
  If TaxCustRec.LastTrans = 0 Then
    Call VATaxMsg(900, "This customer has no transactions saved.")
    Close
    Exit Sub
  End If
  ...
  ElseIf BillRec < 0 Then
    ...
    Exit Sub
  Else
    Call VATaxMsg(900, "ERROR: There was a problem loading the bill data. Please try again.")
  End If
...
Private Sub cmdExit_Click()
  If VACheck4ValidCustNum(CLng(fpLongAcctNum.Value)) = True Then
    If fpCurrTotAdj.Value > 0 Then
      If VATaxMsgWOpts(900, "You have made changes that are not saved. Press F10 to save these changes. Otherwise, press ESC to continue without saving.", "F10 Save", "ESC Don't Save") = "abort" Then
...
Private Sub cmdHelp_Click()
  frmTaxMsgGeneral.Label1.Caption = "1-Billing Downward Adjustment will decrease a customer's balance.  This is used to reduce an incorrect billing amount."
  frmTaxMsgGeneral.Label2.Caption = "2-Billing Upward Adjustment will increase a customer's balance.  This is used to increase an incorrect billing amount."
  frmTaxMsgGeneral.Label3.Caption = "3-Payment Adjustment will increase a customer's balance.  This is used if payments are entered by mistake or with an incorrect amount."
  frmTaxMsgGeneral.Label4.Caption = "4-Release will decrease a customer's balance. This is used to reduce the amount billed because of an official governmental body directive."
  If CDbl(fpCurrPrepayBal.Value) > 0 Then
    frmTaxMsgGeneral.Label5.Caption = "5-Prepay Adjust Down adjusts prepay balance when nothing is owed."
  End If
...
Private Sub cmdPost_Click()
  ...
  If GCustNum = 0 Then
    Call VATaxMsg(900, "ERROR: No customer record number has been assigned.")
    Exit Sub
  End If
  
  If BillCnt = 0 And fpcboAdjType.Text <> "5-Prepay Adjust Down" Then
    Call VATaxMsg(900, "ERROR: This customer has no personal tax bills to adjust.")
  ...
  If fpcboAdjType.Text = "5-Prepay Adjust Down" Then
    If fpCurrPrepayAdjAmt = 0 Then
      Call VATaxMsg(800, "There amount for 'Prepay Adjustment Amount' is zero. Nothing to post.")
      Close
  ...
  If fpcboAdjType.Text <> "5-Prepay Adjust Down" Then
    If fpLngIntBill.Value = 0 Then
      Call VATaxMsg(900, "Please make a selection from the bill list.")
      fpLngIntBill.SetFocus
      Exit Sub
    End If
  End If
  ...
  If fpcboAdjType.Text <> "5-Prepay Adjust Down" Then
    If CDbl(fpCurrTotAdj.Value) = 0 Then
      If fpcboAdjType.Text = "3-Adjustment for Payment" And CDbl(fpCurrPrepayBal.Value) <> CDbl(fpCurrPrepayAdjBal.Value) Then
         GoTo OK2Go
      Else
        Call VATaxMsg(900, "The total adjustment is zero. No save required.")
        Exit Sub
      End If
    End If
  End If
...
OK2Go:
  If fpcboAdjType.Text = "1-Billing Downward Adjustment" Then
    If VATaxMsgWOpts(900, "Are you ready to post this Billing Downward Adjustment transaction? Press F10 to Post. Otherwise, press ESC to abort.", "F10 POST", "ESC Abort") = "abort" Then
      Unload frmVATaxMsgWOpts
      fpLongAcctNum.SetFocus
      Exit Sub
    Else
  ...
  ElseIf fpcboAdjType.Text = "2-Billing Upward Adjustment" Then
    If VATaxMsgWOpts(900, "Are you ready to post this Billing Upward Adjustment transaction? Press F10 to Post. Otherwise, press ESC to abort.", "F10 POST", "ESC Abort") = "abort" Then
      Unload frmVATaxMsgWOpts
      fpLongAcctNum.SetFocus
      Exit Sub
    Else
  ...
  ElseIf fpcboAdjType.Text = "3-Adjustment for Payment" Then
    If BillRec > 0 And Check4PaidAmts(BillRec) = False Then
      ...
      Exit Sub
    End If
    If VATaxMsgWOpts(900, "Are you ready to post this Adjustment for Payment transaction? Press F10 to Post. Otherwise, press ESC to abort.", "F10 POST", "ESC Abort") = "abort" Then
      Unload frmVATaxMsgWOpts
      fpLongAcctNum.SetFocus
      Exit Sub
    Else
  ...
  ElseIf fpcboAdjType.Text = "4-Release" Then
    If VATaxMsgWOpts(900, "Are you ready to post this Release transaction? Press F10 to Post. Otherwise, press ESC to abort.", "F10 POST", "ESC Abort") = "abort" Then
      Unload frmVATaxMsgWOpts
      fpLongAcctNum.SetFocus
      Exit Sub
    Else
  ...
  ElseIf fpcboAdjType.Text = "5-Prepay Adjust Down" Then
    If VATaxMsgWOpts(900, "Are you ready to post this Prepay Adjustment Down? Press F10 to Post. Otherwise, press ESC to abort.", "F10 POST", "ESC Abort") = "abort" Then
      Unload frmVATaxMsgWOpts
      fpLongAcctNum.SetFocus
      Exit Sub
    Else
    ...
    End If
  Else
    Call VATaxMsg(900, "Please supply an adjustment transaction type.")
  ...
  If fpcboAdjType.Text <> "2-Billing Upward Adjustment" And fpcboAdjType.Text <> "5-Prepay Adjust Down" Then
    If CDbl(fpCurrPersAdj.Value) > CDbl(fpCurrPersOwed.Value) Then
      Call VATaxMsg(700, "The 'PERSONAL' adjustment amount is greater than the 'PERSONAL' amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
      fpCurrPersAdj.SetFocus
      Exit Sub
    End If
   
    If CDbl(fpCurrMTAdj.Value) > CDbl(fpCurrMTOwed.Value) Then
      Call VATaxMsg(700, "The 'MACHINE TOOLS' adjustment amount is greater than the 'MACHINE TOOLS' amount owed. Please revise the adjustment aamount so that it is less than or equal to the amount owed.")
      fpCurrMTAdj.SetFocus
      Exit Sub
    End If
  
    If CDbl(fpCurrMCAdj.Value) > CDbl(fpCurrMCOwed.Value) Then
      Call VATaxMsg(700, "The 'MERCHANT CAP' adjustment amount is greater than the 'MERCHANT CAP' amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
      fpCurrMCAdj.SetFocus
      Exit Sub
    End If
  
    If CDbl(fpCurrFEAdj.Value) > CDbl(fpCurrFEOwed.Value) Then
      Call VATaxMsg(700, "The 'FARM EQUIPMENT' adjustment amount is greater than the 'FARM EQUIPMENT' amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
      fpCurrFEAdj.SetFocus
      Exit Sub
    End If
  
    If CDbl(fpCurrMHAdj.Value) > CDbl(fpCurrMHOwed.Value) Then
      Call VATaxMsg(700, "The 'MOBILE HOMES' adjustment amount is greater than the 'MOBILE HOMES' amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
    ...
    If fpCurrOpt1Adj.Enabled = True Then
      If CDbl(fpCurrOpt1Adj.Value) > CDbl(fpCurrOpt1Owed.Value) Then
        Call VATaxMsg(700, "The " + QPTrim$(fptxtOpt1.Text) + " adjustment amount is greater than the " + QPTrim$(fptxtOpt1.Text) + " amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
    ...
    If fpCurrOpt2Adj.Enabled = True Then
      If CDbl(fpCurrOpt2Adj.Value) > CDbl(fpCurrOpt2Owed.Value) Then
        Call VATaxMsg(700, "The " + QPTrim$(fptxtOpt2.Text) + " adjustment amount is greater than the " + QPTrim$(fptxtOpt2.Text) + " amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
    ...
    If fpCurrOpt3Adj.Enabled = True Then
      If CDbl(fpCurrOpt3Adj.Value) > CDbl(fpCurrOpt3Owed.Value) Then
        Call VATaxMsg(700, "The " + QPTrim$(fptxtOpt3.Text) + " adjustment amount is greater than the " + QPTrim$(fptxtOpt3.Text) + " amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
  ...
  End If
'----------------------------------------------------------------------
  Dim TaxAdjTrans As VATaxTransactionType
  ...
  OpenVATaxTransFile TAHandle, NumOfTARecs
  ...
  Select Case QPTrim$(fpcboAdjType.Text)
    ...
    Case Else
      Call VATaxMsg(900, "ERROR: Adjustment type could not be determined.")
      Close
      fpcboAdjType.SetFocus
      Exit Sub
  End Select
...
'----------------------------------------------------------------------
  ...
  SaveOK = True
  Call VASavemsg(900, "The adjustment transaction has been posted successfully.")
  Call TXLog("TaxAdj saved thru-CM," + QPTrim$(fpcboAdjType.Text) + ",Cust-" + Str(GCustNum) + ",for-" + Str(TotalAdj#) + ",on-" + fptxtDate.Text)
  Call CMLog("TaxAdj saved thru-CM," + QPTrim$(fpcboAdjType.Text) + ",Cust-" + Str(GCustNum) + ",for-" + Str(TotalAdj#) + ",on-" + fptxtDate.Text)
  If VATaxMsgWOpts(900, "Do you wish to print an adjustment report? Press F10 to print the report. Otherwise, press ESC to skip the report.", "F10 Print", "ESC Don't Print") = "abort" Then
    Unload frmVATaxMsgWOpts
  Else
    Unload frmVATaxMsgWOpts
    DoEvents
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      KillFile "C:\CPWork\txpadjust.dat"
      GCustNum = 0
      TXLog ("Cm.exe terminated via menu bar on frmVATaxPAdjustments.")
      Call CMTerminate
      End
    End If
  End If
...
Public Sub LoadMeEdit()
  ...
  OpNum = ""
'  If InPayBatchYN(fpLongAcctNum.Value, ThisBillType) = True Then
'    Call Clearscreen
'    fpLongAcctNum.SetFocus
'    Exit Sub
'  End If
  If Check4CustInPayBatch(fpLongAcctNum.Value, OpNum) = True Then
    Call VATaxMsg(700, "This customer is included in a payment file for operator #" + OpNum + " that has not been posted. Please either post this payment or delete this customer from the payment file.")
    Call Clearscreen
    fpLongAcctNum.SetFocus
    Exit Sub
  End If
  ...
  GCustNum = CLng(fpLongAcctNum.Value)
  If GCustNum > 0 Then
    ThisGBal = VAGetCustPersBalance(GCustNum, -1)
  End If
...
Private Sub fpcboAdjType_LostFocus()
  ...
  If fpLngIntBill = 0 Then
    ...
    Call VATaxMsg(900, "Please select a bill first.")
...
Private Sub fpCurrMCAdj_LostFocus()
  If fpcboAdjType.Text = "3-Adjustment for Payment" Then
    If CDbl(fpCurrMCAdj.Value) > CDbl(fpCurrMCOwed.Value) Then
      Call VATaxMsg(900, "Payment adjustment cannot exceed the amount paid.")
...
Private Sub fpCurrPrepayAdjAmt_LostFocus()
  ...
  If fpCurrPrepayAdjBal < 0 Then
    Call VATaxMsg(800, "The prepay adjustment cannot allow the prepay balance to be less than zero.")
...
Private Sub fpLongAcctNum_LostFocus()
  ...
  If VACheck4ValidCustNum(CLng(fpLongAcctNum.Value)) = True Then
    ...
    Else 'adjustment is zero
      ...
      Call LoadMeEdit
    End If
    PrePay = VAGetCustPersBalance(CLng(fpLongAcctNum.Value), -1)
    If PrePay < 0 Then
    ...
    End If
  Else
    Call VATaxMsg(900, "An invalid customer account number, " + fpLongAcctNum.Text + ", has been entered. Resetting account number to zero. Please try again.")
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPAdjustments", "VACheck4ValidCustNum", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub LoadMeBill()
  ...
  Dim TCHandle As Integer
  Dim NumOfTCRecs As Long
  Dim TaxRec As VATaxTransactionType
  ...
  PersPaid = TaxRec.Revenue.Principle1Pd + TaxRec.PPTRADisc '1/16/07 added PPTRADisc
  ...
  If BillNum = CLng(fpLngIntBill.Value) Then
    ...
    TPers# = OldRound(TPers# - (TaxRec.Revenue.Principle1Pd + TaxRec.PPTRADisc)) '1/16/07 added PPTRADisc
    PersBal = TPers#
    ...
    fpCurrTotAdj.Text = 0
  Else
    Call VATaxMsg(800, "The bill number entered could not be found in this customer's transaction records. Please enter another bill number or press F8 to bring up a complete list of all the bills for this customer.")
  End If
...
Private Sub fptxtDate_LostFocus()
  ...
  If Abs(ThatDate - ThisDate) > 60 Then
    If VATaxMsgWOpts(800, "The date entered is more than 60 days from today's date. Press F10 if you want to continue with this date. Otherwise, press ESC to review.", "F10 Continue", "ESC Review") = "abort" Then
      Unload frmVATaxMsgWOpts
      fptxtDate.SetFocus
      Exit Sub
    Else
      Unload frmVATaxMsgWOpts
      TXLog ("ERROR: User warned that the date entered is more than 60 days from today's date and elected to continue with the date entered anyway.")
    End If
  End If
End Sub
...
Private Sub PrintText()
  ...
  If InStr(fpcboAdjType.Text, "3") Then
    Print #RptHandle, Tab(5); "Balance Excluding This Bill: "; Tab(39); Using$("$###,##0.00", OldRound(VAGetCustPersBalance(GCustNum, -1) - NewBalThisBill))
    ...
    Print #RptHandle, Tab(5); "Account Balance: "; Tab(39); Using("$###,##0.00", VAGetCustPersBalance(GCustNum, -1))
    Print #RptHandle,
    Print #RptHandle,
...
Private Function Check4PaidAmts(BillRec As Double) As Boolean
  ...
  If CDbl(fpCurrPersAdj.Value) > PersPaid Then '11/21/2005 error here
    Call VATaxMsg(800, "The total amount paid for PERSONAL is " + QPTrim$(Using$("$###,##0.00", PersPaid)) + ". Adjustments for payments cannot exceed the total amount paid.")
    fpCurrPersAdj.SetFocus
    Close
  ...
  If CDbl(fpCurrMTAdj.Value) > MTPaid Then
    Call VATaxMsg(800, "The total amount already paid for MACHINE TOOLS is " + QPTrim$(Using$("$###,##0.00", MTPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrMTAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If CDbl(fpCurrMCAdj.Value) > MCPaid Then
    Call VATaxMsg(800, "The total amount already paid for MERCHANT CAP is " + QPTrim$(Using$("$###,##0.00", MCPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrMCAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If CDbl(fpCurrFEAdj.Value) > FEPaid Then
    Call VATaxMsg(800, "The total amount already paid for FARM EQUIPMENT is " + QPTrim$(Using$("$###,##0.00", FEPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrFEAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If CDbl(fpCurrMHAdj.Value) > MHPaid Then
    Call VATaxMsg(800, "The total amount already paid for MOBILE HOMES is " + QPTrim$(Using$("$###,##0.00", MHPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrMHAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If CDbl(fpCurrIntAdj.Value) > MHPaid Then
    Call VATaxMsg(800, "The total amount already paid for INTEREST is " + QPTrim$(Using$("$###,##0.00", PIntPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrIntAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If CDbl(fpCurrPenAdj.Value) > PENPAID Then
    Call VATaxMsg(800, "The total amount already paid for PENALTY is " + QPTrim$(Using$("$###,##0.00", PENPAID)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrPenAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If fpCurrOpt1Adj.Enabled = True Then
    ...
    If CDbl(fpCurrOpt1Adj.Value) > OptRev1Paid Then
      Call VATaxMsg(800, "The total amount already paid for " + QPTrim$(fptxtOpt1.Text) + " is " + QPTrim$(Using$("$###,##0.00", OptRev1Paid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
      fpCurrOpt1Adj.SetFocus
      Close
      Check4PaidAmts = False
      Exit Function
  ...
  If fpCurrOpt2Adj.Enabled = True Then
    ...
    If CDbl(fpCurrOpt2Adj.Value) > OptRev2Paid Then
      Call VATaxMsg(800, "The total amount already paid for " + QPTrim$(fptxtOpt2.Text) + " is " + QPTrim$(Using$("$###,##0.00", OptRev2Paid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
      fpCurrOpt2Adj.SetFocus
      Close
      Check4PaidAmts = False
      Exit Function
  ...
  If fpCurrOpt3Adj.Enabled = True Then
    ...
    If CDbl(fpCurrOpt3Adj.Value) > OptRev3Paid Then
      Call VATaxMsg(800, "The total amount already paid for " + QPTrim$(fptxtOpt3.Text) + " is " + QPTrim$(Using$("$###,##0.00", OptRev3Paid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
      fpCurrOpt3Adj.SetFocus
      Close
      Check4PaidAmts = False
      Exit Function
...
Private Sub UpdatePrepay()
  ...
  If fpcboAdjType.Text = "5-Prepay Adjust Down" Then
    fpCurrPrepayAdjBal = OldRound(CDbl(fpCurrPrepayBal.Value) + CDbl(fpCurrPrepayAdjAmt.Value))
    If fpCurrPrepayAdjBal < 0 Then
      Call VATaxMsg(800, "The prepay adjustment cannot cause the prepay balance to be less than zero.")
      fpCurrPrepayAdjBal = fpCurrPrepayBal
...
PrePayOnly:
  ...
  Else
    If InStr(fpcboAdjType.Text, "2") Then
      ...
      '                                        30
      Print #RptHandle, OldRound(VAGetCustPersBalance(GCustNum, -1) - NewBalThisBill); dlm;
      '                       31                      32                                              33
...
Private Sub ApplyDisc(Handle As Integer, TaxRec As VATaxTransactionType)
  ...
  TaxRec.Revenue.Principle1Pd = OldRound(TaxRec.Revenue.Principle1Pd + Disc1)
