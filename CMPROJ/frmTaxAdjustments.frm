...
Begin VB.Form frmTaxAdjustments 
   BackColor       =   &H008F8265&
   ...
   ClientTop       =   435
   ClientWidth     =   11640
   Icon            =   "frmTaxAdjustments.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   8730
   ...
   Begin LpLib.fpCombo fpcboAdjType 
      ...
      EditAlignH      =   1
      EditAlignV      =   0
      ColDesigner     =   "frmTaxAdjustments.frx":08CA
   End
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdPost 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxAdjustments.frx":0BC1
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxAdjustments.frx":0D9D
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdHistory 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxAdjustments.frx":0F79
   End
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdLookup 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxAdjustments.frx":1157
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdBills 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxAdjustments.frx":1339
   End
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdMessage 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxAdjustments.frx":1515
   End
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdHelp 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxAdjustments.frx":16F3
   End
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdReset 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmTaxAdjustments.frx":18D5
   End
   Begin VB.Line Line4 
   ...
   Begin VB.Label Label8 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "Tax Billing Adjustments"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   13.5
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmTaxAdjustments"
...
Private Sub cmdBills_Click()
  ...
  BillRec = 0
  frmTaxAdjustBillList.Show vbModal
  If BillRec > 0 Then
    TempBillRec = BillRec
    Call LoadMeBill
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "cmdBills_Click", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub cmdHistory_Click()
  If GCustNum = 0 Then
    Exit Sub
  End If
  frmTaxCustInfoTHist.Show
  DoEvents
End Sub

Private Sub cmdLookup_Click()
  frmTaxCustLookup.Show vbModal
  DoEvents
End Sub
...
Release:
  ...
  Do While NextRec > 0 '1/25/07
    ...
    If TaxTrans.BelongTo = SaveBillRec And TaxTrans.TranType = 2 And TaxTrans.DiscAmt > 0 Then
      TaxTrans.DiscAmt = 0
...
AdjustPayDown:
  ...
  If CreditAmt <= 0 Then
    ...
    NewBalThisBill = OldRound(NewBalThisBill - (TaxTrans.Revenue.RevOpt2Pd + TaxTrans.Revenue.RevOpt3Pd))
...
PrepayAdjustDown:
  ...
  TaxAdjTrans.TaxYear = TaxMasterRec.TaxYear
  TaxAdjTrans.TransDate = Date2Num(fptxtDate.Text)
  TaxAdjTrans.Amount = fpCurrPrepayAdjAmt
  TaxAdjTrans.BelongTo = 0
  TaxAdjTrans.BillType = ""
  TaxAdjTrans.CustomerRec = GCustNum
  TaxAdjTrans.CustPin = TaxCustRec.PIN
  If Len(QPTrim$(fptxtNote.Text)) = 0 Then
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "cmdPost_Click", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      KillFile "txadjust.dat"
      GCustNum = 0
      TXLog ("CM.exe terminated via menu bar on frmTaxAdjustments.")
      CMLog ("CM.exe terminated via menu bar on frmTaxAdjustments.")
      Call CMTerminate
      End
    End If
  End If
...
Private Sub LoadMe()
  ...
  One = 1
  AHandle = FreeFile
  Open "txadjust.dat" For Output As AHandle
  Print #AHandle, One
  Close AHandle
  fptxtDate.Text = Date
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "LoadMe", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Public Sub LoadMeEdit()
  ...
  If InPayBatchYN(fpLongAcctNum.Value) = True Then
    Call Clearscreen
    fpLongAcctNum.SetFocus
    Exit Sub
  End If
  ...
  GCustNum = CLng(fpLongAcctNum.Value)
  If GCustNum > 0 Then
    ThisGBal = GetCustBalance(GCustNum, -1)
  End If
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "LoadMeEdit", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub fpcboAdjType_LostFocus()
  ...
  If fpLngIntBill = 0 Then
    If frmTaxAdjustBillList.Visible Then Exit Sub
...
Private Sub fpLongAcctNum_LostFocus()
  ...
  If Check4ValidCustNum(CLng(fpLongAcctNum.Value)) = True Then
    ...
    Else 'adjustment is zero
      ...
      Call LoadMeEdit
    End If
    PrePay = GetCustBalance(CLng(fpLongAcctNum.Value), -1)
    If PrePay < 0 Then
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "fpLongAcctNum_LostFocus", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "Check4ValidCustNum", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub LoadMeBill()
  ...
  If BillNum = CLng(fpLngIntBill.Value) Then
    ...
    PrincBal = TPrinc#
    TInterest# = OldRound#((TaxRec.Revenue.Penalty + TaxRec.Revenue.Interest) - (TaxRec.Revenue.InterestPd + TaxRec.Revenue.PenaltyPd))
    IntBal = TInterest#
    ...
    fpCurrRevOpt3Adj.Text = 0
    fpCurrTotOwed.Text = OldRound(TPrinc# + TInterest# + TCollect# + TLateList# + TOpt1# + TOpt2# + TOpt3#)
    fpCurrTotAdj.Text = 0
    fpcboAdjType.SetFocus
  Else
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "LoadMeBill", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub FigureAdjCol(Rev As Integer)
  ...
  TotAdj = OldRound(TotAdj + CDbl(fpCurrLateListAdj.Value) + CDbl(fpCurrRevOpt1Adj.Value))
...
Private Sub fptxtDate_LostFocus()
  ...
  If Abs(ThatDate - ThisDate) > 60 Then
    ...
    Else
      Unload frmTaxMsgWOpts
      TXLog ("ERROR: CM User warned that the date entered is more than 60 days from today's date and elected to continue with the date entered anyway.")
    End If
  End If
End Sub
...
Private Sub PrintText()
  ...
  If InStr(fpcboAdjType.Text, "3") Then
    Print #RptHandle, Tab(5); "Balance Excluding This Bill: "; Tab(39); Using$("$###,##0.00", OldRound(GetCustBalance(GCustNum, -1) - NewBalThisBill))
    ...
    Print #RptHandle, Tab(5); "Account Balance: "; Tab(39); Using("$###,##0.00", GetCustBalance(GCustNum, -1))
    Print #RptHandle,
    Print #RptHandle,
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "PrintText", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "Check4PaidAmts", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
PrePayOnly:
  ...
  If InStr(fpcboAdjType.Text, "1") Or InStr(fpcboAdjType.Text, "4") Then
    ...
    '                       31                                            32                           33
    Print #RptHandle, fpLngIntBill.Text; dlm; OldRound(CDbl(fpCurrTotOwed) - CDbl(fpCurrTotAdj)); dlm; 0
  Else
    If InStr(fpcboAdjType.Text, "2") Then
      '                              12                             13
      Print #RptHandle, CDbl(fpCurrPrincAdj.Value); dlm; CDbl(fpCurrIntAdj.Value); dlm;
      ...
      '                                        30
      Print #RptHandle, OldRound(GetCustBalance(GCustNum, -1) - NewBalThisBill); dlm;
      '                       31                      32                                              33
      Print #RptHandle, fpLngIntBill.Text; dlm; NewBalThisBill; dlm; Abs(OldRound(CDbl(fpCurrPrepayBal.Value) - CDbl(fpCurrPrepayAdjBal.Value)))
    ...
    ElseIf fpcboAdjType.Text = "5-Prepay Adjust Down" Then
      ...
      Print #RptHandle, 0; dlm;
      '                   30           31
      Print #RptHandle, ThisAmt; dlm; "NA"
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxAdjustments", "PrintGraphics", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
