...
Private Sub cmdPost_Click()
  ...
  If BillCnt = 0 Then
    If fpcboAdjType.Text <> "5-Prepay Adjust Down" Then
      Call VATaxMsg(900, "ERROR: This customer has no real tax bills to adjust.")
...
OK2Go:
  ...
  If fpcboAdjType.Text <> "2-Billing Upward Adjustment" And fpcboAdjType.Text <> "5-Prepay Adjust Down" Then
    If CDbl(fpCurrPrincAdj.Value) > CDbl(fpCurrPrincOwed.Value) Then
      Call VATaxMsg(700, "The 'PRINCIPLE' adjustment amount is greater than the 'PRINCIPLE' amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
      fpCurrPrincAdj.SetFocus
      Exit Sub
    End If
   
    If CDbl(fpCurrIntAdj.Value) > CDbl(fpCurrIntOwed.Value) Then
      Call VATaxMsg(700, "The 'INTEREST' adjustment amount is greater than the 'INTEREST' amount owed. Please revise the adjustment aamount so that it is less than or equal to the amount owed.")
      fpCurrIntAdj.SetFocus
      Exit Sub
    End If
  
    If CDbl(fpCurrAdvColAdj.Value) > CDbl(fpCurrAdvColOwed.Value) Then
      Call VATaxMsg(700, "The 'ADV/COLLECT' adjustment amount is greater than the 'ADV/COLLECT' amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
      fpCurrAdvColAdj.SetFocus
      Exit Sub
    End If
  
    If CDbl(fpCurrLateListAdj.Value) > CDbl(fpCurrLateListOwed.Value) Then
      Call VATaxMsg(700, "The 'LATE LISTING' adjustment amount is greater than the 'LATE LISTING' amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
      fpCurrLateListAdj.SetFocus
      Exit Sub
    End If
  
    If CDbl(fpCurrPenAdj.Value) > CDbl(fpCurrPenOwed.Value) Then
      Call VATaxMsg(700, "The 'PENALTY' adjustment amount is greater than the 'PENALTY' amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
    ...
    If fpCurrRevOpt1Adj.Enabled = True Then
      If CDbl(fpCurrRevOpt1Adj.Value) > CDbl(fpCurrRevOpt1Owed.Value) Then
        Call VATaxMsg(700, "The " + QPTrim$(fptxtRevOpt1.Text) + " adjustment amount is greater than the " + QPTrim$(fptxtRevOpt1.Text) + " amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
    ...
    If fpCurrRevOpt2Adj.Enabled = True Then
      If CDbl(fpCurrRevOpt2Adj.Value) > CDbl(fpCurrRevOpt2Owed.Value) Then
        Call VATaxMsg(700, "The " + QPTrim$(fptxtRevOpt2.Text) + " adjustment amount is greater than the " + QPTrim$(fptxtRevOpt2.Text) + " amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
    ...
    If fpCurrRevOpt3Adj.Enabled = True Then
      If CDbl(fpCurrRevOpt3Adj.Value) > CDbl(fpCurrRevOpt3Owed.Value) Then
        Call VATaxMsg(700, "The " + QPTrim$(fptxtRevOpt3.Text) + " adjustment amount is greater than the " + QPTrim$(fptxtRevOpt3.Text) + " amount owed. Please revise the adjustment amount so that it is less than or equal to the amount owed.")
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      KillFile "C:\CPWork\txradjust.dat"
      GCustNum = 0
      TXLog ("cm.exe terminated via menu bar on frmVATaxAdjustments.")
      Call CMTerminate
      End
    End If
  End If
...
'  End If
  ...
  GCustNum = CLng(fpLongAcctNum.Value)
  If GCustNum > 0 Then
    ThisGBal = VAGetCustRealBalance(GCustNum, -1)
  End If
...
Private Sub fpLongAcctNum_LostFocus()
  ...
  If VACheck4ValidCustNum(CLng(fpLongAcctNum.Value)) = True Then
    ...
    Else 'adjustment is zero
      ...
      Call LoadMeEdit
    End If
    PrePay = VAGetCustRealBalance(CLng(fpLongAcctNum.Value), -1)
    If PrePay < 0 Then
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxAdjustments", "VACheck4ValidCustNum", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub LoadMeBill()
  ...
  TotPaid = OldRound(PrincPaid + IntPaid + AdvColPaid + LateListPaid + Rev1Paid + Rev2Paid + Rev3Paid)
...
Private Sub PrintText()
  ...
  If InStr(fpcboAdjType.Text, "3") Then
    Print #RptHandle, Tab(5); "Balance Excluding This Bill: "; Tab(39); Using$("$###,##0.00", OldRound(VAGetCustRealBalance(GCustNum, -1) - NewBalThisBill))
    ...
    Print #RptHandle, Tab(5); "Account Balance: "; Tab(39); Using("$###,##0.00", VAGetCustRealBalance(GCustNum, -1))
    Print #RptHandle,
    Print #RptHandle,
...
Private Function Check4PaidAmts(BillRec As Double) As Boolean
  ...
  If CDbl(fpCurrPrincAdj.Value) > PrincPaid Then
    Call VATaxMsg(800, "The total amount paid for PRINCIPLE is " + QPTrim$(Using$("$###,##0.00", PrincPaid)) + ". Adjustments for payments cannot exceed the total amount paid.")
    fpCurrPrincAdj.SetFocus
    Close
  ...
  If CDbl(fpCurrIntAdj.Value) > IntPaid Then
    Call VATaxMsg(800, "The total amount already paid for INTEREST is " + QPTrim$(Using$("$###,##0.00", IntPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrIntAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If CDbl(fpCurrIntAdj.Value) > IntPaid Then
    Call VATaxMsg(800, "The total amount already paid for ADV/COLLECT is " + QPTrim$(Using$("$###,##0.00", AdvColPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrAdvColAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If CDbl(fpCurrLateListAdj.Value) > LateListPaid Then
    Call VATaxMsg(800, "The total amount already paid for LATE LISTING is " + QPTrim$(Using$("$###,##0.00", LateListPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrLateListAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If CDbl(fpCurrPenAdj.Value) > PenaltyPaid Then
    Call VATaxMsg(800, "The total amount already paid for PENALTY is " + QPTrim$(Using$("$###,##0.00", PenaltyPaid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
    fpCurrPenAdj.SetFocus
    Close
    Check4PaidAmts = False
    Exit Function
  ...
  If fpCurrRevOpt1Adj.Enabled = True Then
    ...
    If CDbl(fpCurrRevOpt1Adj.Value) > OptRev1Paid Then
      Call VATaxMsg(800, "The total amount already paid for " + QPTrim$(fptxtRevOpt1.Text) + " is " + QPTrim$(Using$("$###,##0.00", OptRev1Paid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
      fpCurrRevOpt1Adj.SetFocus
      Close
      Check4PaidAmts = False
      Exit Function
  ...
  If fpCurrRevOpt2Adj.Enabled = True Then
    ...
    If CDbl(fpCurrRevOpt2Adj.Value) > OptRev2Paid Then
      Call VATaxMsg(800, "The total amount already paid for " + QPTrim$(fptxtRevOpt2.Text) + " is " + QPTrim$(Using$("$###,##0.00", OptRev2Paid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
      fpCurrRevOpt2Adj.SetFocus
      Close
      Check4PaidAmts = False
      Exit Function
  ...
  If fpCurrRevOpt3Adj.Enabled = True Then
    ...
    If CDbl(fpCurrRevOpt3Adj.Value) > OptRev3Paid Then
      Call VATaxMsg(800, "The total amount already paid for " + QPTrim$(fptxtRevOpt3.Text) + " is " + QPTrim$(Using$("$###,##0.00", OptRev3Paid)) + ". Adjustments for payments cannot exceed the total amount already paid.")
      fpCurrRevOpt3Adj.SetFocus
      Close
      Check4PaidAmts = False
      Exit Function
...
'          InPayBatchYN = True
'          Call VATaxMsg(700, "This customer, " + QPTrim$(TaxPaymentRec.CustName) + ", is currently included in an unposted payment file for operator " + OPNames(x) + ". Please post this payment file before continuing with this adjustment.")
'          Close PHandle
'          Exit Function
'        End If
'      Next y
...
PrePayOnly:
  Close TTHandle
  ThisBal = VAGetCustRealBalance(GCustNum, -1)
  ...
  Else
    If InStr(fpcboAdjType.Text, "2") Then
      ...
      '                                        30
      Print #RptHandle, OldRound(VAGetCustRealBalance(GCustNum, -1) - NewBalThisBill); dlm;
      '                       31                      32                                              33
