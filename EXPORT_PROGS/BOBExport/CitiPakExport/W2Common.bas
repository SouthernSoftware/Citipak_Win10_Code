...
'  One = 1
'  AHandle = FreeFile
'  Open "W2.dat" For Output As AHandle
...
'    If QPTrim$(Emp2Rec(1).EmpNo) = 157 Then Stop
...
'  Print #AHandle, CStr(Emp2Rec(1).EmpNo) + "~" + Using$("$###,##0.00", W2InfoRec(1).FEDWAGE)
...
'  W2SetUpRec(1).Deds(0).CHKDED = QPTrim$(frm.fpcomboRetire.Text)
  ...
  For ECnt = 1 To ENumOfRec
    Get EHandle, ECnt, Emp2Rec(1) '12/03
'    If Emp2Rec(1).EmpNo = 157 Then Stop
    GoSub GetEmpTranRecNums
    Select Case W2Type%
    Case 1
      If Emp2Rec(1).EMPSOCX <> "Y" Then
