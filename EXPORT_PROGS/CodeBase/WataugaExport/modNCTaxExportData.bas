...
'    Print #RptHandle, CStr(x);
    ...
    Print #RptHandle, CStr(TaxCust.Acct + 1000);
...
Public Sub ProcessNCTaxReal()
  ...
  For x = 1 To NumOfCustRecs
    ...
    Do While NextRec > 0
      ...
      tMoCo = QPTrim$(RealRec.MORTCODE)
      If Len(tMoCo) > 0 Then
        tMoCo = "w" + tMoCo
      End If
      Print #RptHandle, B & QPTrim$(tMoCo);
      ...
      Print #RptHandle, B & Using$(N, CStr(zzz + 1000));
...
Public Sub ProcessNCTaxPers2()
  ...
  For x = 1 To NumOfCustRecs
    ...
    Do While NextRec > 0
      Get PHandle, NextRec, PersRec
      zzz = CLng(PersRec.CustPin)
      If zzz <> x Then Stop
      PersRec.CustPin = zzz + 1000
...
'    Print #RptHandle, CStr(x);
    Print #RptHandle, "w" + QPTrim$(MortRec.MORTCODE);
...
Public Sub ProcessNCMessages()
  ...
  For x = 1 To NumOfMRecs
    ...
    For Y = 1 To 15
      ...
      End If
    Next Y
    Print #RptHandle, B & CStr(MessRec.TaxRec + 1000);
...
Public Sub ProcessNCTransHist()
  ...
  For x = 1 To NumOfTRecs
    ...
    Print #RptHandle, B & CStr(TransRec.CustomerRec + 1000);
...
SendThisCreditBal:
    ...
    Print #RptHandle, B & CStr(TaxCust.Acct + 1000);
    Print #RptHandle, B & "";
    Print #RptHandle, B & "";
...
'Checks for a personal property by the PIN.
  ...
  If Len(ChkPin) = 1 And ChkPin <> "0" Then Stop
