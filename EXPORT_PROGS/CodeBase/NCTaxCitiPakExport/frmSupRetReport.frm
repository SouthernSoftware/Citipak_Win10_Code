...
Begin VB.Form frmSupRetReport 
   ...
   Begin ImpproLib.vaImprint vaImprint2 
      ...
      Begin fpBtnAtlLibCtl.fpBtn cmdProcess 
         ...
         DropShadowColor =   0
         Redraw          =   -1  'True
         ButtonDesigner  =   "frmSupRetReport.frx":14C2
      End
      Begin fpBtnAtlLibCtl.fpBtn cmdEscape 
         ...
         DropShadowColor =   0
         Redraw          =   -1  'True
         ButtonDesigner  =   "frmSupRetReport.frx":16D9
      End
...
'    If QPTrim(Emp2Rec.EMPLNAME) = UCase("WOODRUFF") Then Stop
...
No401KMatch:
  ...
  Print #1, Using("####0.00", TotLawMatAmt#); dlm; Using("####0.00", TotGenMatAmt#); dlm; Str$(EPrinted); dlm; Using("#,###,##0.00", TotalGross#); dlm; "Y"; dlm; LowDateString; dlm; EndDateString
...
PrintGrandTots:
  ...
  Print #1, Using("####0.00", TVLawAmt#); dlm; Using("####0.00", TVGenAmt#); dlm; Using("####0.00", TLLawAmt#); dlm; Using("####0.00", TLGenAmt#); dlm;
  '                      23                                 24                               25                             26                           27              28                29
  Print #1, Using("####0.00", TotLawMatAmt#); dlm; Using("####0.00", TotGenMatAmt#); dlm; Str$(TEPrinted); dlm; Using("#,###,##0.00", TotalGross#); dlm; "N"; dlm; LowDateString; dlm; EndDateString
Return
...
PrintThisOne:
'  If QPTrim$(Emp2Rec(1).EMPLNAME) = "CREEK" And QPTrim$(Emp2Rec(1).EMPFNAME) = "SHANNON D." Then Stop
...
PrintThisOne:
  If QPTrim$(Emp2Rec.YN401K) <> "Y" Then 'added 8/28/03
    TMatchAmt# = 0 'filter out non participators only when
    'matching amounts are calculated
    If VCalcAmt# > 0 Or LCalcAmt# > 0 Or TMatchAmt# > 0 Then 'added 9/26/03
      GoTo No401KMatch
    Else
      GoTo SkipEMBubba
    End If
...
No401KMatch:
    ...
    Print #1, "  "; K401Rec(1).MAmt; "    "; K401Rec(1).HDate; ""; EmpRType
    PntCnt = PntCnt + 1
...
PrintGrandTots:
  ...
  Print #RHandle,
  Print #RHandle, "  Total Vol Law "; Using("####0.00", TVLawAmt#)
  Print #RHandle, "  Total Vol Gen "; Using("####0.00", TVGenAmt#)
  Print #RHandle,
  Print #RHandle, " Total Loan Law "; Using("####0.00", TLLawAmt#)
  Print #RHandle, " Total Loan Gen "; Using("####0.00", TLGenAmt#)
  Print #RHandle,
  Print #RHandle, "Total Match Law "; Using("####0.00", TotLawMatAmt#)
  Print #RHandle, "Total Match Gen "; Using("####0.00", TotGenMatAmt#)
  Print #RHandle,
  Print #RHandle, "Total Gross Pay"; " "; Using("#,###,##0.00", TotalGross#)
  Print #RHandle, FF$
