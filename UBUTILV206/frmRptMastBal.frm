...
Begin VB.Form frmRptMastBal 
   ...
   Begin LpLib.fpCombo fpcboCustStatus 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptMastBal.frx":0C61
   End
   Begin LpLib.fpCombo fpcboBalType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptMastBal.frx":0FF8
   End
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptMastBal.frx":138F
   End
   Begin LpLib.fpCombo fpcboPrintOrder 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptMastBal.frx":1731
   End
   Begin LpLib.fpCombo fpcboDetRev 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptMastBal.frx":1AC8
   End
...
Private Sub Form_Load()
  ...
  fpcboRptType.InsertRow = "Mismatched Credits"
  fpcboRptType.InsertRow = "Rev Tot <> Balance"
...
'  End If
End Sub
Private Sub MasterBalanceListing1()
...
'right here
    ...
    For RevChk = 1 To 15        'MaxRevenue '15
      If Round#(UBCustRec(1).CurrRevAmts(RevChk)) <> 0 Then
        IndPRevAmt = Round#(IndPRevAmt + UBCustRec(1).CurrRevAmts(RevChk))
      End If
      
    Next
    If RealBalance# <> IndPRevAmt Then
...
bskipem:
  ...
  ARptMastBalList.Title = "MBR REV TOTAL <> Acct TOTAL"
...
Private Sub MasterBalanceListing2()
  ...
  Dim ToPrintH2 As String, UBRpt2 As Integer, ToPrintS As String
  Dim ToPrintD2 As String, DetFlag As Integer, ReportFile As String
  Dim Report2 As String, Rev15 As Double
  Dim IndPRevAmt As Double, IndCRevAmt As Double
...
'right here
    ...
    For RevChk = 1 To 15        'MaxRevenue '15
     ...
     If Round#(UBCustRec(1).CurrRevAmts(RevChk)) > 0 Then
        IndPRevAmt = Round#(IndPRevAmt + UBCustRec(1).CurrRevAmts(RevChk))
        If RevChk = 15 Then Rev15 = UBCustRec(1).CurrRevAmts(RevChk)
      End If
      
      If Round#(UBCustRec(1).CurrRevAmts(RevChk)) < 0 Then
        IndCRevAmt = Round#(IndCRevAmt + Abs(UBCustRec(1).CurrRevAmts(RevChk)))
        If RevChk = 15 Then Rev15 = UBCustRec(1).CurrRevAmts(RevChk)
      End If
    Next
    If (IndCRevAmt > 0 And IndPRevAmt > 0) Or (Rev15 <> 0) Then
      If (IndPRevAmt <> IndCRevAmt) Or (Rev15 <> 0) Then
        OKToSkip = False
        Rev15 = 0
         End If
    End If
...
bskipem:
  ...
  ARptMastBalList.txtDate = Now
  ARptMastBalList.txtTown = TOWNNAME$
  ARptMastBalList.Title = "MBR Mismatched Credits"
  ARptMastBalList.txtRptParm1.Caption = ToPrintH1$
  ARptMastBalList.txtRptParm2.Caption = ToPrintH2$
  ARptMastBalList.txtTotCust = CustCnt
  ARptMastBalList.txtTotCur.DataValue = TCurrBalance#
