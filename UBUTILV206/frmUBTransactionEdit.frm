...
Begin VB.Form frmUBTransactionEdit 
   BackColor       =   &H008F8265&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Utility Transaction Edit"
   ClientHeight    =   8640
   ClientLeft      =   30
   ClientTop       =   540
   ClientWidth     =   12225
   ...
   Begin EditLib.fpText fpTransNum 
      Height          =   324
      Left            =   240
      TabIndex        =   53
      TabStop         =   0   'False
   ...
   Begin fpBtnAtlLibCtl.fpBtn fpCmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUBTransactionEdit.frx":0000
   End
   Begin fpBtnAtlLibCtl.fpBtn CmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUBTransactionEdit.frx":01DC
   End
   Begin fpBtnAtlLibCtl.fpBtn fpCmdTranHist 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUBTransactionEdit.frx":03B8
   End
   Begin fpBtnAtlLibCtl.fpBtn fpCmdFind 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUBTransactionEdit.frx":0595
   End
   ...
   Begin EditLib.fpDoubleSingle fpTotal 
      ...
      OLEDragMode     =   0
   End
   Begin EditLib.fpText fpTransDesc 
      Height          =   348
      Left            =   3168
      TabIndex        =   2
      Top             =   4680
      ...
      OLEDragMode     =   0
   End
   Begin EditLib.fpText fpTransType 
      Height          =   348
      Left            =   3168
      TabIndex        =   51
      Top             =   4296
      ...
      OLEDragMode     =   0
   End
   Begin EditLib.fpDoubleSingle fpRunBal 
      Height          =   324
      Left            =   3168
      TabIndex        =   54
      Top             =   5184
      ...
      OLEDragMode     =   0
   End
   Begin fpBtnAtlLibCtl.fpBtn fpBtnDelete 
      Height          =   390
      Left            =   6270
      TabIndex        =   56
      Top             =   7830
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUBTransactionEdit.frx":0770
   End
   ...
   End
End
Attribute VB_Name = "frmUBTransactionEdit"
...
'  End If
  CustAcct = 0
  BeenDone = False
 ' ActivateControls frmDispList

  Load frmUBEditMenu
  DoEvents
  frmUBEditMenu.Show
'  UBLog "OUT: UTIL ADJUST"
'  Load frmUBBillingMenu
'  DoEvents
'  frmUBBillingMenu.Show
'  Unload frmAdjustmentEntry
'  DoEvents
  Unload Me
End Sub
...
'  frmCustEditLookUP.Wheretogo frmAdjustmentEntry, frmAdjustmentEntry
'  '
'  DoEvents
...
'        Cancel = True
'      Else
'        UBLog "Closed via UtilAdj by " + PWUser$
'        CitiTerminate
'      End If
'    End If
'  End If
...
Private Sub loadCustrec()
  ...
  Close UBFile
  fpCustRecNo = CustAcct
  fptxtAccount = fpCustRecNo
  fpCustName = QPTrim(UBCustRec(1).CustName)
  fpAddr1 = QPTrim(UBCustRec(1).ADDR1)
...
Private Sub SaveTransaction()
  Dim UBTransRecLen As Integer, TransRec As Long
  ...
  Get UBTran, TransRec&, UBTransRec
  Close UBTran
  TransDate = Date2Num(txtDate)
  Transamt# = fpTotal.DoubleValue

  UBTransRec.TransDate = TransDate
 'UBTransRec.BillMsg = fpTransDesc
  UBTransRec.Transamt = Transamt#
  cnt = 0
  For cnt = 1 To 15
      UBTransRec.RevAmt(cnt) = fpAmount(cnt - 1)
  Next
  UBTransRec.RunBalance = fpRunBal.Value
  UBTran = FreeFile
  Open UBPath$ + "UBTRANS.DAT" For Random Shared As UBTran Len = UBTransRecLen
  Put UBTran, TransRec&, UBTransRec
  Close UBTran

  UBLog "Edited Trans:" + Str$(CustAcct&) + "  TRANS:" + Str$(TransRec&)
...
Private Sub DeleteTransaction()
 ...
 'UBTransRec.BillMsg = fpTransDesc
  UBTransRec.Transamt = 0
   ...
   UBTransRec.TransType = 0
   UBTransRec.TransDesc = "Invalid"
 For cnt = 1 To 7
  UBTransRec.MtrTypes(cnt) = 0
 UBTransRec.CurRead(cnt) = 0
 UBTransRec.PrevRead(cnt) = 0
 UBTransRec.ESTREAD(cnt) = "0"
 Next
   UBTransRec.BillNumber = 0
   UBTransRec.ReadDate = TransDate
   UBTransRec.BillDate = TransDate
   UBTransRec.PastDueDate = TransDate
   UBTransRec.DraftDate = TransDate
   UBTransRec.ProRatePCT = 0
   UBTransRec.ChkByte = "0"
   UBTransRec.EPPFlag = "0"
   UBTransRec.CustStatus = "0"
   UBTransRec.EPPTrans = 0
   UBTransRec.PenAtBill = 0
   UBTransRec.PayTypeCode = 0
   UBTransRec.OperatorNumber = 0
   UBTransRec.CustAcctNo = 0
   UBTransRec.PrevTrans = 0
   UBTransRec.VoidFlag = 0
   UBTransRec.FromCMFlag = 0
   UBTransRec.ActiveFlag = 0
   UBTransRec.RunBalance = 0
   UBTransRec.CheckAmount = 0
   UBTransRec.CashAmount = 0
   UBTransRec.BillMsg = ""
   UBTransRec.ApplyDepFlag = "0"
   UBTransRec.Posted2GL = "0"
   UBTransRec.PrevDate = 0
   UBTransRec.PenalFlag = "0"
   UBTransRec.TaxExempt = "0"
   UBTransRec.NONProfit = "0"
  
   cnt = 0
  For cnt = 1 To 15
      UBTransRec.RevAmt(cnt) = 0
      UBTransRec.TaxAmt(cnt) = 0
  Next
