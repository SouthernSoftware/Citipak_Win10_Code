...
Begin VB.Form frmSecurityPass 
   ...
   Begin VB.Label Label1 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "Enter CitiPak Password"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   12
         Charset         =   0
...
Private Sub cmdEnter_Click()
  ...
  Else
    If Len(Dir$("Citipass.dat")) Then
      ...
      If Findnum > 0 Then
        Call MainLog("In Citipak " + PWUser + " on " + QPTrim(ComputerName$))
        SetTemp
        frmMainMenu.Show
        Unload frmSecurityPass
      Else
        Notvalid = True
        If Findnum = -1 Then
          'MsgBox "Password for User ", vbOKOnly, "Access Denied"
          Call MainLog("Password in session, No access.")
          txtPW = ""
          txtPW.SetFocus
          Notvalid = False
        End If
...
'            LevelPass = 1
'          Else
'            If CitiPass.Module(3).ReportsOnly = True Then
'              LevelPass = 2
'            End If
'          End If
...
'            CitiPass.FlagMod = 3
        ...
        Else
          ...
          frmWarning.Label1.Caption = "User: " + theyare$ + " In Session on " + onwhat$
          frmWarning.Show 1
          'MsgBox "Password for User ", vbOKOnly, "Access Denied"
          Call MainLog("Password in session, Msg.")
          If frmWarning.nogo = True Then
            Findsettings = -1
          Else
            Call MainLog("Password in session, Go on ahead anyway.")
            Oktogo = True
            Citipass.InUseFlag = True
'            CitiPass.FlagMod = 3
            PWUser = QPTrim(Citipass.username)
