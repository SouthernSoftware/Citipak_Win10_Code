VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} ARptVendDist 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Vendor Default Distribution"
   ClientHeight    =   4380
   ClientLeft      =   36
   ClientTop       =   264
   ClientWidth     =   8964
   Icon            =   "ARptVendDist.dsx":0000
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   _ExtentX        =   15812
   _ExtentY        =   7726
   SectionData     =   "ARptVendDist.dsx":08CA
End
Attribute VB_Name = "ARptVendDist"
...
'    ' or user defined.
    ...
    Fields("Acct").Value = arr(2)
    Fields("AcctName").Value = arr(3)
    Fields("Percent").Value = arr(4)
...
Private Sub ActiveReport_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
  ...
  If Shift = 4 Then
    ...
    If KeyCode = vbKeyE Then
      ...
      DoEvents
      MsgBox "File - APVendDist.xls, created in the Citipak Directory.", vbOKOnly
    End If
    If KeyCode = vbKeyT Then
      ...
      Screen.MousePointer = vbDefault
      DoEvents
      MsgBox "File - APVendDist.txt, created in the Citipak Directory.", vbOKOnly
    End If
  End If
End Sub
...
Private Sub ExportReport(x As Integer)
  ...
  Select Case x
    Case 1   '"Excel"
        ...
        oEXL.FileName = outfile & "APVendDist.xls"
        oEXL.Export Me.Pages
    Case 2   '"Text"
        Set oTXT = New ActiveReportsTextExport.ARExportText
        oTXT.FileName = outfile & "APVendDist.txt"
        oTXT.PageDelimiter = ";"
        oTXT.TextDelimiter = ","
