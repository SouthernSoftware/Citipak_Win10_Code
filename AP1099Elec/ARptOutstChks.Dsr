VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} ARptOutstChks 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Outstanding Checks Report"
   ClientHeight    =   4380
   ClientLeft      =   36
   ClientTop       =   264
   ClientWidth     =   8664
   Icon            =   "ARptOutstChks.dsx":0000
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   _ExtentX        =   15282
   _ExtentY        =   7726
   SectionData     =   "ARptOutstChks.dsx":08CA
End
Attribute VB_Name = "ARptOutstChks"
...
'    ' or user defined.
    Fields("CheckNum").Value = arr(0)
    Fields("Date").Value = arr(1)
    Fields("Type").Value = arr(2)
    Fields("Desc").Value = arr(3)
    Fields("Amount").Value = arr(4)
    Fields("Bank").Value = arr(5)
...
Private Sub ActiveReport_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
  ...
  If Shift = 4 Then
    ...
    If KeyCode = vbKeyE Then
      ...
      DoEvents
      MsgBox "File - OutChks.xls, created in the Citipak Directory.", vbOKOnly
    End If
    If KeyCode = vbKeyT Then
      ...
      Screen.MousePointer = vbDefault
      DoEvents
      MsgBox "File - OutChks.txt, created in the Citipak Directory.", vbOKOnly
    End If
  End If
...
Private Sub ExportReport(x As Integer)
  ...
  Select Case x
    Case 1   '"Excel"
        ...
        oEXL.FileName = outfile & "OutChks.xls"
        oEXL.Export Me.Pages
    Case 2   '"Text"
        Set oTXT = New ActiveReportsTextExport.ARExportText
        oTXT.FileName = outfile & "OutChks.txt"
        oTXT.PageDelimiter = ";"
        oTXT.TextDelimiter = ","
