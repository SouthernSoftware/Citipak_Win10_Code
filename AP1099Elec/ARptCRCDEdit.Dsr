VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} ARptCRCDEdit 
   ...
   _ExtentX        =   20055
   _ExtentY        =   10292
   SectionData     =   "ARptCRCDEdit.dsx":08CA
End
Attribute VB_Name = "ARptCRCDEdit"
...
'    ' or user defined.
    Fields("Rec").Value = arr(0)
    Fields("Date").Value = arr(1)
    Fields("Desc").Value = arr(2)
    Fields("Ref").Value = arr(3)
    Fields("Bank").Value = arr(4)
    Fields("Amount").Value = arr(5)
    Fields("Acct").Value = arr(6)
    Fields("DistAmt").Value = arr(7)
...
Private Sub ActiveReport_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
  ...
  If Shift = 4 Then
    ...
    If KeyCode = vbKeyE Then
      ...
      DoEvents
      MsgBox "File - CEditrpt.xls, created in the Citipak Directory.", vbOKOnly
    End If
    If KeyCode = vbKeyT Then
      ...
      Screen.MousePointer = vbDefault
      DoEvents
      MsgBox "File - CEditrpt.txt, created in the Citipak Directory.", vbOKOnly
    End If
  End If
...
Private Sub ExportReport(x As Integer)
  ...
  Select Case x
    Case 1   '"Excel"
        ...
        oEXL.FileName = outfile & "CEditrpt.xls"
        oEXL.Export Me.Pages
    Case 2   '"Text"
        Set oTXT = New ActiveReportsTextExport.ARExportText
        oTXT.FileName = outfile & "CEditrpt.txt"
        oTXT.PageDelimiter = ";"
        oTXT.TextDelimiter = ","
