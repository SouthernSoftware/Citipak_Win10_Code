VERSION 5.00
Begin VB.Form frmCashReceiptsMenu 
   BackColor       =   &H008F8265&
   ...
   Begin VB.CommandButton cmdExitCashReceiptsMenu 
      Appearance      =   0  'Flat
      BackColor       =   &H00D0D0D0&
      Caption         =   "E&xit Cash Receipts Menu"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   10.5
         Charset         =   0
      ...
      Width           =   3612
   End
   Begin VB.CommandButton cmdPostCashRec 
      Appearance      =   0  'Flat
      BackColor       =   &H00D0D0D0&
      ...
      Width           =   3612
   End
   Begin VB.CommandButton cmdPrintCashRec 
      Appearance      =   0  'Flat
      BackColor       =   &H00D0D0D0&
      Caption         =   "&Print Cash Receipts Journal"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   10.5
         Charset         =   0
      ...
      Width           =   3612
   End
   Begin VB.CommandButton cmdEnterCashRec 
      Appearance      =   0  'Flat
      BackColor       =   &H00D0D0D0&
      Caption         =   "&Enter/Edit Cash Receipts"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   10.5
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmCashReceiptsMenu"
...
Private Sub cmdEnterCashRec_Click()
  Dim CRBusy As Boolean
  If Exist("GLCREd.DAT") Then CRBusy = GetAttr("GLCREd.DAT") And vbReadOnly
  If Not CRBusy Then
    frmCashReceiptEntry.Show
    Unload frmCashReceiptsMenu
    frmCashReceiptEntry.FirstOpenCR
  Else
    MsgBox "Posting Is In Progress, Editing Not Allowed At This Time.", vbOKOnly, "Request Canceled"
  End If
End Sub

Private Sub cmdExitCashReceiptsMenu_Click()
  frmGLMainMenu.Show
  Unload frmCashReceiptsMenu
End Sub

Private Sub cmdPostCashRec_Click()
frmPostCR.Show
End Sub

Private Sub cmdPrintCashRec_Click()
  frmReportOpt.Show 1
  If rptopt = 1 Then
    Call PrintEditList
...
Private Sub Form_Load()
  ...
  Temp_Class.InitResizeClass Me
   ' App.HelpFile = "helpfiles\citipak.hlp"
  Me.HelpContextID = hlpCashReceipts
End Sub
...
Private Sub PrintEditList()
  ...
  Dim LookFor As String, ReportFile2 As String, ToPrintF As String
  Dim Newrp As String, PRNFileNum2 As Integer
  Dim ReportFile As String, ToPrint As String
  Dim Header As String, ToPrintC As String
  ...
  For cnt = 1 To NumEdTrans
    ...
    If GLCREd(1).DelFlag = 0 Then
      Howmany = Howmany + 1
      ToPrintC$ = ""
      ToPrintC$ = Str(cnt) + "~" + Format(DateAdd("d", (GLCREd(1).TRDATE), "12-31-1979"), "mm/dd/yyyy")
      ToPrintC$ = ToPrintC$ + "~" + GLCREd(1).Desc + " " + Trim(GLCREd(1).LDesc)
      ToPrintC$ = ToPrintC$ + "~" + GLCREd(1).DOCREF
      ToPrintC$ = ToPrintC$ + "~" + Str$(GLCREd(1).RECCODE)
      ToPrintC$ = ToPrintC$ + "~" + Using$(CommaFmt$, GLCREd(1).Amt)
      
      GRANDTOT = Round#(GRANDTOT + GLCREd(1).Amt)
      totdist = 0
      For CntD = 1 To 36
        If Val(GLCREd(1).Dist(CntD).DACREC) > 0 Then
          ToPrint$ = ""
          ToPrint$ = ToPrintC$ + "~" + QPTrim(GLCREd(1).Dist(CntD).DACN) + " " + QPTrim(GLCREd(1).Dist(CntD).DACNM)
          ToPrint$ = ToPrint$ + "~" + Using$(CommaFmt$, GLCREd(1).Dist(CntD).DAMT)
          Print #PRNFileNum, ToPrint$
          
          totdist = Round#(totdist + GLCREd(1).Dist(CntD).DAMT)
        
        ' Sum into proper fund
          Found = False
          For Fund = 1 To NumFunds
            FundNum$ = Left$(GLCREd(1).Dist(CntD).DACN, GLFundLen)
            If FundNum$ = FundList$(Fund) Then
              Found = True
              FundTot#(Fund) = Round#(FundTot#(Fund) + GLCREd(1).Dist(CntD).DAMT)
              Exit For
            End If
           Next
        Else
...
Private Sub PrintEditList2()
  ...
  Dim Linecnt As Integer, PageNum As Integer, Newrp As String
  Dim ReportFile As String, ToPrint As String
  ...
  For cnt = 1 To NumEdTrans
    ...
    If GLCREd(1).DelFlag = 0 Then
      Howmany = Howmany + 1
      ToPrint$ = Space$(80)
      LSet ToPrint$ = Format(DateAdd("d", (GLCREd(1).TRDATE), "12-31-1979"), "mm/dd/yyyy")
      Mid$(ToPrint$, 13) = GLCREd(1).Desc
      Mid$(ToPrint$, 35) = GLCREd(1).DOCREF
      Mid$(ToPrint$, 46) = GLCREd(1).RECCODE
      Mid$(ToPrint$, 63) = Using$(CommaFmt$, GLCREd(1).Amt)
      Print #PRNFileNum, ToPrint$
      GRANDTOT = Round#(GRANDTOT + GLCREd(1).Amt)
      Linecnt = Linecnt + 1
      ToPrint$ = Space$(80)
      ...
      For CntD = 1 To 36
        If Val(GLCREd(1).Dist(CntD).DACREC) > 0 Then
          ToPrint$ = Space$(80)
          Mid$(ToPrint$, 2) = GLCREd(1).Dist(CntD).DACN
          Mid$(ToPrint$, 20) = GLCREd(1).Dist(CntD).DACNM
          Mid$(ToPrint$, 48) = Using$(CommaFmt$, GLCREd(1).Dist(CntD).DAMT)
          Print #PRNFileNum, ToPrint$
          Linecnt = Linecnt + 1
          totdist = Round#(totdist + GLCREd(1).Dist(CntD).DAMT)
          If Linecnt > MaxLines Then
...
'don't open file because viewprint does it there
   ViewPrint ReportFile$, "Cash Receipt Report"
   Kill ReportFile$
Exit Sub
...
PrintCRHeader:
  ...
  Print #PRNFileNum, "Cash Receipt Register"
