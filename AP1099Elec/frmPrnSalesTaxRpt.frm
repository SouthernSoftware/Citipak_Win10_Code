...
Begin VB.Form frmPrnSalesTaxRpt 
   BackColor       =   &H008F8265&
   ...
   ClientTop       =   495
   ClientWidth     =   12195
   Icon            =   "frmPrnSalesTaxRpt.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   8895
   ...
   Begin LpLib.fpCombo fpcboCoAcct 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmPrnSalesTaxRpt.frx":08CA
   End
   Begin LpLib.fpCombo fpcboStAcct 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmPrnSalesTaxRpt.frx":0CCD
   End
   Begin LpLib.fpCombo fpcboCombined 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmPrnSalesTaxRpt.frx":10D0
   End
   Begin LpLib.fpCombo fpcboStateCode 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmPrnSalesTaxRpt.frx":13FF
   End
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmPrnSalesTaxRpt.frx":172E
   End
   ...
   Begin VB.Image Image1 
      Height          =   345
      Left            =   2280
      Picture         =   "frmPrnSalesTaxRpt.frx":1A94
      Top             =   2370
      Width           =   360
   End
   ...
   End
End
Attribute VB_Name = "frmPrnSalesTaxRpt"
...
Private Sub Form_Load()
  ...
  Over.OverRide Me
  Me.HelpContextID = hlpSalesTx
  fpDate1.Text = Format(Now, "mm/dd/yyyy")
  fpDate2.Text = Format(Now, "mm/dd/yyyy")
  fpcboCombined.AddItem "No"
  fpcboCombined.AddItem "Yes"
  fpcboCombined.ListIndex = 0
  FillAcctstwo fpcboStAcct, fpcboCoAcct
  VendStCodeList fpcboStateCode
...
End Sub
Private Sub fpcboStAcct_LostFocus()
  fpcboStAcct.Action = ActionClearSearchBuffer
End Sub
...
Private Sub SalesTaxReport()
  ...
  Dim ToPrint1 As String, xx As String
...
Private Sub SalesTaxReport2()
  ...
  Dim StTax As Double, ccnt As Integer, CoTax As Double, PRNFile As Integer
  Dim ReportFile As String, Header As String, TotList As Integer
  Dim StateAmt As Double, CountyAmt As Double, StateCd As String, OnceTrans As Integer
  Dim all As Boolean, Newrp As String, TEST As String, PurchTot As Double
  If fpcboCombined.ListIndex = 1 Then Combined = True
  fpcboStAcct.col = 1
  fpcboCoAcct.col = 1
  StateTaxRecAcct$ = QPTrim$(fpcboStAcct.ColText)
  CountyTaxRecAcct$ = QPTrim$(fpcboCoAcct.ColText)
  StateFactor# = Val(fpdblFactor) * 0.01
  BegDate = DateDiff("d", "12/31/1979", fpDate1)
  EndDate = DateDiff("d", "12/31/1979", fpDate2)
  PRNFile = FreeFile
  Newrp = "SalTax"
              ...
              Do
                ...
                If QPTrim$(APDist.DistAcctNum) = StateTaxRecAcct$ Then
                  ...
                  If StateCd = QPTrim(Vendor.StCode) Then
                    ...
                    PrintFlag = 1
                    If OnceTrans = 0 Then
                      PurchTot = PurchTot + ApLedger.Amt
                      OnceTrans = 1
                    End If
                    'offset = State
                    If all Then offset = 0
...
PrintState:
  ...
  End If
'   Print #PRNFile, "Total Purchased "; Using("#,###,###,###.##", PurchTot)
...
Private Function VendStCodeList(x As fpCombo)
  ...
  Dim Vendor As VendorRecType
    ...
    For cnt = 1 To NumVRecs
      ...
      If Not Vendor.DelFlag Then
        x.SearchText = QPTrim(Vendor.StCode)
        x.Action = 0
        If x.SearchIndex = -1 Then
          If Len(QPTrim(Vendor.StCode)) > 0 Then
            x.AddItem Vendor.StCode
          End If
        End If
      End If
    Next
...
Private Sub SalesTaxReporttest()
  Dim Combined As Boolean, StateTaxRecAcct As String, CountyTaxRecAcct As String
  Dim StateFactor As Double, BegDate As Integer, EndDate As Integer
  Dim STATE As Integer, PrintFlag As Integer, StTotTax As Double
  Dim TotState As Double, TotCounty As Double, TCnt As Integer
  ...
  Dim StateAmt As Double, CountyAmt As Double, StateCd As String, OnceTrans As Integer
  Dim all As Boolean, Newrp As String, TEST As String, PurchTot As Double
  Dim stacct As String, coacct As String, StAmt As String, CoAmt As String
  If fpcboCombined.ListIndex = 1 Then Combined = True
  fpcboStAcct.col = 1
  fpcboCoAcct.col = 1
  StateTaxRecAcct$ = QPTrim$(fpcboStAcct.ColText)
        ...
        Do
          ...
          If ApLedger.TRCode = 1 Then
            ...
            If ApLedger.GLDistDate >= BegDate And ApLedger.GLDistDate <= EndDate Then
              ...
              If StateCd = QPTrim(Vendor.StCode) Or all = True Then
                    Do
                      Get APDistFile, NextDist&, APDist
                      Get VendorFile, ApLedger.VRecNum, Vendor
                      If QPTrim$(APDist.DistAcctNum) = StateTaxRecAcct$ Then
                        Print #PRNFile, QPTrim(Vendor.StCode) + "," + QPTrim(Vendor.CoCode) + "," + QPTrim$(APDist.DistAcctNum) + "," + Vendor.vnum + "," + Str$(APDist.DistAmt)
                        StTax# = StTax# + APDist.DistAmt
                      ElseIf QPTrim$(APDist.DistAcctNum) = CountyTaxRecAcct$ Then
                        Print #PRNFile, QPTrim(Vendor.StCode) + "," + QPTrim(Vendor.CoCode) + "," + QPTrim$(APDist.DistAcctNum) + "," + Vendor.vnum + "," + Str$(APDist.DistAmt)
                        CoTax# = CoTax# + APDist.DistAmt
                      End If
'               Do
...
'                        'If all Then offset = 0
'                        StSalesTaxPaid#(Coffset) = StSalesTaxPaid#(Coffset) + APDist.DistAmt
'                        Print #PRNFile, QPTrim(Vendor.StCode) + "," + QPTrim(Vendor.CoCode) + "," + QPTrim$(APDist.DistAcctNum) + "," + Vendor.vnum + "," + Str$(APDist.DistAmt) + "," + Str$(StSalesTaxPaid#(Coffset)) + "," + Str$(CoSalesTaxPaid#(Coffset))
'
'                    End If
'                    If QPTrim$(APDist.DistAcctNum) = CountyTaxRecAcct$ Then
'                     ' If StateCd = QPTrim(Vendor.StCode) Or all = True Then
...
' "State Code Being Searched: ";: Color 15: Print State
  ...
  If TotList < 1 Then
    ...
    Print #PRNFile, "No Information to Display"
  End If
   Print #PRNFile, Str$(StTax#) + "," + Str$(CoTax#)
  Close
  Unload FrmShowPctComp
  ActivateControls frmPrnSalesTaxRpt, True
  ViewPrint ReportFile$, Header$
...
PrintState:
  ...
  Else
    ...
    Next
    Print #PRNFile, "--------------------------------------------"
    Print #PRNFile, "      Totals"; Tab(19); Using("######.##", StTotTax#); Tab(34); Using("######.##", CoTotTax#)
  End If
'   Print #PRNFile, "Total Purchased "; Using("#,###,###,###.##", PurchTot)
  Print #PRNFile, Chr$(12)
  Return
