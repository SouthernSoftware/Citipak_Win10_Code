VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} ARptCkListDist 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "A/P Check Listing Distribution Detail"
   ClientHeight    =   4380
   ClientLeft      =   36
   ClientTop       =   264
   ClientWidth     =   9204
   Icon            =   "ARptCkListDist.dsx":0000
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   _ExtentX        =   16235
   _ExtentY        =   7726
   SectionData     =   "ARptCkListDist.dsx":08CA
End
Attribute VB_Name = "ARptCkListDist"
...
'    ' or user defined.
    Fields("ChkNum").Value = arr(0)
    Fields("VDesc").Value = arr(1)
    Fields("Amount").Value = arr(2)
    Fields("InvRec").Value = arr(3)
    Fields("InvNum").Value = arr(4)
    Fields("InvDate").Value = arr(5)
    Fields("DueDate").Value = arr(6)
    Fields("PO").Value = arr(7)
    Fields("InvAmt").Value = arr(8)
    Fields("DistAcct").Value = arr(9)
...
Private Sub ActiveReport_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
  ...
  If Shift = 4 Then
    ...
    If KeyCode = vbKeyE Then
      ...
      DoEvents
      MsgBox "File - APChkListD.xls, created in the Citipak Directory.", vbOKOnly
    End If
    If KeyCode = vbKeyT Then
      ...
      Screen.MousePointer = vbDefault
      DoEvents
      MsgBox "File - APChkListD.txt, created in the Citipak Directory.", vbOKOnly
    End If
  End If
...
Private Sub ExportReport(x As Integer)
  ...
  Select Case x
    Case 1   '"Excel"
        ...
        oEXL.FileName = outfile & "APChkListD.xls"
        oEXL.Export Me.Pages
    Case 2   '"Text"
        Set oTXT = New ActiveReportsTextExport.ARExportText
        oTXT.FileName = outfile & "APChkListD.txt"
        oTXT.PageDelimiter = ";"
        oTXT.TextDelimiter = ","
