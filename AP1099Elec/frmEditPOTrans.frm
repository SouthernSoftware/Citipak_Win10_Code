...
Begin VB.Form frmEditPOTrans 
   BackColor       =   &H008A775B&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Edit PO Transaction"
   ClientHeight    =   8892
   ClientLeft      =   36
   ClientTop       =   264
   ClientWidth     =   12192
   Icon            =   "frmEditPOTrans.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "frmEditPOTrans"
...
Private Function TransSearch()
  ...
  If RecNum& > 0 And RecNum& <= NumTrans& Then
    Get POTransFile, RecNum&, POTrans
    fpmskAcctNum = POTrans.AcctNum
    txtDate = Format(DateAdd("d", (POTrans.TRDATE), "12-31-1979"), "mm/dd/yyyy")
    txtDescription = POTrans.Desc
    txtReference = POTrans.Ref
    txtDebit = Str$(POTrans.DrAmt)
    txtCredit = Str$(POTrans.CrAmt)
    txtSource = POTrans.Src
    Close POTransFile
    GoodRec = True
  Else
...
Private Sub SaveRec()
  ...
  Open "POTRANS.DAT" For Random Access Read Write Shared As POTransFile Len = TransRecLen
  NumTrans& = LOF(POTransFile) \ TransRecLen
  POTrans.AcctNum = QPTrim(fpmskAcctNum)
  POTrans.TRDATE = DateDiff("d", "12/31/1979", txtDate)
  POTrans.Desc = QPTrim(txtDescription)
  POTrans.Ref = QPTrim(txtReference)
  POTrans.DrAmt = txtDebit.DoubleValue()
  POTrans.CrAmt = txtCredit.DoubleValue()
  POTrans.Src = QPTrim(txtSource)
  Put POTransFile, RecNum&, POTrans
  Close
  Call MainLog("poTrans: " + Str(RecNum&) + " " + QPTrim(fpmskAcctNum) + " Edit/GLUtil.")
End Sub
