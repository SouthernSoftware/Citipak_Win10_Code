...
Begin VB.Form frmEmployerInfoFile 
   ...
   End
   Begin LpLib.fpCombo fpcmbRothDed 
      Height          =   405
      Left            =   7920
      TabIndex        =   54
      ToolTipText     =   "Sets the default Loan Payment deduction that will automatically appear on the Supplemental Retirement report screen."
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmEmployerInfoFile.frx":1BD7
   End
   Begin EditLib.fpCurrency fpCurrMaxSalary 
      Height          =   375
      Left            =   3360
      TabIndex        =   24
      Top             =   7800
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmEmployerInfoFile.frx":1ECE
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmEmployerInfoFile.frx":20AA
   End
   Begin EditLib.fpDoubleSingle fptxtCodeL 
      ...
      OLEDragMode     =   0
   End
   Begin EditLib.fpText fptxtORBITAgencyNum 
      Height          =   375
      Left            =   3360
      TabIndex        =   23
      ToolTipText     =   "This field is for North Carolina only."
      Top             =   7320
      Width           =   1935
      _Version        =   196608
      _ExtentX        =   3413
...
'   On Error Resume Next
   ...
   Dim ORec As OrbitHeader 'new for ORBIT
   ...
   Open401KDedFile KHandle
    ...
    K401Rec.RothDed = QPTrim$(fpcmbRothDed.Text)
    Put KHandle, 1, K401Rec
   Close KHandle
   
   If Exist(OrbitHeader) Then
    OpenOrbHeader OHandle 'new for ORBIT
    Get OHandle, 1, ORec
   Else
    OpenOrbHeader OHandle 'new for ORBIT
   End If
   ORec.AgencyNum = QPTrim$(fptxtORBITAgencyNum.Text)
   ORec.MaxSalary = fpCurrMaxSalary.Value
   Put OHandle, 1, ORec
   Close OHandle
...
'   On Error Resume Next
   ...
   If NumOfDeds = 0 Then
     ...
     fpcmbRothDed.Text = "No Deductions Saved"
     Close DHandle
   End If
   ...
   For x = 1 To NumOfDeds
     ...
     If x < 10 Then
       ...
       fpcmbRothDed.AddItem " " + CStr(x) + ") " + (DedRec.DCDESC1)
     Else
       fpcomboVolDed.AddItem CStr(x) + ") " + (DedRec.DCDESC1)
       fpcomboLPDed.AddItem CStr(x) + ") " + (DedRec.DCDESC1)
       fpcmbRothDed.AddItem CStr(x) + ") " + (DedRec.DCDESC1)
   ...
   If Exist(K401DedName) Then
     ...
     fpcmbRothDed.Text = QPTrim$(K401Rec.RothDed)
   Else
     fpcomboVolDed.Text = "Unsaved"
     fpcomboLPDed.Text = "Unsaved"
     fpcmbRothDed.Text = "Unsaved"
   End If
...
NoUnitFileYet:
   ...
   Close UnitHandle
   If unitStateTxt.Text <> "NC" Then
     fptxtORBITAgencyNum.Enabled = False
     Exit Sub
   End If
   If Exist(OrbitHeader) Then 'new for ORBIT
     OpenOrbHeader OHandle
     Get OHandle, 1, ORec
     Close OHandle
     fptxtORBITAgencyNum.Text = QPTrim$(ORec.AgencyNum)
     fpCurrMaxSalary = ORec.MaxSalary
   Else
     fptxtORBITAgencyNum.Text = ""
     fpCurrMaxSalary = 0
   End If
End Sub
...
Private Sub fpcmbRothDed_BeforeDropDown(Cancel As Boolean)
  If fpcmbRothDed.ListCount = 0 And fpcmbRothDed.Text = "Unsaved" Then
    MsgBox "No deduction list available at this time. Check to make sure deductions are saved."
  End If

End Sub

Private Sub fpcmbRothDed_KeyDown(KeyCode As Integer, Shift As Integer)
  If KeyCode = vbKeySpace Then
    fpcmbRothDed.ListDown = True
  End If
  If fpcmbRothDed.ListDown = False Then
    If KeyCode = vbKeyDown Then
      SendKeys "{Tab}"
      KeyCode = 0
    ElseIf KeyCode = vbKeyUp Then
...
Private Sub unitStateTxt_Change() 'new for ORBIT
  If unitStateTxt.Text = "NC" Then
    fptxtORBITAgencyNum.Enabled = True
  Else
    fptxtORBITAgencyNum.Enabled = False
  End If
End Sub
