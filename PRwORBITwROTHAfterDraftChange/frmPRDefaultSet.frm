...
Private Sub cmdSave_Click()
  ...
  If Look4ORBITEligibilityIssue = True Then
    frmOrbElProbList.Show vbModal
    If frmOrbElProbList.fptxtResponse.Text = "abort" Then
      Exit Sub
    End If
  End If
...
Private Function Look4ORBITEligibilityIssue() As Boolean
 ...
 Dim ORecEmpNum As String
 
 OrbElCnt = 0
 ORecEmpNum = ""
 Look4ORBITEligibilityIssue = False
 If Not Exist(OrbitEmpData) Then Exit Function
 OpenOrbEmpData OHandle, NumOfORecs
 OpenEmpData2File EHandle
 NumOfERecs = LOF(EHandle) / Len(EmpRec)
 ReDim OrbElProb(1 To 1) As Integer
 For x = 1 To NumOfORecs
   Get OHandle, x, ORec
   If Len(QPTrim$(ORec.EligibleDate)) <> 8 Then GoTo SkipIt
   ORecEmpNum = QPTrim$(ORec.EmpNum)
   GoSub ConvertODate
   If Date2Num(ConvDate) <= Date2Num(fpDateTimeBeg.Text) Then
     For y = 1 To NumOfERecs
       Get EHandle, y, EmpRec
       If ORecEmpNum = QPTrim$(EmpRec.EmpNo) And ORec.Deleted = False Then Exit For 'added ORec.Deleted = False on 12/5/08
     Next y
     If y > NumOfERecs Then GoTo SkipIt
     If QPTrim$(EmpRec.EMPRETNO) = "" Then
       OrbElCnt = OrbElCnt + 1
       ReDim Preserve OrbElProb(1 To OrbElCnt) As Integer
       OrbElProb(OrbElCnt) = x
     End If
    End If
SkipIt:
 Next x
 ...
 If OrbElCnt > 0 Then
  Look4ORBITEligibilityIssue = True
 End If
 Exit Function
...
Private Function FormatODate(ByRef ThisDate As String, ByVal Vers As Integer) As String
  Dim ch As String
