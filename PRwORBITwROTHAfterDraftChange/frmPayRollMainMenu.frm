...
Begin VB.Form frmPayrollMainMenu 
   ...
   Begin VB.Menu mnuOptions 
      ...
      End
      Begin VB.Menu mnuRelinkPayRates 
         Caption         =   "Relink Pay Rates"
      End
      Begin VB.Menu mnuFixCanton 
         Caption         =   "Fix Canton Orbit Detail"
      End
      Begin VB.Menu mnuFixTown 
         Caption         =   "Fix Maggie Valley"
      End
   End
End
...
'  End If
  ...
  If Exist(PRData + UnitFileName) Then 'new for ORBIT
    OpenUnitFile UHandle
    Get UHandle, 1, UnitRec
    Close UHandle
    If UnitRec.UFSTATE = "NC" Then
      ThisDir = StartPath + "\NCORBIT"
      If Not DirExists(ThisDir) Then
        frmMessageWOpts.Label1.Caption = "The directory 'NCORBIT' could not be located in the Citipak directory. This file is only needed for customers in North Carolina."
        frmMessageWOpts.Label1.Top = 900
        frmMessageWOpts.cmdCont.Text = "F10 Make NCORBIT"
        frmMessageWOpts.cmdExit.Text = "ESC Escape"
        frmMessageWOpts.Show vbModal
        If frmMessageWOpts.fptxtChoice.Text = "continue" Then
          Unload frmMessageWOpts
          MkDir StartPath + "\NCORBIT"
        Else
          Unload frmMessageWOpts
        End If
      End If
...
'  LevelPass = 1
'  PWcnt = 6
'  MsgBox ("Don't forget to change the start date on the Report Menu for retirement to 10/1/07 for NC customers.")
...
'  Dim NumOFERRecs As Integer
'  Dim DHandle As Integer
'  Dim NextRec As Long
'  Dim Checks(10) As Long
'  Checks(1) = 17869
'  Checks(2) = 17870
'  Checks(3) = 17871
'  Checks(4) = 17872
...
'  GotIt = False
'  ThisDate = Date2Num("12/15/2009")
'  OpenTransHistFile TCHandle
'  NumOfTCRecs = LOF(TCHandle) / Len(TransRec)
'  NumOFERRecs = LOF(DHandle) / Len(EmpRec)
'  y = 1
'  For x = 1 To NumOfTCRecs
'    Get TCHandle, x, TransRec
'    For y = 1 To 10
'        If TransRec.CheckNum = Checks(y) Then
'            If GotIt = False And TransRec.CheckDate = ThisDate Then
'              GotIt = True
'              GoTo NextOne
'            End If
'            If GotIt = True And TransRec.CheckDate = ThisDate Then
''              y = y + 1
'              Check = CStr(TransRec.CheckNum)
'              Check = Check + "1"
'              TransRec.CheckNum = CLng(Check)
'              Put TCHandle, x, TransRec
'              GotIt = False
'            End If
'        End If
'    Next y
...
Private Sub mnuFixCanton_Click()
  Dim ORec As OrbitDetail
  Dim OHandle As Integer
  Dim NumOfORecs As Integer
  
  OpenOrbDetail OHandle, NumOfORecs
  Get OHandle, 291, ORec
  ORec.EmpRecNum = 74
  Put OHandle, 291, ORec
  
  Get OHandle, 292, ORec
  ORec.EmpRecNum = 74
  Put OHandle, 292, ORec
...
Private Sub mnuFixTown_Click()
Call ReIndexOrbEmpRecs
...
Private Sub mnuRelinkPayRates_Click()
  ...
  TYHandle = FreeFile
  Open "RelinkPayRates.txt" For Output As TYHandle
  Print #TYHandle, "Emp # Before " + "~" + "Emp # After " + "~" + "Employee Name"
  OpenPayRateFile PHandle
  ...
  For x = 1 To NumOfPayRecs
    ...
    If QPTrim$(PayRec.EmpNo) <> QPTrim$(EmpRec.EmpNo) Then
      Print #TYHandle, CStr(PayRec.EmpNo) + "~" + CStr(EmpRec.EmpNo) + "~" + QPTrim$(EmpRec.EmpFName) + " " + QPTrim$(EmpRec.EmpLName)
      PayRec.EmpNo = QPTrim$(EmpRec.EmpNo)
  ...
  Close
  MakePayRateIndex
  MsgBox ("A total of " + CStr(cnt) + " pay rate accounts were updated. Look for RelinkPayRates.txt in Citipak folder.")
  
End Sub
Private Sub ReIndexOrbEmpRecs()
  Dim ORec As OrbitEmpData
  Dim OHandle As Integer
  ReDim TempORec(1 To 1) As OrbitEmpData
  Dim TempCnt As Integer
  Dim TempOHandle As Integer
  Dim NumOfORecs As Integer
  Dim EmpRec As EmpData2Type
  Dim EHandle As Integer
...
'  NumOfERecs = LOF(EHandle) / Len(EmpRec)
  ...
  OpenOrbEmpData OHandle, NumOfORecs
  For x = 1 To NumOfORecs - 1
    Get OHandle, x, ORec
    TempCnt = TempCnt + 1
    ReDim Preserve TempORec(1 To TempCnt) As OrbitEmpData
    TempORec(TempCnt) = ORec
  Next x
  Close
  
  KillFile OrbitEmpData
  
  OrbLen = Len(ORec)
  OHandle = FreeFile
  Open OrbitEmpData For Random Shared As OHandle Len = OrbLen
  For x = 1 To TempCnt
    ORec = TempORec(x)
    Put OHandle, x, ORec
  Next x
...
'    Get OHandle, x, ORec
'    OEmpRec = ORec.EmpRecNum
'    If OEmpRec = 197 Then Stop
'    If OEmpRec = 151 Then Stop
'
'    OSSN = ORec.SSN
'    If ReplaceString(OSSN, "-", "") = "238293801" Then Stop
'    Get EHandle, OEmpRec, EmpRec
''    If ReplaceString(EmpRec.EmpSSN, "-", "") = "238293801" Then Stop
'    EmpRec.EmpNo = EmpRec.EmpNo
'    If ReplaceString(OSSN, "-", "") <> ReplaceString(EmpRec.EmpSSN, "-", "") Then
'     ' ORec.SSN = EmpRec.EmpSSN
'      'Put OHandle, x, ORec
'      x = x
'    End If
'   Next x
