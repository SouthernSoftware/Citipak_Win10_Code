...
Option Explicit
  ...
  Public OrbElProb() As Integer
  Public OrbElCnt As Integer
...
'  Public GEmpNum As Integer
  ...
  Public MaxSalaryPL() As String 'new for ORBIT
  Public MaxCnt As Integer 'new for ORBIT
  ...
  lpBuffer As String, nSize As Long)
         ...
         Public Const SupHeader = "SUPHEADER.DAT"
         Public Const SupDetail = "SUPDETAIL.DAT"
           Public Const SupLoan = "SUPLOAN.DAT"
        Public Const SupTrailer = "SUPTRAILER.DAT"
...
Public Const FederalTaxFileName = "PRFEDTAX.DAT"
 ...
 Public Const TransHistFileName = "PRTRANSH.DAT"
    ...
    Public Const AccrueFileName = "PRDATA\PRACCRUE.DAT"
...
Public Const PPDraftInfoFileName = "PRDATA\PPDFINFO.DAT"
  Public Const DraftInfoFileName = "PRDATA\PRDRAFTI.DAT"
     ...
     Public Const Draft9FileName = "PRDATA\PRDRAFT9.DAT"
      ...
      Public Const PayRateName = "PRDATA\PAYRATE.DAT"
      ...
      Public Const MessageName = "PRDATA\EMPMESS.DAT"
      Public Const OrbitHeader = "PRDATA\OrbHeader.Dat"
      Public Const OrbitDetail = "PRDATA\OrbDetail.Dat"
      Public Const OrbitTrailer = "PRDATA\OrbTrailer.Dat"
      Public Const OrbitEmpData = "PRDATA\OrbEmpData.Dat"
      Public Const OrbitEmpDataBatch = "PRDATA\OrbEmpDataBatch.Dat"
      Public Const ESCLineN = "PRDATA\ESCLineN.Dat"
Public Sub OpenESCLineN(ESCHandle As Integer, NumOfESCRecs As Integer)
  Dim ESCLen As Integer
'  Dim ESCRec As ESCLineN
'  ESCLen = Len(ESCRec)
'  ESCHandle = FreeFile
'  Open ESCLineN For Random Shared As ESCHandle Len = ESCLen
'  NumOfESCRecs = LOF(ESCHandle) / ESCLen
End Sub
Public Sub OpenOrbEmpDataBatch(OrbHandle As Integer, NumOfOrbRecs As Integer)
  Dim OrbLen As Integer
  Dim OrbRec As OrbitEmpDataBatch
  OrbLen = Len(OrbRec)
  OrbHandle = FreeFile
  Open OrbitEmpDataBatch For Random Shared As OrbHandle Len = OrbLen
  NumOfOrbRecs = LOF(OrbHandle) / OrbLen
End Sub
Public Sub OpenOrbEmpData(OrbHandle As Integer, NumOfOrbRecs As Integer)
  Dim OrbLen As Integer
  Dim OrbRec As OrbitEmpData
  OrbLen = Len(OrbRec)
  OrbHandle = FreeFile
  Open OrbitEmpData For Random Shared As OrbHandle Len = OrbLen
  NumOfOrbRecs = LOF(OrbHandle) / OrbLen
End Sub
Public Sub OpenOrbHeader(OrbHandle As Integer)
  Dim OrbLen As Integer
  Dim OrbRec As OrbitHeader
  OrbLen = Len(OrbRec)
  OrbHandle = FreeFile
  Open OrbitHeader For Random Shared As OrbHandle Len = OrbLen
End Sub
Public Sub OpenOrbDetail(OrbHandle As Integer, NumOfOrbRecs As Integer)
  Dim OrbLen As Integer
  Dim OrbRec As OrbitDetail
  OrbLen = Len(OrbRec)
  OrbHandle = FreeFile
  Open OrbitDetail For Random Shared As OrbHandle Len = OrbLen
  NumOfOrbRecs = LOF(OrbHandle) / OrbLen
End Sub
Public Sub OpenOrbTrailer(OrbHandle As Integer)
  Dim OrbLen As Integer
  Dim OrbRec As OrbitTrailer
  OrbLen = Len(OrbRec)
  OrbHandle = FreeFile
  Open OrbitTrailer For Random Shared As OrbHandle Len = OrbLen
End Sub
...
End Sub
Public Sub OpenSupHeader(SHandle As Integer)
  Dim SupHeaderLen As Integer
  Dim SupHdr As SupHeaderRecType
  SupHeaderLen = Len(SupHdr)
  SHandle = FreeFile
  Open SupHeader For Random Shared As SHandle Len = SupHeaderLen
End Sub
Public Sub OpenSupTrailer(SHandle As Integer)
  Dim SupTrailerLen As Integer
  Dim SupTrlr As SupTrailerRecType
  SupTrailerLen = Len(SupTrlr)
  SHandle = FreeFile
  Open SupTrailer For Random Shared As SHandle Len = SupTrailerLen
End Sub
Public Sub OpenSupDetail(SHandle As Integer, NumOfDetRecs As Integer)
  Dim SupDetailLen As Integer
  Dim SupDtl As SupDetailRecType
  SupDetailLen = Len(SupDtl)
  SHandle = FreeFile
  Open SupDetail For Random Shared As SHandle Len = SupDetailLen
  NumOfDetRecs = LOF(SHandle) / SupDetailLen
End Sub
Public Sub OpenSupLoan(SHandle As Integer, NumOfLoanRecs As Integer)
  Dim SupLoanLen As Integer
  Dim SupLn As SupLoanRecType
  SupLoanLen = Len(SupLn)
  SHandle = FreeFile
  Open SupLoan For Random Shared As SHandle Len = SupLoanLen
  NumOfLoanRecs = LOF(SHandle) / SupLoanLen
End Sub
...
Public Sub OpenPayRateFile(PayRateHandle As Integer)
  ...
  Open PayRateName For Random Shared As PayRateHandle Len = PayRateLen
End Sub
...
Public Sub OpenAccrualDatesFile(AccrualDatesHandle As Integer) '12/12/02
  ...
  Open AccrueFileName For Random Shared As AccrualDatesHandle Len = AccrualDatesLen
End Sub
...
Public Sub MakeEmpIndexs()
  ...
  For x = 1 To NumOfEmpRec
    ...
    EmpIdxLNameRec(Found).EmpName = QPTrim$(EmpData2FileRec.EmpLName) + QPTrim$(EmpData2FileRec.EmpFName)
...
Public Sub CalcPay(TransRec As TransRecType, TransRecNo As Long, ReCalcFlag As Boolean)
  ...
  Case "H"
    ...
    '6/20/07 you have not added any difference corrections
  ...
  For x = 1 To DedCnt
    ...
    DedCodes.DCDESC1 = DedCodes.DCDESC1
...
'  If Len(Emp2Rec(1).EMPRETNO) > 0 And Mid(Emp2Rec(1).EMPRETNO, 1, 1) <> "T" And Mid(Emp2Rec(1).EMPRETNO, 1, 1) <> "R" Then '7/22/2004
  If Len(QPTrim$(Emp2Rec(1).EMPRETNO)) > 0 And Mid(Emp2Rec(1).EMPRETNO, 1, 1) <> "T" And Mid(Emp2Rec(1).EMPRETNO, 1, 1) <> "R" Then '8/23/2005
    ...
    Case Else
      ...
      TransRec.RetGrossPay = 0 '1/29/08
    End Select
  Else
    TransRec.RetireAmt = 0
    TransRec.MatchRetAmt = 0
...
'      Exmp3Amt# = OldRound(Exmp3Amt# * (StateTax(1).TAX105 * 0.01))
      ...
      If Exmp3Amt# < 0 Then Exmp3Amt# = 0
      TotalExmp# = Exmp3Amt# 'OldRound(Exmp1Amt# + Exmp2Amt# + Exmp3Amt#)

      TaxableAmtSta# = TotalExmp# 'OldRound#(AnnualizedStaGross# - TotalExmp#)
...
'******
  ...
  Select Case QPTrim$(Emp2Rec(1).EMPSTAS)
    ...
    Case "H" '"F"  'Table F - Married one income, "H",
...
'        TransRec.StaTaxAmt = OldRound#(OldRound#(StateTax(1).STM(1, 5) + (TaxableAmtSta# * (StateTax(1).STM(2, 5) * 0.01))) / PayFreq)
    ...
    Case "F", "" '"H", ""  'Georgia table H,"S", - Single Individual
...
'  OSChkFile$ = QPTrim$(PSysRec(1).CITIDIR)
  ...
  If TransRec(1).TActive = -1 Then
    ...
    If FirstFlag Then
      ...
      '** Update OSChk File
      If DoOSChkFlag Then
        If Check(1).CheckNum = 0 Then GoTo CheckNumIsZero 'added 4/8/09
...
'  End If
  ...
  If Exist("draftsent.dat") Then
    KillFile "draftsent.dat"
  End If
...
''    If VoidRec.CheckDate = Date2Num("6/25/2008") Then Stop
''      If QPTrim$(VoidRec.EmpNum) = "41602" Then
''        Stop
''      End If
''    End If
'    If VoidRec.CheckNum = -1235 Then Stop
'    Next x
      ...
      If VoidRec.CheckNum = Chknum Then
        If VoidRec.CheckDate = ChkDate Then
          If QPTrim$(VoidRec.EmpNum) = QPTrim$(EmpNum$) Then
            ...
            If VoidRec.PRNet = 0 Then GoTo TryThis 'SkipPRNet
...
Public Sub UpdatePayRate(JobName$, PayType$, OTRate As Double, RegRate As Double, Freq As String, RecNum, NewEmp As Boolean)
  ...
  If NewEmp = True Then
    GoSub SaveNewEmp
    GoTo GotIt
  ElseIf frmEmpQuickMaintJobDesc.Visible = False Then 'added 1/22/08 and 3/6/08
    OpenPayRateFile PHandle
    Get PHandle, RecNum, PayRate
    If QPTrim$(frmEditEmpData.txtNumber.Text) <> QPTrim$(PayRate.EmpNo) Then
      PayRate.EmpNo = QPTrim$(frmEditEmpData.txtNumber.Text)
