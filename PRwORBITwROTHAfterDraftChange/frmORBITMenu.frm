...
Begin VB.Form frmORBITMenu 
   BackColor       =   &H008F8265&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Retirement/ORBIT Menu"
   ClientHeight    =   8835
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   11640
   Icon            =   "frmORBITMenu.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdPrint 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmORBITMenu.frx":08CA
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdEdit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmORBITMenu.frx":0AAA
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdProcess 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmORBITMenu.frx":0C99
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdCreateFile 
      Height          =   495
      Left            =   4005
      TabIndex        =   3
      Top             =   5025
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmORBITMenu.frx":0E85
   End
   Begin fpBtnAtlLibCtl.fpBtn exitCmd 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmORBITMenu.frx":106F
   End
   ...
   End
End
Attribute VB_Name = "frmORBITMenu"
...
Dim FromCreateExport As Boolean

Private Sub cmdCreateFile_Click()
  Dim ThisDir As String

  ThisDir = StartPath + "\NCORBIT" 'new for ORBIT
'
  If Not DirExists(ThisDir) Then
    MsgBox ("The directory 'NCORBIT' cannot be found in the Citipak directory. The NC ORBIT file cannot be created without it.")
    Exit Sub
  End If
  ...
  If Check4PostedORBIT = True Then
    MsgBox ("The current ORBIT file has already been posted.")
  End If
...
'  MainLog ("User created ORBIT export file.")
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmORBITMenu", "PrintGraphics", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
'    If ORec.Salary > 0 Then 'removed 9/3/2008
      For t = 1 To 19
...
PrintRptFooter:
  ...
  For y = 1 To 19
    If PCCnt(y) > 0 Then
      ...
      Print #RptHandle, ThisPC; Tab(27); Using$("#,###", PCCnt(y)); Tab(35); Using$("$###,###.##", PCPay(y)); Tab(57); Using$("$##,###.##", PCEmpCont(y)); Tab(78); Using$("$##,###.##", PCCityCont(y))
    End If
  Next y
  Print #RptHandle, String$(87, "-")
  Print #RptHandle, "Plan Code Grand Totals "; Tab(27); Using$("#,###", TotPCCnt); Tab(35); Using$("$###,###.##", TotPCPay); Tab(57); Using$("$##,###.##", TotPCEmpCont); Tab(78); Using$("$##,###.##", TotPCCityCont)
  Print #RptHandle, String$(87, "-")
  Print #RptHandle, "Job Class Totals: "
  Print #RptHandle,
  For t = 1 To 28
    If TJCCnt(t) > 0 Then
      ...
      Print #RptHandle, ThisJC; Tab(27); Using$("#,###", TJCCnt(t)); Tab(35); Using$("$###,###.##", TJCPay(t)); Tab(57); Using$("$##,###.##", TJCEmpCont(t)); Tab(78); Using$("$##,###.##", TJCCityCont(t))
    End If
  Next t
  Print #RptHandle, String$(87, "-")
  Print #RptHandle, "Job Class Grand Totals "; Tab(27); Using$("#,###", TotJCCnt); Tab(35); Using$("$###,###.##", TotJCPay); Tab(57); Using$("$##,###.##", TotJCEmpCont); Tab(78); Using$("$##,###.##", TotJCCityCont)
  Return
  
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmORBITMenu", "PrintText", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub cmdProcess_Click()
  If Not Exist(OrbitEmpData) Then
    MsgBox ("No employees are currently set up for ORBIT processing.")
    Exit Sub
  End If
  frmEmpORBITProcessing.Show
  DoEvents
  Unload Me
End Sub
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    ElseIf MsgBox("Are You Sure You Wish To Close Program?", vbYesNo, "Close?") = vbNo Then
      Cancel = True
    Else
      MainLog ("Payroll.exe terminated via menu bar on frmORBITMenu.")
      Call Terminate
      End
    End If
  End If
...
'  Next x
  ...
  For x = 1 To NumOfODRecs
    ...
    Print #RptHandle, LeftJustAlphaNumeric(ODRec.State, 2); RightJustNumeric(ODRec.Zip, 9, -1); LeftJustAlphaNumeric(ODRec.OutOfCntryAdd, 50);
    ...
    Print #RptHandle, ODRec.PayPrdBeginDate; ODRec.PayPrdEndDate; ODRec.IncDecSalary; RightJustNumeric(ODRec.Salary, 12, 0);
    Print #RptHandle, ODRec.IncDecEmpleeCntrb; RightJustNumeric(ODRec.EmployeeCntrb, 12, 0); LeftJustAlphaNumeric(ODRec.Adjustment, 10);
...
Deleted:
  ...
  Print #RptHandle, RightJustNumeric(TSalary, 12, 0); OTRec.IncDecTtlEmpContrb; RightJustNumeric(TempCont, 12, 0)
...
'      If ODRecd(ODCnt) = 0 Then Stop
'      ODRec.PayType = ODRec.PayType
'      ODRec.FirstName = ODRec.FirstName
'      ODRec.LastName = ODRec.LastName
    End If
...
Deleted1:
  ...
  If TotReg <> TotOReg Then
    Answer = MsgBox("The total retirement gross pay for ORBIT (" & QPTrim$(Using$("$###,###.##", TotOReg)) & ") and for stored pay transactions (" & Using$("$###,###.##", TotReg) & ") for the same pay period do not match. Continue anyway?", vbYesNo)
    If Answer = vbNo Then
      Close
      MsgBox ("ORBIT Export File creation aborted.")
      Exit Sub
    Else
      MainLog ("The total Orbit retirement gross and the stored transactions for the same period did not match and the user elected to continue anyway.")
    End If
  End If
  
  If TotOT <> TotOOT Then
    Answer = MsgBox("The total OT pay for ORBIT (" & QPTrim$(Using$("$###,###.##", TotOOT)) & ") and for stored pay transactions (" & Using$("$###,###.##", TotOT) & ") for the same pay period do not match. Continue anyway?", vbYesNo)
    If Answer = vbNo Then
      Close
      MsgBox ("ORBIT Export File creation aborted.")
      Exit Sub
    Else
      MainLog ("The total Orbit OT pay and the stored transactions for the same period did not match and the user elected to continue anyway.")
    End If
  End If
  If TotEmployeeMatch <> TotOEmployeeMatch Then
    Answer = MsgBox("The total employee match amount for ORBIT (" & QPTrim$(Using$("$###,###.##", TotOEmployeeMatch)) & ") and for stored pay transactions (" & Using$("$###,###.##", TotEmployeeMatch) & ") for the same pay period do not match. Continue anyway?", vbYesNo)
    If Answer = vbNo Then
      Close
      MsgBox ("ORBIT Export File creation aborted.")
      Exit Sub
    Else
      MainLog ("The total Orbit employee match amount and the stored transactions for the same period did not match and the user elected to continue anyway.")
    End If
  End If
  
  If TotEmployerMatch <> TotOEmployerMatch Then
    Answer = MsgBox("The total employer match amount for ORBIT (" & QPTrim$(Using$("$###,###.##", TotOEmployerMatch)) & ") and for stored pay transactions (" & Using$("$###,###.##", TotEmployerMatch) & ") for the same pay period do not match. Continue anyway?", vbYesNo)
    If Answer = vbNo Then
      Close
      MsgBox ("ORBIT Export File creation aborted.")
      Exit Sub
    Else
      MainLog ("The total Orbit employer match amount and the stored transactions for the same period did not match and the user elected to continue anyway.")
    End If
  End If
...
GetTransTotal:
  For y = 1 To ODCnt
    ...
    If ODRec.Deleted = True Then GoTo Deleted2
    Get EHandle, ODRecd(y), EmpRec
'    If EmpRec.EmpNo = 51008 Then Stop
