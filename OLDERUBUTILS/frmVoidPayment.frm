...
Private Sub VoidMisc()
  ...
  UBPaymentRec(1).AMTOWED = Val(fpAmtOwed)
  UBPaymentRec(1).TENDERTY = QPTrim$(fpTenderType.Caption)
  UBPaymentRec(1).CASHAMT = Val(fpCashAmt)
  UBPaymentRec(1).CHKAMT = Val(fpChkChgAmt)
  UBPaymentRec(1).AMTRECD = Val(fptotreceived)
  UBPaymentRec(1).CHANGE = Val(fpChange)
  UBPaymentRec(1).DESC = QPTrim(fpDesc)
  UBPaymentRec(1).TOTOWED = Val(fpAmtOwed)
  UBPaymentRec(1).AMTPAID = Val(fpTotAmt)
...
Private Sub VoidUBTrans()
  ...
  For cnt = 1 To 15
    ...
    If Revs(cnt - 1) < -100000# Then
      UBPaymentRec(1).PaidOwed(cnt).AMTPD1 = 0
    Else
      UBPaymentRec(1).PaidOwed(cnt).AMTPD1 = Val(Revs(cnt - 1))
    End If
    Else
      UBPaymentRec(1).PaidOwed(cnt).AMTPD1 = 0
    End If
...
' Close
  ...
  Print #RHandle,
  Print #RHandle, "Total Owed: "; Using("$##,###.##", -(Val(fpAmtOwed)))
  Print #RHandle, "Total Paid: "; Using("$##,###.##", -(Val(fptotreceived)))
  Print #RHandle, "Change Due: "; Using("$##,###.##", -(Val(fpChange)))
  ...
  For RevCnt = 1 To 15    ' NumOfRevs
    If Revs(RevCnt - 1).Visible = True Then
      If Val(Revs(RevCnt - 1)) <> 0 Then
        Print #RHandle, QPTrim$(fpDetDesc(RevCnt - 1).Caption) + "  "; Using("$#####.##", (-(Val(Revs(RevCnt - 1)))))
      End If
    End If
  Next
  Else
    ...
    If Revs(RevCnt - 1).Visible = True Then
      If Val(Revs(RevCnt - 1)) <> 0 Then
        Print #RHandle, Mid$(fpDetDesc(RevCnt - 1).Caption, 11, 20) + "  "; Using("$#####.##", (-(Val(Revs(RevCnt - 1)))))
      End If
    End If
    Next
