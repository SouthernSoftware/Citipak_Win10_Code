...
Begin VB.Form frmRptCutOff 
   ...
   Begin LpLib.fpCombo fpcboBalType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCutOff.frx":0C99
   End
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCutOff.frx":1068
   End
   Begin LpLib.fpCombo fpcboPrintOrder 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCutOff.frx":1442
   End
   Begin LpLib.fpCombo fpcboMetInf 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCutOff.frx":1811
   End
   Begin LpLib.fpCombo fpcboDeposit 
      Height          =   375
      Left            =   5550
      TabIndex        =   7
      Top             =   5970
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCutOff.frx":1BEB
   End
  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        UBLog "Closed via RptCutOff by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
Private Sub fpcboMetInf_KeyDown(KeyCode As Integer, Shift As Integer)
  ...
  End If
End Sub
Private Sub fpcboDeposit_KeyDown(KeyCode As Integer, Shift As Integer)
  If KeyCode = vbKeySpace Then
    fpcboDeposit.ListDown = True
  End If
  If fpcboDeposit.ListDown <> True Then
    If KeyCode = vbKeyDown Or KeyCode = vbKeyReturn Then
      fpcboRptType.SetFocus
      KeyCode = 0
    Else
...
Private Function ValidRoutes()
  If fptxtRoute1 <> "" And fptxtRoute2 <> "" Then
    If Val(fptxtRoute1) > Val(fptxtRoute2) Then
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpCustomerCutOff
  ...
  fpcboDeposit.AddItem "Yes"
  fpcboDeposit.AddItem "No"
  fpcboDeposit.ListIndex = 1
...
Private Sub CustCutOffListing()
  ...
  Dim ReportFile As String, DepP As Boolean, Deptotal As Double
  PageNo = 0
  ...
  End If
  Deptotal = 0
  If fpcboDeposit.ListIndex = 0 Then
    DepP = True
  Else
    DepP = False
  End If
    ...
    If CustCheck >= First And CustCheck <= Last Then
      If UBCustRec(1).CUTOFFYN = "Y" Then
          ...
          If CutOffBalance# >= MINAMT# Then
            If RealBalance# >= MINAMT# And CutOffBalance# > 0 Then
              GoSub PrintLine
            End If
          End If
        End If
...
DoCutOffRptFooter:
  ...
  If DepP = True Then
      Print #UBRpt, "      Total Deposit Amount: "; Using("#####.##", Deptotal#)
  End If
  Print #UBRpt,
  Print #UBRpt, Tab(10); "Report Parameters *****************"
  Print #UBRpt, Tab(10); LabelB1.Caption; ": "; First; LabelB2.Caption; ": "; Last
  Print #UBRpt, Tab(10); "Printing Order: "; fpcboPrintOrder.Text; " "; "     Meter Info: "; fpcboMetInf.Text; "   Deposit: "; fpcboDeposit.Text
...
PrintLine:
  ...
  If RptTyp = 2 Then
    ...
    If DepP Then
      If UBCustRec(1).DepositAmt > 0 Then
        Print #UBRpt, Tab(30); "Deposit "; Using("###.##", UBCustRec(1).DepositAmt);
        Deptotal = Round(Deptotal + UBCustRec(1).DepositAmt)
      Else
        Print #UBRpt, Tab(30); "Deposit 0";
      End If
    End If
  ...
  Else
    If DepP Then
      If UBCustRec(1).DepositAmt > 0 Then
        Print #UBRpt, Tab(50); "Deposit "; Using("###.##", UBCustRec(1).DepositAmt);
        Deptotal = Round(Deptotal + UBCustRec(1).DepositAmt)
      Else
        Print #UBRpt, Tab(50); "Deposit 0";
      End If
    End If
  ...
  For MrtCnt = 1 To 7
    If Len(QPTrim$(UBCustRec(1).LocMeters(MrtCnt).MTRType)) > 0 Then
      Print #UBRpt, QPTrim$(UBCustRec(1).LocMeters(MrtCnt).MtrNum);
...
Private Sub CustCutOffListing2()
  ...
  Dim ReportFile As String, DepP As Boolean, Dep As String, Deptotal As Double
  If fpcboMetInf.ListIndex = 0 Then
    NOMetInf = False
  Else
    NOMetInf = True
...
''  End If
      ...
      If UBCustRec(1).CUTOFFYN = "Y" Then
          ...
          If CutOffBalance# >= MINAMT# Then
            If RealBalance# >= MINAMT# And CutOffBalance# > 0 Then
              ...
              If DepP Then
                If UBCustRec(1).DepositAmt > 0 Then
                  Dep$ = "Deposit " & Using("###.##", UBCustRec(1).DepositAmt)
                  Deptotal = Round(Deptotal + UBCustRec(1).DepositAmt)
                Else
                  Dep$ = "Deposit 0"
                End If
              Else
                Dep$ = " "
              End If
              Print #UBRpt, ToPrint$ + "~" + ToPrintM$ + Dep$
              ToPrintM$ = ""
            End If
          End If
        End If
  ...
  If CustomerCnt > 0 Then
    Load frmLoadingRpt
    frmLoadingRpt.setwherefrom frmRptCutOff
    ARptCutOff.txtDate = Now
    ARptCutOff.txtTown = TOWNNAME$
    ARptCutOff.Title = "Customer Cut-Off Report."
    ARptCutOff.totCust = CustomerCnt
    ARptCutOff.LblDep = Using("##,###.##", Deptotal#)
    ARptCutOff.LblDep.Visible = True
    ARptCutOff.Label23.Visible = True
    ToPrintP$ = LabelB1.Caption + ": " + Str(First) + " " + LabelB2.Caption + ": " + Str(Last)
    ToPrintP$ = ToPrintP$ + "  Printing Order: " + fpcboPrintOrder.Text + " " + "     Meter Info: " + fpcboMetInf.Text + "  Deposit: " + fpcboDeposit.Text
