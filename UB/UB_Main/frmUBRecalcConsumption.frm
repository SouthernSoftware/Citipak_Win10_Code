...
Begin VB.Form frmUBRecalcConsumption 
   ...
   Begin VB.CheckBox Option1 
      Caption         =   "Remove Highest and Lowest Reads from Average."
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   12
         Charset         =   0
   ...
   Begin VB.Frame Frame1 
      ...
      Begin fpBtnAtlLibCtl.fpBtn cmdCancel 
         ...
         DropShadowColor =   0
         Redraw          =   -1  'True
         ButtonDesigner  =   "frmUBRecalcConsumption.frx":08CA
      End
      Begin fpBtnAtlLibCtl.fpBtn cmdOK 
         ...
         DropShadowColor =   0
         Redraw          =   -1  'True
         ButtonDesigner  =   "frmUBRecalcConsumption.frx":0AA3
      End
...
Dim Wadesboroflag As Boolean, wftemp As String
...
Private Sub cmdOk_Click()
  ...
  If Option1.Value = 1 Then
    If AvgCnt < 3 Or AvgCnt > 36 Then
      MsgBox "         Invalid Number of Months!         " & Chr$(13) & "         Range:  3 - 36", vbOKOnly
      fpNumMonths.Value = 3
    Else
      DeActivateControls Me
      DoEvents
      Call RecalcConsumption22
      DoEvents
      ActivateControls Me
      Call cmdCancel_Click
    End If
...
Private Sub RecalcConsumption()
  ...
  Dim DidCnt As Integer, TotU As Long
  Dim MCnt As Integer, MaxMeterAmt&
  ...
  For CCnt = 1 To NumOfCRecs
      ...
      If UBTranRec(1).TransType = TranUtilityBill Then
        For MCnt = 1 To 7
          If UBTranRec(1).CurRead(MCnt) > 0 Then
            TotU = 0
            TotU = (UBTranRec(1).CurRead(MCnt) - UBTranRec(1).PrevRead(MCnt))
            If TotU < 0 Then
              MaxMeterAmt& = 10& ^ (Len(Str$(UBTranRec(1).PrevRead(MCnt))) - 1)
              TotU = (MaxMeterAmt& - UBTranRec(1).PrevRead(MCnt)) + UBTranRec(1).CurRead(MCnt)
            End If
            TotalUse(MCnt) = TotalUse(MCnt) + TotU
    ...
    For MCnt = 1 To 7
      ...
      If Wadesboroflag Then
        wftemp$ = ""
        wftemp$ = Str(UBCustRec(1).LocMeters(MCnt).AvgUse)
        If Len(wftemp$) > 0 Then
          wftemp$ = Left$(wftemp$, (Len(wftemp$) - 1))
          wftemp$ = wftemp$ + "0"
          UBCustRec(1).LocMeters(MCnt).AvgUse = Val(wftemp$)
        End If
      End If
    Next
 ...
 ' Call cmdCancel_Click
End Sub
Private Sub RecalcConsumption22()
  frmBlinkTimer.Enabled = True
  
  DoEvents
  UBLog " IN: Recalc Average w/HL out Consumption"
  Dim doitonce As Boolean
  Dim UBCustRecLen As Integer
  Dim UBTranRecLen As Integer
  Dim UBFile As Integer
  Dim UBTran As Integer
  ...
  Dim CCnt As Long
  Dim LastTran As Long
  Dim DidCnt As Integer, MaxMeterAmt&
  Dim MCnt As Integer, TotU As Long
  ReDim UBCustRec(1) As NewUBCustRecType
  ReDim UBTranRec(1) As UBTransRecType
  ...
  UBLog "AVG: Recalc Using" + Str$(AvgCnt) + " Months, with Hi/Low dropped."
  FrmShowPctComp.Label1 = "Recalculating Consumption."
  FrmShowPctComp.cmdCancel.Enabled = False
  FrmShowPctComp.Show
  ...
  For CCnt = 1 To NumOfCRecs
    ...
    ReDim TotalUse(1 To 7) As Long
    ReDim UseCnt(1 To 7) As Integer
    ReDim HUse(1 To 7) As Long
    ReDim LUse(1 To 7) As Long
   ''' If CCnt = 9831 Then Stop
        ...
        For MCnt = 1 To 7
          If UBTranRec(1).CurRead(MCnt) > 0 Then
            ...
            If TotU < 0 Then
              ...
              TotU = (MaxMeterAmt& - UBTranRec(1).PrevRead(MCnt)) + UBTranRec(1).CurRead(MCnt)
            End If
            If HUse(MCnt) < TotU Then
              HUse(MCnt) = TotU
            End If
            If LUse(MCnt) > TotU Then
              LUse(MCnt) = TotU
            End If
            TotalUse(MCnt) = TotalUse(MCnt) + TotU
            UseCnt(MCnt) = UseCnt(MCnt) + 1
    ...
    For MCnt = 1 To 7
      If TotalUse(MCnt) > 0 Then
        If TotalUse(MCnt) > (HUse(MCnt) + LUse(MCnt)) Then
          TotalUse(MCnt) = TotalUse(MCnt) - (HUse(MCnt) + LUse(MCnt))
        End If
        If UseCnt(MCnt) > 2 Then
          UseCnt(MCnt) = UseCnt(MCnt) - 2
        End If
        If TotalUse(MCnt) > 0 Then
          UBCustRec(1).LocMeters(MCnt).AvgUse = TotalUse(MCnt) / UseCnt(MCnt)
          UBCustRec(1).LocMeters(MCnt).UseCnt = UseCnt(MCnt)
  ...
  Close
  Erase UBCustRec, UBTranRec
  UBLog "AVG: Recalc Average w/HL out Use Complete."
  DoEvents
  Call UPDateOK
  DoEvents
  UBLog "OUT: Recalc Average w/HL out Consumption" + CrLf$
 ' Call cmdCancel_Click
End Sub
