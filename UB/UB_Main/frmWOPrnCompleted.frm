...
Begin VB.Form frmWOListPrint 
   ...
   Begin LpLib.fpCombo fpCombo1 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmWOPrnCompleted.frx":0BED
   End
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmWOPrnCompleted.frx":0EE4
   End
   Begin LpLib.fpCombo fpcboWOStatus 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmWOPrnCompleted.frx":1212
   End
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpPrinWorkOrder
End Sub
...
Private Sub PrintWOList()
  ...
  Dim Rem5 As String, Rem6 As String, ToPrint As String, nexttr As Long
  ...
  Dim WorkDate As Integer, NumOfWRecs As Long, cntW As Long
...
skipthis:
  ...
  NumOfWRecs = FileSize(UBPath$ + "UBWRKORD.DAT") \ WorkOrderRecLen
    ...
    For lcnt& = 1 To NumOfRecs
      ...
      If Book >= BegRoute And Book <= EndRoute Then
        If UBCustRec(1).WOLastTrans > 0 Then
          nexttr = UBCustRec(1).WOLastTrans
          Do While nexttr > 0
            Get #UBWOFile, nexttr, WorkOrderRec(1)
            cntW = nexttr
            If WOStatus = 0 Then
            ...
            ElseIf WOStatus = 2 Then
              GoSub PrintThemOne
            End If
            nexttr = WorkOrderRec(1).PrevTransRec
          Loop
        End If
      End If
  ...
  If WorkDate >= FromDate And WorkDate <= ThruDate Then
    If graphicflag = False Then
      ...
      Else
        Print #RptHandle,
      End If
      Print #RptHandle, QPTrim$(UBCustRec(1).ServAddr); Tab(50); cntW; Tab(65); Num2Date$(WorkOrderRec(1).ENTRYDATE)
    ...
    Else
      ...
      ToPrint$ = ToPrint$ + "~" + QPTrim$(UBCustRec(1).ServAddr) + "~" + Str(cntW) + "~" + Num2Date$(WorkOrderRec(1).ENTRYDATE)
