...
Begin VB.Form frmRptCustListAU 
   BackColor       =   &H008F8265&
   ...
   Icon            =   "frmRptCustListAU.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   Moveable        =   0   'False
   ...
   Begin LpLib.fpCombo fpcboCustStatus 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCustListAU.frx":08CA
   End
   Begin LpLib.fpCombo fpCombo1 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCustListAU.frx":0BED
   End
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptCustListAU.frx":0EE4
   End
   Begin VB.CheckBox chkUsage 
      Alignment       =   1  'Right Justify
      BackColor       =   &H008F8265&
      Caption         =   "Include Average Usage:"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmRptCustListAU"
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpQuickCustList
End Sub
  ...
  For cnt = 1 To NumOfRecs
    ...
    If UBCustRec(1).DelFlag = 0 Then
      ...
      If UsingRead Then
        ...
        ToPrint$ = ToPrint$ + "~" + Using$("#########", UBCustRec(1).Seq)
      Else
        ToPrint$ = ToPrint$ + "~" + QPTrim$(Left$(UBCustRec(1).ServAddr, 25))
        ToPrint$ = ToPrint$ + "~" + UBCustRec(1).Status
        ToPrint$ = ToPrint$ + "~" + QPTrim$(UBCustRec(1).ZONE)
      End If
      CustCnt = CustCnt + 1
      Print #UBRpt, ToPrint$
      ToPrint$ = ""
...
Private Sub mnuPrnScn_Click()
  PrintForm
End Sub
Public Sub UBQuickCustList22(Parent As Form)
  Dim IdxName As String, ToPrint As String, NumOfRecs As Long
  Dim Title As String, ReportFile As String, RStatus As String
  Dim UBCustRecLen As Integer, IdxRecLen As Integer, UseStatus As Boolean
  Dim IdxFileSize As Long, cnt As Long, UsingAcct As Boolean
  ...
  For cnt = 1 To NumOfRecs
    ...
    Next
      ...
      If UsingRead Then
        ...
        End If
        ToPrint$ = ToPrint$ + "~" + UBCustRec(1).Status
        If chkUsage.Value = ValueTrue Then
          'ToPrint$ = ToPrint$ + " - " + Using$("########", UBCustRec(1).AvgUse)
        End If
        ToPrint$ = ToPrint$ + "~" + Using$("#########", UBCustRec(1).Seq)
      Else
        ToPrint$ = ToPrint$ + "~" + QPTrim$(Left$(UBCustRec(1).ServAddr, 25))
...
SkipEm:
  ...
  If CustCnt > 0 Then
    Load frmLoadingRpt
    frmLoadingRpt.setwherefrom
    ARptMtrInstDate.txtDate = Now
    ARptMtrInstDate.txtTown = TOWNNAME$
    ARptMtrInstDate.Title = "Customer Average Consumption Report."
    ARptMtrInstDate.Label7.Caption = "Average"
