...
If txtAdmin.Text = "INEEDTransactions" Then
    txtAdmin.Text = ""
...
Private Sub ExpCustStuff()
  ...
  If InStr(TOWNNAME$, "DEEP RUN") Or InStr(TOWNNAME$, "CLINTWOOD") > 0 Then
  'If InStr(TOWNNAME$, "DEEP RUN") > 0 Then
    DoPBillFlag = True
'    Exit Sub
  End If
...
'meters
  ...
  If DoPBillFlag Then
    'UBCustRec(1).SEARCH
    If UBCustRec(1).PrnBillYN = "N" Then
      Print #UBRpt, qcq$; "N";
    Else
      Print #UBRpt, qcq$; "Y";
    End If
...
Private Sub optgroup_Click()
  ...
  End If
End Sub
Private Sub TransNExpCustStuff()
  Dim Dash80 As String, IndexName As String, IdxRecLen As Integer
  Dim UBCustRecLen As Integer, UBCust As Integer, UBTran As Integer
  Dim IdxFileSize As Long, IdxNumOfRecs As Long, cnt As Long
  Dim UBTranRecLen As Integer, NumOfRecs As Long, NumOfCust As Long
  ...
  Dim WhatBook As Integer, Export As Long, RCnt As Integer, FCnt As Integer
  Dim MCnt As Integer, tempTot As Double, MessageRec As Integer
  Dim ThisFile As String, Today As String, UBTrans As Integer, TransDesc$
  Dim Ext As String, GCode As String, UBSetupreclen As Integer, Amount#
  Dim Txt As String, ChkName As String, Trans&, EstCnt As Integer
  Dim MaxRevenue As Integer, UBTransRecLen As Long, FromDate As Integer, ThruDate As Integer
...
'  Dim GrpCodeRecLen As Integer, ghandle As Integer
  ...
  Else
    ...
    ThruDate = 99999
  End If
  FrmShowPctComp.Label1 = "Creating Export Files  2"
  FrmShowPctComp.cmdCancel.Enabled = False
  FrmShowPctComp.Show , Me
  q$ = ""
  qc$ = ""
...
'  Open UBPath$ + "UBCUST.DAT" For Random Shared As UBCust Len = UBCustRecLen
 ...
 NumOfRecs = FileSize(UBPath$ + "UBTRANS.DAT") \ UBTransRecLen
    ThisFile$ = "UBExTrans.ASC"
    KillFile (UBPath$ + ThisFile$)
...
'  End If
  ...
  For Trans& = 1 To NumOfRecs
    FrmShowPctComp.ShowPctComp Trans&, NumOfRecs
    If FrmShowPctComp.Out Then
      Close
      Unload FrmShowPctComp
      GoTo ExitMastCustListing
    ...
    If UBTransRec(1).TransDate >= FromDate And UBTransRec(1).TransDate <= ThruDate Then
'    AcctNumber = IdxBuff(cnt).RecNum
'    Get UBCust, AcctNumber, UBCustRec(1)
...
'  Do While Trans& <> 0
     ...
     ' If UBTransRec(1).CustAcctNo = AcctNumber Then
        GoSub DefineType
        Print #UBRpt, q$; QPTrim$(Str$(UBTransRec(1).CustAcctNo));
'        If chkLocation.Value = 1 Then Print #UBRpt, qcq$; LocationNumber$;
'        If chkCustName.Value = 1 Then Print #UBRpt, qcq$; QPTrim$(UBCustRec(1).CustName);
'        If chkCurrBal.Value = 1 Then Print #UBRpt, qcq$; Using(BuckFmt$, UBCustRec(1).CurrBalance);
'        If chkPrevBal.Value = 1 Then Print #UBRpt, qcq$; Using(BuckFmt$, UBCustRec(1).PrevBalance);
...
'        End If
'        If chkDeposit.Value = 1 Then Print #UBRpt, qcq$; Using(BuckFmt$, UBCustRec(1).DepositAmt);
        Print #UBRpt, qcq$; Str(Trans&);
        Print #UBRpt, qcq$; Num2Date$(UBTransRec(1).TransDate);
        Print #UBRpt, qcq$; TransDesc$;
        Print #UBRpt, qcq$; Using("$###,###.##", Amount#);
'        For RCnt = 1 To MaxRevenue Step 3
'          Print #UBRpt, qcq$; Using("#####.##", Round#(UBTransRec(1).RevAmt(RCnt) + UBTransRec(1).TaxAmt(RCnt)));
'        Next RCnt
        Print #UBRpt, q$
 '   End If
...
'    Next
  ...
  Case 10, 110
    ...
    Amount# = UBTransRec(1).Transamt
  Case 11, 111
    TransDesc$ = "TranUpwardAdjustment" 'QPTrim$(UBTransRec(1).BillMsg)"
    Amount# = UBTransRec(1).Transamt
  Case 12, 112
    TransDesc$ = "TranDownwardAdjustment" 'QPTrim$(UBTransRec(1).BillMsg)
    Amount# = UBTransRec(1).Transamt
  Case 33
    TransDesc$ = "TranOverPayAdjustment" 'QPTrim$(UBTransRec(1).BillMsg)
    Amount# = UBTransRec(1).Transamt
  Case 37
    TransDesc$ = "TranDepCreditRemoval" 'QPTrim$(UBTransRec(1).BillMsg)
    Amount# = UBTransRec(1).Transamt
  Case 39
    TransDesc$ = "TranDepPaymentVoid" 'QPTrim$(UBTransRec(1).BillMsg)
    Amount# = UBTransRec(1).Transamt
  Case 99
...
'  If chkDeposit.Value = 1 Then Print #UBRpt, qcq$; "Deposit";
  Print #UBRpt, qcq$; "Rec";
  Print #UBRpt, qcq$; "Date";
  Print #UBRpt, qcq$; "TransType";
  Print #UBRpt, qcq$; "TransAmt";
'  For RCnt = 1 To NumofRevs
'      Print #UBRpt, qcq$; "TransRevAmts";
'    Next
  Print #UBRpt, q$
