...
Begin VB.Form frmUBHarryImpExpHHRead 
   BackColor       =   &H008F8265&
   ...
   ClientTop       =   2175
   ClientWidth     =   12210
   Icon            =   "frmUBHarryImpExpHHRead.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   Moveable        =   0   'False
   ...
   Begin LpLib.fpCombo fpMtrImpExpFlag 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmUBHarryImpExpHHRead.frx":08CA
   End
   Begin LpLib.fpCombo fpMtrType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmUBHarryImpExpHHRead.frx":0C30
   End
   ...
   End
End
Attribute VB_Name = "frmUBHarryImpExpHHRead"
...
'
'THIS FORM IS COPY OF ORIGINAL HHIMPEXP WITH LARGER ARRAY FOR BOOKS/CYCLES
'ENTERED AT ONCE -- FOR HARRISBURG  INCREASED FROM 30 to 50  - PS 7/14/2009
'
...
Private Sub Form_Load()
  ...
  If InStr(TOWNNAME$, "APPALACHIA") Or InStr(TOWNNAME$, "SHENANDOAH") Then
    applachflag = True
  Else
    applachflag = False
  End If
  ...
  Else
    schlnglat = False
  End If
  If InStr(TOWNNAME$, "MIDDLETOWN") Or InStr(TOWNNAME$, "BISCOE") Or InStr(TOWNNAME$, "SUMMERTON") Or InStr(TOWNNAME$, "GILBERT SUMMIT") Then
    Middle = True
  Else
    Middle = False
  End If
  ...
  Else
    SaludaCoF = False
  End If
  If InStr(TOWNNAME$, "PERQUIMANS") Or InStr(TOWNNAME$, "WADESBORO") Or InStr(TOWNNAME$, "GILEAD") Then
    Perquim = True
  Else
    Perquim = False
  End If
...
BadSkip:
  ...
  If rptopt = 1 Then
    'do the graphics
    Load frmLoadingRpt
    frmLoadingRpt.setwherefrom frmUBHarryImpExpHHRead
...
SchlumExtractRecord:
  ...
  Select Case UCode1
  Case 0
    If Middle Then   'this is for middleton and Bicoe and Summerton
      Multi# = 1
    End If
  Case 1
    Multi# = 10
  ...
  If schlnglat = True Then
    UBCustRec(1).LocMeters(MeterSlot).MtrLat = Val(Mid$(QPTrim$(UBSchlumHHRec(1).Notes4), 7, (Len(QPTrim$(UBSchlumHHRec(1).Notes4)) - 6)))
    UBCustRec(1).LocMeters(MeterSlot).MtrLng = Val(Mid$(QPTrim$(UBSchlumHHRec(1).Notes5), 8, (Len(QPTrim$(UBSchlumHHRec(1).Notes5)) - 7)))
  End If
...
End Sub
  Private Sub ImpExpRadioReadHHInfo(ByVal ImpExpFlag As Boolean)
    ...
    Dim Chkbook As Boolean, RType As String, MPIDNO As String
      '''' C:\mvrs50\xfer
