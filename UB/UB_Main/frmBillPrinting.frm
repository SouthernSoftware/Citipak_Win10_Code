...
Dim totTax As Double, curtax As Double, pretax As Double
  Dim UBSetupreclen As Integer, cnt As Long
  ...
  Dim FntSize As Integer, ToPrint2 As String, endit As Boolean, MsgORdates As Integer
  ...
  Dim Zip As String, ZDigit As String, FoundAMtr As Boolean, Zero As String, PZip As String
  ...
  Dim Today As String, BillOutRecLen As Integer, Ext As String, UBBill21Linelen As Integer
...
Dim Bucksport As Boolean, DFoundMtr As Boolean, Brooknl As Boolean, Middletown As Boolean
Dim DCurrRead As Long, DPrevRead As Long, DUsageAmt As Long, CedarBlf As Boolean
Dim Bladenflag As Boolean, Bladen As Boolean, CBeach As Boolean, Cthg As Boolean, HamletF As Boolean
Dim Wrights As Boolean, Summerton As Boolean, Celinaflag As Boolean, MontrossFlag As Boolean
Dim TaxAmt(1 To 14) As Double
Dim prevtax As Double
Dim blnAllowEBill As Boolean
...
Public Sub PBillPrn(CustNum1 As Long, TNum1 As Long, O1 As String, M11 As String, M21 As String, M31 As String, M41 As String, Due As String)
  ...
  For cnt = 1 To 14 'MaxRevsCnt
    TaxAmt(cnt) = Round(UBSetUp(1).Revenues(cnt).TAXRATE)
  Next
  ...
  If InStr(TOWNNAME$, "NORTHWEST CLAY") Or InStr(TOWNNAME$, "RYAN") Then
    PenTaxFlag = True
  Else
    PenTaxFlag = False
  End If
  ...
  Else
    TennRdg = False
  End If
  If InStr(TOWNNAME$, "BROOKNEAL") Then
    Brooknl = True
  Else
    Brooknl = False
  End If
  If InStr(TOWNNAME$, "CEDAR BLUFF") Then
    CedarBlf = True
  Else
    CedarBlf = False
  End If
  If InStr(TOWNNAME$, "BLADEN") Then
    Bladenflag = True
  Else
    Bladenflag = False
  End If
  ...
  Else
    CBeach = False
  End If
  If InStr(TOWNNAME$, "CARTHAGE") Then
    Cthg = True
  Else
    Cthg = False
  End If
  If InStr(TOWNNAME$, "SUMMERTON") Then
    Summerton = True
  Else
    Summerton = False
  End If
 If InStr(TOWNNAME$, "WRIGHTSVILLE") Then
    Wrights = True
  Else
    Wrights = False
  End If
  If InStr(TOWNNAME$, "MONTROSS") Then
    MontrossFlag = True
  Else
    MontrossFlag = False
  End If
  ...
  M3 = M31
  M4 = M41
  If BLType = 1 Or BLType = 2 Or BLType = 9 Or BLType = 10 Or _
     BLType = 11 Or BLType = 13 Or BLType = 16 Or BLType = 17 Or _
     BLType = 19 Or BLType = 20 Then
     PrintUtilBills True
  Else
...
Private Sub cmdPrint_Click()
  ...
  Dim FntSize As Integer, cntd As Integer
    cntd = 0
   If Not ValidDate Then Exit Sub
    Today = Format(Now, "mm/dd/yyyy")
    chkthedate = Date2Num(Today)
    entdate = Date2Num(fpBillingDate)
    If entdate > 0 Then If entdate > (chkthedate + 30) Or entdate < (chkthedate - 30) Then cntd = cntd + 1
    entdate = Date2Num(fpDueDate)
    If entdate > 0 Then If entdate > (chkthedate + 30) Or entdate < (chkthedate - 30) Then cntd = cntd + 1
    entdate = Date2Num(fpPastDueDate)
    If entdate > 0 Then If entdate > (chkthedate + 30) Or entdate < (chkthedate - 30) Then cntd = cntd + 1
    entdate = Date2Num(fpCurrDate)
    If entdate > 0 Then If entdate > (chkthedate + 30) Or entdate < (chkthedate - 30) Then cntd = cntd + 1
    entdate = Date2Num(fpPrevDate)
    If entdate > 0 Then If entdate > (chkthedate + 30) Or entdate < (chkthedate - 30) Then cntd = cntd + 1
    entdate = Date2Num(fpDraftDate)
    If entdate > 0 Then If entdate > (chkthedate + 30) Or entdate < (chkthedate - 30) Then cntd = cntd + 1
    entdate = Date2Num(fpPastDate2)
    If entdate > 0 Then If entdate > (chkthedate + 30) Or entdate < (chkthedate - 30) Then cntd = cntd + 1
...
Private Sub Form_Load()
 ...
 ' ReDim TaxAmt(1 To 15) As Double
  ...
  If UBBillSetup(1).PSNBills = "Y" Then
    blnAllowEBill = True
  Else
    blnAllowEBill = False
  End If
...
'at same time get the OkiMode 1 is not ibm, 2 is ibm
  ...
  If UBSetUp(1).BANKDFT = "Y" Then
    UseDraftFlag = True
  End If
'removed the fflag = false with the bankdft setting above and rem out the fflag If statement below to allow draft on final
'Did this on 6/30/11 PS for Hamlet
'  If Fflag = True Then
'    DidADraftFlag = False
'    UseDraftFlag = False
'    fpDraftDate.Enabled = False
...
'  End If
  ...
  Else
    Cthg = False
  End If
  If InStr(TOWNNAME$, "HAMLET") Then
    HamletF = True
  Else
    HamletF = False
  End If
  If InStr(TOWNNAME$, "MIDDLETOWN") Then
    Middletown = True
  Else
    Middletown = False
  End If
...
'    Case 2:
    ...
    Case 21:
      ...
      Labelm3.Visible = False
      Labelm4.Visible = False
      If Exist(UBPath$ + "UBBil21L.DAT") Then
        ReDim UBBilltxt(1) As UBBill21LineType
        UBBill21Linelen = Len(UBBilltxt(1))
        LoadUBBill21LineFile UBBilltxt(), UBBill21Linelen
        If QPTrim$(UBBilltxt(1).MsgORdates) <> "M" Then
          fptxtMessage(0).Enabled = False
        End If
      End If
    Case 97, 98:
...
End If
If BLType = 21 Then
    ReDim UBbill21line(1) As UBBill21LineType
    UBBill21Linelen = Len(UBbill21line(1))
    LoadUBBill21LineFile UBbill21line(), UBBill21Linelen
    If UBbill21line(1).MsgORdates = "M" Then
      MsgORdates = 0
    Else
      MsgORdates = 1
    End If
...
If BLType = 98 Then
  ...
  Next
End If
If BLType = 97 Then  'this is for new company Holly Springs uses
  If Fflag Then
    scs$ = "F"
  ElseIf BFlag Then
    scs$ = "B"
  ...
  Fmt15$ = "############.##"
  Today$ = Date$
  ReDim PrintRecr(1) As BillOutRec2Type
  BillOutRecLen = Len(PrintRecr(1))
  Ext$ = ".BTF"
  SCSFileName$ = Left$(Today$, 2) + Mid$(Today$, 4, 2) + Right$(Today$, 2) + scs$
  For cnt = 1 To 9
    ChkName$ = SCSFileName$ + QPTrim$(Str$(cnt)) + Ext$
...
If BLType = 99 Then  'DW1
  ...
  Fmt15$ = "############.##"
  Today$ = Date$
  ReDim PrintRecM(1) As BillOutRec3Type
  BillOutRecLen = Len(PrintRecM(1))
  Ext$ = ".BTF"
  SCSFileName$ = Left$(Today$, 2) + Mid$(Today$, 4, 2) + Right$(Today$, 2) + scs$
  For cnt = 1 To 9
    ChkName$ = SCSFileName$ + QPTrim$(Str$(cnt)) + Ext$
...
'      End If
  ...
  UBRpt = FreeFile
  If Not BLType = 98 And Not BLType = 97 And Not BLType = 99 Then
  ...
  'IF DaysINRead > 59 THEN STOP
            ...
            For TaxCnt = 1 To 14 'MaxRevsCnt
              ...
              prevtax# = Round(prevtax# + (UBCustRec(1).CurrRevAmts(TaxCnt) * TaxAmt(TaxCnt)))
            Next
  ...
  'NOTE: Maccelsfield bill has best code to parse out meter readings
            ...
            End If
'06/04/19 Deep Run
'            If Not UCase$(UBCustRec(1).PrnBillYN) = "N" Then
...
'121598 Added bill to owner
      ...
      For TaxCnt = 1 To 14 'MaxRevsCnt
        TotalTax# = Round(TotalTax# + UBBillRec(1).TaxAmt(TaxCnt))
        prevtax# = Round(prevtax# + Round(UBCustRec(1).CurrRevAmts(TaxCnt) * TaxAmt(TaxCnt)))
      Next
...
If PostedFlag Then
  ...
  'was originally printed it is only temporary.
    ...
    For TaxCnt = 1 To 14 'MaxRevsCnt
      ...
      prevtax# = Round(prevtax# + UBCustRec(1).CurrRevAmts(TaxCnt) * TaxAmt(TaxCnt))
    Next
    If prevtax# > TotalTax# Then prevtax# = prevtax# - TotalTax#
''*********************************
    If O = "O" Then
      UBOwn = FreeFile
      Open UBPath$ + "UBOWNER.DAT" For Random Shared As UBOwn Len = UBOwnerRecLen
...
End If 'End BFlag Bills
  ...
  Case 1:
    ...
    If InStr(UCase$(UBBillSetup(1).BL1Head1), "ASHLEY") > 0 Then
      ARptBillLaserLegal.Label9 = "DUE DATE"
    End If
...
'If SpenTn Then Stop
  ...
  Case 3, 4, 5, 6, 7, 8, 12, 14, 15, 18, 21:
...
'      ViewPrint ReportFile$, "Utility Bill Printing"
  Case 16:
    ...
    End If
    If CBeach Then
      ARptBillLaserLetterForm.Field83.Visible = False
      ARptBillLaserLetterForm.Field151.Visible = False
      'changed the following line for zip and dp code for Carolina Beach
      'ARptBillLaserLetterForm.Barcode3.DataField = "28258-040202"
      'to the following line on 12/10/08  - PS  Then took out same day  they are using envelopes with bar code preprinted
    '  ARptBillLaserLetterForm.Barcode3.DataField = "28428-413021"
    '  ARptBillLaserLetterForm.Barcode3.Visible = True
    End If
    If PostBar = False Then
      ARptBillLaserLetterForm.Barcode1.Visible = False
    End If
  ...
  Case 17:
    ...
    If Rteflag Then ARptBillLaserLegal3F.FieldPostalRte.Visible = True
...
'BillTypes
   Select Case BLType
    Case 1, 10, 11, 17, 20: 'Laser 3bill legal 1)full format and 10)preprinted stock and 11)F,17)3F
      If Wrights = True Then
        PrnLaserLegalWR UBCustRec(), UBBillRec(), UBSetUpRec(), True
      Else
        PrnLaserLegal UBCustRec(), UBBillRec(), UBSetUpRec()
      End If
    Case 2, 13:
      GoSub PrnLaser '3-bill letter SIZE format
    Case 3:
      If ElkFlag Or InStr(TOWNNAME$, "LAWRENCEVILLE") Then
...
'      ElseIf Bladen = True Then
'        PrnBladenBill UBCustRec(), UBBillRec(), UBSetUpRec()
      Else
        GoSub PrintNewStandV1
      End If
    Case 5:
      If CedarBlf Then
        CedarBill UBCustRec(), UBBillRec(), UBSetUpRec()
      Else
    ...
    Case 21:
       Prn21LineTxtPlainP UBCustRec(), UBBillRec(), UBSetUpRec(), UBbill21line()
    Case 97:
       CreateSCSFileTransfer2 UBCustRec(), UBBillRec(), UBSetUpRec()
    Case 98:
       CreateSCSFileTransfer UBCustRec(), UBBillRec(), UBSetUpRec()
    Case 99:
       CreateSCSFileTransfer3 UBCustRec(), UBBillRec(), UBSetUpRec()
    Case Else
   End Select
...
'MUST SHOW BOTH METERS OR, TOTAL CONSUMPTION ON THIS BILL
    ...
    If Not LPIFlag Then
      LPIFlag = -2
      If InStr(TOWNNAME$, "PEACHLAND") Or Summerton = True Then
...
'Stop
    If Totalamt# > 0 And Not FinalFlag Then
      If UBCustRec(1).LATEFEE = "Y" Then
        If TotalTax > 0 Then
          CalcPenamt Round(UBBillRec(1).Transamt - TotalTax), Round(Previous - prevtax), Round(Totalamt - (TotalTax + prevtax))
        Else
...
'))))))))))))))))))))))))))
    ...
    Print #UBRpt, Tab(22); Left$(UBCustRec(1).CITY, 14); " "; UBCustRec(1).STATE; " "; PZip$ 'UBCustRec(1).ZIPCODE
    Print #UBRpt, " "
...
PrintNewStandV1: 'use billing date, due and pastdue
    If InStr(TOWNNAME$, "RICH CREEK") Or InStr(TOWNNAME$, "PENNINGTON") Then
      Print #UBRpt, Chr$(27); Chr$(48); Chr$(27); Chr$(77);
    Else
      If Not LPIFlag Then
        LPIFlag = -2
...
'    End If
      ...
      If UBCustRec(1).LATEFEE = "Y" Then
        If TennRdg Then
          ...
          Else
            CalcPenamt UBBillRec(1).Transamt, Previous, Totalamt
          End If
        ElseIf SpenTn Or Celinaflag Then
            CalcPenamt UBBillRec(1).Transamt, Previous, Totalamt
        Else
          If TotalTax > 0 Then
            CalcPenamt Round(UBBillRec(1).Transamt - TotalTax), Previous, Round(Totalamt - (TotalTax))
    ...
    Print #UBRpt, " "
    If Not BFlag Then
      Print #UBRpt, Tab(50); DueDate$; Tab(61); Using("########.##", Totalamt#)
    ...
    If Not Totalamt# > 0 Then
      Print #UBRpt, Tab(61); Using("########.##", Round#(Totalamt#))
    Else
      Print #UBRpt, Tab(61); Using("########.##", Round#(Totalamt# + CustPenalty#))
    End If
...
'          Print #UBRpt, Tab(28); Using("#######", UsageAmt&);
        ...
        Case 3
          If EFoundMtr Then
            If Not BFlag Then
              Print #UBRpt, Tab(5); Using("##########", ECurrRead&);
              Print #UBRpt, Tab(15); Using("##########", EPrevRead&);
              Print #UBRpt, Tab(26); Using("#######", EUsageAmt&);
            End If
         End If
        End Select
'       End If
...
'))))))))))))))))))))))))))
      ...
      Case 5
        If BLType = 4 Then Print #UBRpt, Tab(46); Left$(UBCustRec(1).CustName, 29)
        If BLType = 18 Then Print #UBRpt, Tab(47); Left$(UBCustRec(1).CustName, 29)
      Case 6
        If BLType = 4 Then Print #UBRpt, Tab(46); QPTrim$(UBCustRec(1).ADDR1)
        If BLType = 18 Then Print #UBRpt, Tab(47); QPTrim$(UBCustRec(1).ADDR1)
      Case 7
        If BLType = 4 Then Print #UBRpt, Tab(46); QPTrim$(UBCustRec(1).ADDR2)
        If BLType = 18 Then Print #UBRpt, Tab(47); QPTrim$(UBCustRec(1).ADDR2)
      Case 8
        If BLType = 4 Then Print #UBRpt, Tab(46); Left$(UBCustRec(1).CITY, 17); " "; QPTrim$(UBCustRec(1).STATE); " "; Left$(UBCustRec(1).ZIPCODE, 5)
        If BLType = 18 Then Print #UBRpt, Tab(47); Left$(UBCustRec(1).CITY, 14); " "; QPTrim$(UBCustRec(1).STATE); " "; Left$(UBCustRec(1).ZIPCODE, 5)
      Case Else
        Print #UBRpt, " "
      End Select
    Next
...
'      Print #UBRpt, " "
    End If
     If Previous# <> 0 Then
      If Cthg = True Then
        Print #UBRpt, "                  Past Due:  "; Tab(31); Using("$##,###,###.##", Previous#)
      Else
        Print #UBRpt, "                  Previous:  "; Tab(31); Using("$##,###,###.##", Previous#)
      End If
'    Else
'      Print #UBRpt, " "
    End If
    Print #UBRpt, "                   Current:  "; Tab(31); Using("$##,###,###.##", UBBillRec(1).Transamt)
    Print #UBRpt, Tab(32); "--------------"
'line 24
    If FinalFlag And CDeposit# Then
...
'      Print #UBRpt, " "
    ...
    Else
      Print #UBRpt, "                     Total:  "; Tab(31); Using("$##,###,###.##", Totalamt#)
    End If
    If Not BLType = 18 Then
      Print #UBRpt, " "
    End If
    If Bladenflag Then
       Print #UBRpt, Tab(3); Msg1$
    Else
...
PrintNewStandRmStamp: 'use billingdate, due and pastdue
    ...
    AcctNum = UBBillRec(1).CustAcctNo
    Previous# = Round#(UBCustRec(1).PrevBalance + UBCustRec(1).CurrBalance)
    pretax# = Round(pretax# + curtax#)
    Previous# = Round(Previous# - pretax#)
    Totalamt# = Round#(Previous# + UBBillRec(1).Transamt)
    If FinalFlag And CDeposit# Then
      Totalamt# = Round#(Totalamt# - UBCustRec(1).DepositAmt)
    End If
      ...
      If UBBillRec(1).RevAmt(WRevCnt) <> 0 Then
        ...
        If UBBillRec(1).CurRead(WRevCnt) > 0 Then
          ...
          If UsageAmt& < 0 Then
            ...
            UsageAmt& = (MaxMeterAmt& - UBBillRec(1).PrevRead(WRevCnt)) + UBBillRec(1).CurRead(WRevCnt)
          End If
          If Bladen Then UsageAmt& = Round(UsageAmt& * UBCustRec(1).LocMeters(WRevCnt).MTRMulti)
...
'      Print #UBRpt, " "
    ...
    Print #UBRpt,
    Print #UBRpt, Tab(8); CustAcctNo&; Tab(20); Using("######.##", Totalamt#);
    If BLType = 6 Then
      Print #UBRpt, Tab(38); CustAcctNo&; Tab(50); Using("######.##", Totalamt#)
    Else
      If Totalamt# > 0 Then
        Print #UBRpt, Tab(35); CustAcctNo&; Tab(41); Using("######.##", Totalamt#); Tab(51); Using("######.##", Totalamt# + CustPenalty#)
      Else
        Print #UBRpt, Tab(35); CustAcctNo&; Tab(41); Using("######.##", Totalamt#)
      End If
    End If
...
'Bill 14 has penalty, also brookneal special no pen on tax amts
...
'    Else
    ...
    Else
      If Brooknl = True Then
        Print #UBRpt, Tab(37); CustAcctNo&; Tab(50); Using("######.##", Totalamt# + CustPenalty#)
      Else
...
'this is same for all since meter types all same
    For mChk = 1 To 7
      If UBBillRec(1).MtrTypes(mChk) > 0 Then
          ...
          End If
          If Not Wrights Then SUsageAmt&(mChk) = Round(SUsageAmt&(mChk) * UBCustRec(1).LocMeters(mChk).MTRMulti)
          Select Case UBBillRec(1).MtrTypes(mChk)
          Case 1
            SMtrType(mChk) = "W"
          Case 2
...
'    End If
    ...
    Print #UBRpt, " "
    Print #UBRpt, Using("##########", UBBillRec(1).CustAcctNo);
    Print #UBRpt, Tab(15); Left$(QPTrim(UBCustRec(1).ServAddr), 31); Tab(48); Using("########", UBBillRec(1).CustAcctNo);
    Print #UBRpt, Tab(62); Num2Date$(UBBillRec(1).BillDate)
    Print #UBRpt, " "

    Print #UBRpt, Tab(50); DueDate$; Tab(64); Using("#####.##", Totalamt#)
    If Not BFlag Then
      Print #UBRpt, Tab(3); Num2Date$(UBBillRec(1).BillDate); Tab(14); PrevDate$; Tab(25); DateRead$;
       'Only Print Days if Greater than 0
       If DaysINRead > 0 Then
         Print #UBRpt, Tab(38); Using("####", DaysINRead)
       Else
         Print #UBRpt, " "
       End If
    Else
...
'      End If
    ...
    For WRevCnt = 1 To lpcnt - 1
      PCnt = PCnt + 1  'Printable lines
      If UBBillRec(1).RevAmt(WRevCnt) <> 0 Then
        'Reduced Revname to 3 and moved reads over by 2  PS 7/31/2009 (for Montross)
        If MontrossFlag = True Then
          Print #UBRpt, "  "; Left$(UBSetUpRec(1).Revenues(WRevCnt).RevName, 3);
        Else
          Print #UBRpt, " "; Left$(UBSetUpRec(1).Revenues(WRevCnt).RevName, 5);
        End If
        ...
        For mChk = 1 To 7
          If UBBillRec(1).MtrTypes(mChk) > 0 Then
            If UBCustRec(1).serv(WRevCnt).RMtrType = SMtrType(mChk) Then
              If Not BFlag Then
                If MontrossFlag = True Then
                    Print #UBRpt, Tab(6); Using("##########", SCurrRead&(mChk));
                    Print #UBRpt, Tab(16); Using("##########", SPrevRead&(mChk));
                    Print #UBRpt, Tab(26); Using("#######", SUsageAmt&(mChk));
                Else
...
'Utility Bill LaserLetterFormat(Uses Blank Stock)BAR CODE PRINTABLE
  Dim tmpdate As String, tmpstr As String, tmptot As Integer, tmpcnt As Integer, liltemp As String
  liltemp = ""
  tmpstr = ""
  tmptot = 0
  Zip$ = QPTrim$(UBCustRec(1).ZIPCODE)
...
'This String is created for the OCR Scanning of bill coupon for lockbox payments for BBT
'  They quit using this in Dec of 2009 -  PS
'     So far only for Carolina Beach,  Total Billing was printing this for them but they wanted
'     to start printing thier bills in house.
'     this is 2121 calc and if 2 digits in mult then add to make one....
'    If CBeach = True Then
'      tmpdate = Num2Date$(UBBillRec(1).BillDate)
'      tmpstr = Left$(tmpdate$, 2) + Mid$(tmpdate$, 4, 2) + Right$(tmpdate$, 2)
'      tmpdate = Num2Date$(UBBillRec(1).PastDueDate)
'      tmpstr = tmpstr + Left$(tmpdate$, 2) + Mid$(tmpdate$, 4, 2) + Right$(tmpdate$, 2)
'      tmpstr = tmpstr + Using("00000000", Acct$)
'      tmpdate = Str(Abs(Previous#) * 100)
'      tmpstr = tmpstr + Using("000000000000", tmpdate$)
'      tmpdate = Str(Abs(Totalamt#) * 100)
'      tmpstr = tmpstr + Using("000000000000", tmpdate$)
'      'tmpstr = tmpstr + "379"
'      For tmpcnt = 1 To 44
'       liltemp = Str$(Val(Mid$(tmpstr, tmpcnt, 1)) * 2)
'       liltemp = QPTrim$(liltemp)
'       If Len(liltemp) > 1 Then
'        tmptot = tmptot + ((Val(Mid$(liltemp, 1, 1))) + (Val(Mid$(liltemp, 2, 1))))
'       Else
'        tmptot = tmptot + (Val(liltemp))
'      End If
'       tmpcnt = tmpcnt + 1
'      Next
'      For tmpcnt = 2 To 44
'       tmptot = tmptot + (Val(Mid$(tmpstr, tmpcnt, 1)) * 1)
'       tmpcnt = tmpcnt + 1
'      Next
'      tmptot = tmptot Mod 10
'      If tmptot > 0 Then
'        tmptot = 10 - tmptot
'      Else
'        tmptot = 0
'      End If
'      tmpstr = Mid$(tmpstr, 1, 44)
'      ToPrint$ = ToPrint$ + "~" + tmpstr + Using("0", tmptot) + "402"
   ' Else
      ToPrint$ = ToPrint$ + "~ "
    'End If
    Print #UBRpt, ToPrint$
...
End Sub
Private Sub PrnLaserLegalWR(UBCustRec() As NewUBCustRecType, UBBillRec() As UBTransRecType, UBSetUpRec() As UBSetupRecType, blnPrintThisBill As Boolean)
...
'-=-=-=-=-=-=-=-=-=-=-=-
  ...
  If npcnt <= lpcnt Then
    For WRevCnt = 1 To 9
      If UBBillRec(1).RevAmt(WRevCnt) <> 0 Then
       PCnt = PCnt + 1
       ToPrint$ = ToPrint$ + "~" + Left$(UBSetUpRec(1).Revenues(WRevCnt).RevName, 4)
        For mChk = 1 To 7
        If Not BFlag Then
          If UBBillRec(1).MtrTypes(mChk) > 0 Then
            If UBCustRec(1).serv(WRevCnt).RMtrType = SMtrType(mChk) Then
        ...
        ToPrint$ = ToPrint$ + "~" + Using("#####.##", UBBillRec(1).RevAmt(WRevCnt))
      End If
   Next '1 to 9 revs when all revs can print
     For xcnt = (npcnt + 1) To lpcnt 'fill out rest of spaces
        ToPrint$ = ToPrint$ + "~ ~ ~ ~ ~ "
     Next
ElseIf npcnt > lpcnt Then
  WRevCnt = 0
'Revs 1 to 5
  For xcnt = 1 To 5
      WRevCnt = WRevCnt + 1
      ...
      End If
   Next '1 to 5 revs
   'if no prev charge line then can print rev 7
   If lpcnt = 8 Then
      WRevCnt = 7
      If UBBillRec(1).RevAmt(WRevCnt) <> 0 Then
       ToPrint$ = ToPrint$ + "~" + Left$(UBSetUpRec(1).Revenues(WRevCnt).RevName, 4)
    ...
    tmprev# = 0
      ...
      If lpcnt = 7 Then
        If UBBillRec(1).RevAmt(7) <> 0 Then
          tmprev# = tmprev# + UBBillRec(1).RevAmt(7)
          xother$ = Mid$(UBSetUpRec(1).Revenues(7).RevName, 1, 4)
        End If
      End If
        If UBBillRec(1).RevAmt(9) <> 0 Then
          tmprev# = tmprev# + UBBillRec(1).RevAmt(9)
          If Len(xother$) > 0 Then
            xother$ = xother$ + ", "
          End If
          xother$ = xother$ + Mid$(UBSetUpRec(1).Revenues(9).RevName, 1, 4)
        End If
        If UBBillRec(1).RevAmt(6) <> 0 Then
          tmprev# = tmprev# + UBBillRec(1).RevAmt(6)
          If Len(xother$) > 0 Then
            xother$ = xother$ + ", "
          End If
          xother$ = xother$ + Mid$(UBSetUpRec(1).Revenues(9).RevName, 1, 4)
...
'      Print #UBRpt,
    ...
    End If
    If DidADraftFlag Then
      ToPrint$ = ToPrint$ + "~" + "Draft Date-" + Num2Date$(UBBillRec(1).DraftDate)
    ElseIf Len(Message$) > 0 Then
      ToPrint$ = ToPrint$ + "~" + Message$
    Else
      ToPrint$ = ToPrint$ + "~" + Msg4$
...
Private Sub CreateSCSFileTransfer(UBCustRec() As NewUBCustRecType, UBBillRec() As UBTransRecType, UBSetUpRec() As UBSetupRecType)
    'Dim wFmt01 As String * 10
    'Dim wFmt02 As String * 10
    WFoundMtr = False
    IFoundMtr = False
    CubicMtr = False
    ...
    LSet PrintRec(1).MtrNumb = MtrNumb$
        ...
        'PrintRec(1).TotalDue = Using(Fmt15$, Str$(Totalamt#))
    PrintRec(1).PenaltyAmt = Using(Fmt15$, Str$(CustPenalty#))
    PrintRec(1).WithPenAmt = Using(Fmt15$, Str$(Round(Totalamt# + CustPenalty#)))
    
    PrintRec(1).CrLf = CrLf$
    
    Put #UBRpt, , PrintRec(1)
End Sub
Private Sub CreateSCSFileTransfer2(UBCustRec() As NewUBCustRecType, UBBillRec() As UBTransRecType, UBSetUpRec() As UBSetupRecType)
    MtrNumb$ = ""
    MTRType$ = ""
    Previous# = Round#(UBCustRec(1).PrevBalance + UBCustRec(1).CurrBalance)
    Totalamt# = Round#(Previous# + UBBillRec(1).Transamt)
    ...
    PrintRecr(1).AcctNo = Using("########", Str$(CustAcctNo&))
    PrintRecr(1).LocationNum = UBCustRec(1).Book + "-" + UBCustRec(1).SEQNUMB
    PrintRecr(1).Cycle = Using("##", Str$(UBCustRec(1).BILLCYCL))
    RSet PrintRecr(1).CustName = QPTrim$(UBCustRec(1).CustName)
    RSet PrintRecr(1).ADDR1 = QPTrim$(UBCustRec(1).ADDR1)
    RSet PrintRecr(1).ADDR2 = QPTrim$(UBCustRec(1).ADDR2)
    RSet PrintRecr(1).ServAddr = QPTrim$(UBCustRec(1).ServAddr)
    RSet PrintRecr(1).CITY = QPTrim$(UBCustRec(1).CITY)
    RSet PrintRecr(1).STATE = QPTrim$(UBCustRec(1).STATE)
    RSet PrintRecr(1).ZIPCODE = QPTrim$(UBCustRec(1).ZIPCODE)
    If Fflag Then
      PrintRecr(1).BillType = "F"
      PrintRecr(1).DepAppAmt = Using(Fmt10a$, Str$(CDeposit#))
    Else
      PrintRecr(1).BillType = "N"
      PrintRecr(1).DepAppAmt = ""
    End If

    PrintRecr(1).PrevDue = Using(Fmt15$, Str$(Previous#))
    PrintRecr(1).CurrDue = Using(Fmt15$, Str$(UBBillRec(1).Transamt))
    PrintRecr(1).TotalDue = Using(Fmt15$, Str$(Totalamt#))
    PrintRecr(1).CurrDate = DateRead$
    PrintRecr(1).PrevDate = PrevDate$
    IFoundMtr = False
    For mChk = 1 To 7
      UsageAmt& = 0
      MaxMeterAmt& = 0
        ...
        If UBBillRec(1).MtrTypes(mChk) > 0 Then
          ...
          If Len(MtrNumb$) > 0 Then
            If MtrNFlag = 2 Then
              LSet PrintRecr(1).MtrInfo(mChk).MtrIDNum = QPTrim(UBCustRec(1).LocMeters(mChk).MtrIDNO)
            Else
              LSet PrintRecr(1).MtrInfo(mChk).MtrIDNum = QPTrim(UBCustRec(1).LocMeters(mChk).MtrNum)
            End If
          Else
            LSet PrintRecr(1).MtrInfo(mChk).MtrIDNum = "?"
          End If
          MtrTyp = UBBillRec(1).MtrTypes(mChk)
          If MtrTyp = 1 Then
            MTRType$ = "W"
          ...
          ElseIf MtrTyp = 7 Then
            MTRType$ = "T"
          ElseIf MtrTyp = 4 Then
            MTRType$ = "E"
          ElseIf MtrTyp = 5 Then
            MTRType$ = "D"
          ElseIf MtrTyp = 6 Then
            MTRType$ = "G"
          ElseIf MtrTyp = 9 Then
            MTRType$ = "I"
          Else
            MTRType$ = " "
          End If
          PrintRecr(1).MtrInfo(mChk).MTRType = MTRType$
          If CubicMtr Then
            PrintRecr(1).MtrInfo(mChk).MtrUnit = "C"
          Else
            PrintRecr(1).MtrInfo(mChk).MtrUnit = "G"
          End If
          If UBBillRec(1).CurRead(mChk) > 0 Then
          ...
          End If
          UsageAmt& = Round(UsageAmt& * UBCustRec(1).LocMeters(mChk).MTRMulti)
          If UBCustRec(1).LocMeters(mChk).MtrUnit = "C" Then
            CubicMtr = True
            UsageAmt& = UsageAmt& * 7.481
          End If
          PrintRecr(1).MtrInfo(mChk).CurrRead = Using("##########", UBBillRec(1).PrevRead(mChk))
          PrintRecr(1).MtrInfo(mChk).PrevRead = Using("##########", UBBillRec(1).CurRead(mChk))
          PrintRecr(1).MtrInfo(mChk).Consump = Using("##########", UsageAmt&)
        Else
          PrintRecr(1).MtrInfo(mChk).MTRType = " "
          PrintRecr(1).MtrInfo(mChk).MtrUnit = " "
          PrintRecr(1).MtrInfo(mChk).MtrIDNum = " "
          PrintRecr(1).MtrInfo(mChk).CurrRead = " "
          PrintRecr(1).MtrInfo(mChk).PrevRead = " "
          PrintRecr(1).MtrInfo(mChk).Consump = " "
        End If
      Next
    ...
    End If
     ...
     PrintRecr(1).ServDays = Using("####", Str$(DaysINRead))
    For serv = 1 To 15
      PrintRecr(1).ServInfo(serv).ServText = QPTrim$(UBSetUpRec(1).Revenues(serv).RevName)
      PrintRecr(1).ServInfo(serv).ServAmt = Using(Fmt10a$, Str$(UBBillRec(1).RevAmt(serv)))
    Next


    PrintRecr(1).BillDate = BillDateS$
    PrintRecr(1).PastDueDate = PastDueDate$
    ...
    End If
    If DidADraftFlag Then
      PrintRecr(1).DraftDate = DraftDateS$
    Else
      PrintRecr(1).DraftDate = ""
    End If
    If Len(Message$) > 0 Then
      RSet PrintRecr(1).MsgLine1 = Message$
    Else
      RSet PrintRecr(1).MsgLine1 = Msg1$
    End If
    RSet PrintRecr(1).MsgLine2 = Msg2$
    RSet PrintRecr(1).MsgLine3 = Msg3$
    RSet PrintRecr(1).MsgLine4 = Msg4$
    
    PrintRecr(1).PenaltyAmt = Using(Fmt15$, Str$(CustPenalty#))
    PrintRecr(1).WithPenAmt = Using(Fmt15$, Str$(Round(Totalamt# + CustPenalty#)))

    PrintRecr(1).CrLf = CrLf$
    Put #UBRpt, , PrintRecr(1)
End Sub

Private Sub CreateSCSFileTransfer3(UBCustRec() As NewUBCustRecType, UBBillRec() As UBTransRecType, UBSetUpRec() As UBSetupRecType)
    MtrNumb$ = ""
    MTRType$ = ""
    Previous# = Round#(UBCustRec(1).PrevBalance + UBCustRec(1).CurrBalance)
    Totalamt# = Round#(Previous# + UBBillRec(1).Transamt)
    ...
    PrintRecM(1).AcctNo = Using("########", Str$(CustAcctNo&))
    PrintRecM(1).LocationNum = UBCustRec(1).Book + "-" + UBCustRec(1).SEQNUMB
    PrintRecM(1).Cycle = Using("##", Str$(UBCustRec(1).BILLCYCL))
    RSet PrintRecM(1).CustName = QPTrim$(UBCustRec(1).CustName)
    RSet PrintRecM(1).ADDR1 = QPTrim$(UBCustRec(1).ADDR1)
    RSet PrintRecM(1).ADDR2 = QPTrim$(UBCustRec(1).ADDR2)
    RSet PrintRecM(1).ServAddr = QPTrim$(UBCustRec(1).ServAddr)
    RSet PrintRecM(1).CITY = QPTrim$(UBCustRec(1).CITY)
    RSet PrintRecM(1).STATE = QPTrim$(UBCustRec(1).STATE)
    RSet PrintRecM(1).ZIPCODE = QPTrim$(UBCustRec(1).ZIPCODE)
    If Fflag Then
      PrintRecM(1).BillType = "F"
      PrintRecM(1).DepAppAmt = Using(Fmt10a$, Str$(CDeposit#))
    Else
      PrintRecM(1).BillType = "N"
      PrintRecM(1).DepAppAmt = ""
    End If

    PrintRecM(1).PrevDue = Using(Fmt15$, Str$(Previous#))
    PrintRecM(1).CurrDue = Using(Fmt15$, Str$(UBBillRec(1).Transamt))
    PrintRecM(1).TotalDue = Using(Fmt15$, Str$(Totalamt#))
    PrintRecM(1).CurrDate = DateRead$
    PrintRecM(1).PrevDate = PrevDate$
    IFoundMtr = False
    For mChk = 1 To 7
      UsageAmt& = 0
      MaxMeterAmt& = 0
        ...
        If UBBillRec(1).MtrTypes(mChk) > 0 Then
          ...
          If Len(MtrNumb$) > 0 Then
            If MtrNFlag = 2 Then
              LSet PrintRecM(1).MtrInfo(mChk).MtrIDNum = QPTrim(UBCustRec(1).LocMeters(mChk).MtrIDNO)
            Else
              LSet PrintRecM(1).MtrInfo(mChk).MtrIDNum = QPTrim(UBCustRec(1).LocMeters(mChk).MtrNum)
            End If
          Else
            LSet PrintRecM(1).MtrInfo(mChk).MtrIDNum = "?"
          End If
          MtrTyp = UBBillRec(1).MtrTypes(mChk)
          If MtrTyp = 1 Then
            MTRType$ = "W"
          ...
          Else
            MTRType$ = " "
          End If
          PrintRecM(1).MtrInfo(mChk).MTRType = MTRType$
          If CubicMtr Then
            PrintRecM(1).MtrInfo(mChk).MtrUnit = "C"
          Else
            PrintRecM(1).MtrInfo(mChk).MtrUnit = "G"
          End If
          If UBBillRec(1).CurRead(mChk) > 0 Then
              UsageAmt& = UBBillRec(1).CurRead(mChk) - UBBillRec(1).PrevRead(mChk)
          End If
          ...
          If UBCustRec(1).LocMeters(mChk).MtrUnit = "C" Then
            ...
            UsageAmt& = UsageAmt& * 7.481
          End If
          PrintRecM(1).MtrInfo(mChk).CurrRead = Using("##########", UBBillRec(1).PrevRead(mChk))
          PrintRecM(1).MtrInfo(mChk).PrevRead = Using("##########", UBBillRec(1).CurRead(mChk))
          PrintRecM(1).MtrInfo(mChk).Consump = Using("##########", UsageAmt&)
        Else
          PrintRecM(1).MtrInfo(mChk).MTRType = " "
          PrintRecM(1).MtrInfo(mChk).MtrUnit = " "
          PrintRecM(1).MtrInfo(mChk).MtrIDNum = " "
          PrintRecM(1).MtrInfo(mChk).CurrRead = " "
          PrintRecM(1).MtrInfo(mChk).PrevRead = " "
          PrintRecM(1).MtrInfo(mChk).Consump = " "
        End If
      Next
    ...
    End If
     ...
     PrintRecM(1).ServDays = Using("####", Str$(DaysINRead))
    For serv = 1 To 15
      PrintRecM(1).ServInfo(serv).ServText = QPTrim$(UBSetUpRec(1).Revenues(serv).RevName)
      PrintRecM(1).ServInfo(serv).ServAmt = Using(Fmt10a$, Str$(UBBillRec(1).RevAmt(serv)))
    Next
    PrintRecM(1).BillDate = BillDateS$
    PrintRecM(1).PastDueDate = PastDueDate$
'121718  Draft flag was not being set
    DidADraftFlag = False
    If UseDraftFlag And UBCustRec(1).USEDRAFT = "Y" And UBCustRec(1).PreNoteFlag Then
      DidADraftFlag = True
    End If
    ...
    If DidADraftFlag Then
      PrintRecM(1).DraftDate = DraftDateS$
    Else
      PrintRecM(1).DraftDate = ""
    End If
    If Len(Message$) > 0 Then
      RSet PrintRecM(1).MsgLine1 = Message$
    Else
      RSet PrintRecM(1).MsgLine1 = Msg1$
    End If
    RSet PrintRecM(1).MsgLine2 = Msg2$
    RSet PrintRecM(1).MsgLine3 = Msg3$
    RSet PrintRecM(1).MsgLine4 = Msg4$
    
    PrintRecM(1).PenaltyAmt = Using(Fmt15$, Str$(CustPenalty#))
    PrintRecM(1).WithPenAmt = Using(Fmt15$, Str$(Round(Totalamt# + CustPenalty#)))

    PrintRecM(1).CrLf = CrLf$
    Put #UBRpt, , PrintRecM(1)
End Sub

'
'

Private Sub CedarBill(UBCustRec() As NewUBCustRecType, UBBillRec() As UBTransRecType, UBSetUpRec() As UBSetupRecType)
'Cedar Bluff VA
'last revised 09-09-98
  Dim CustMsg As String, DidWMrt As Boolean, WMtrCnt As Integer
  ...
  If Not DidWMrt Then
    Print #UBRpt,
  End If
  Print #UBRpt, Tab(35); "Acct:"; Using("#####", CustAcctNo&)
  If UBBillRec(1).RevAmt(1) > 0 Then
...
End Sub
''Bladen Bill
Private Sub PrnBladenBill(UBCustRec() As NewUBCustRecType, UBBillRec() As UBTransRecType, UBSetUpRec() As UBSetupRecType)
  Dim CutOff As String, Z As String
    CutOff$ = QPTrim$(Msg1$)
...
'    'PAmt# = Round#(TotalAmt# + FiveBucks)
    ...
    Print #UBRpt, Tab(33); Left$(BillDateS$, 6); Right$(BillDateS$, 2);
    Print #UBRpt, Tab(42); Left$(PastDateS$, 6); Right$(PastDateS$, 2);
...
PrintNewStandV1Mask:      '4
    ...
    '**************************
        ...
        End If
      Case 5
        If Bucksport = False And BLType = 4 Then
            Print #UBRptA, Tab(46); "XXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
        Else
      ...
      Case 8
        If Bucksport = False And BLType = 4 Then
            Print #UBRptA, Tab(46); "XXXXXXXXXXXXXXXXX"; " "; "XX"; " "; "XXXXX"
        Else
...
PrnCedarMask:
  ...
  Print #UBRptA,
  Print #UBRptA, "#######";
  Print #UBRptA, Tab(19); "XX/XX/XXXX"
  Print #UBRptA,
  Print #UBRptA,
  ...
  Print #UBRptA, Tab(11); "#########";
  Print #UBRptA, Tab(20); "#########"
  Print #UBRptA, Tab(35); "Acct:"; "#####"
  Print #UBRptA, Tab(2); "Rev Name"; Tab(21); "#######.##";
  Print #UBRptA, Tab(35); "Customer Name"

  Print #UBRptA, Tab(2); "Rev Name"; Tab(21); "#######.##";
  Print #UBRptA, Tab(35); "XXXXXXXXXXXXXXXXXXXXXXXXXXX"

  Print #UBRptA, Tab(2); "Rev Name"; Tab(21); "#######.##";
  Print #UBRptA, Tab(35); "City ST 99999"
 
  Print #UBRptA, Tab(2); "Rev Name"; Tab(21); "#######.##"

  Print #UBRptA, Tab(2); "Previous:"; Tab(21); "#######.##"
  Print #UBRptA,
  Print #UBRptA, Tab(2); "  Tax"; Tab(21); "#####.##";
  Print #UBRptA, Tab(35); "XXXXXXXXXXXXXXXXXXXXXXXXXXXX"
  Print #UBRptA,
  Print #UBRptA, Tab(21); "#######.##";
  Print #UBRptA, Tab(35); "XX/XX/XXXX";
  Print #UBRptA, Tab(49); "#####.##"
  Print #UBRptA,
  Print #UBRptA, Tab(2); "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
  Print #UBRptA, Tab(2); "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
  Print #UBRptA, Tab(2); "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
  Print #UBRptA, Tab(2); "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
  Print #UBRptA, "~"
...
PrnBladenMask:
    ...
    Print #UBRptA, "########"; Tab(4); "##"
    Print #UBRptA,
    Print #UBRptA, Tab(2); "Rev1"; Tab(22); "######.##"
    Print #UBRptA, Tab(2); "Rev2"; Tab(22); "######.##"
    Print #UBRptA, Tab(2); "Rev3"; Tab(22); "######.##"
    Print #UBRptA, Tab(3); "Rev4"; Tab(22); "######.##";
    Print #UBRptA, Tab(33); "Service Address"
    Print #UBRptA, " Previous:"; Tab(22); "######.##"
    Print #UBRptA, Tab(33); "XX/XX/XX";
    Print #UBRptA, Tab(42); "XX/XX/XX";
    Print #UBRptA, Tab(51); "XX/XX/XX"
    Print #UBRptA,
    Print #UBRptA, Tab(22); "######.##";
    Print #UBRptA, Tab(32); "#######";
    Print #UBRptA, Tab(41); "#######";
    ...
    Print #UBRptA, Tab(51); "XX/XX/XX"
    Print #UBRptA,
    Print #UBRptA, Tab(31); "Customer Name"
    Print #UBRptA, Tab(31); "Address Line 111111111111"
    Print #UBRptA, Tab(31); "Address Line 222222222222"
    Print #UBRptA, Tab(31); "City"; " "; "XX"; " "; "11223"
    Print #UBRptA,
    Print #UBRptA, "Message line 2"
    Print #UBRptA, "Message line 3"
    Print #UBRptA, "DRAFT NOTICE DO NOT PAY"
    Print #UBRptA, "~"
Return
prn21linetxtplainmask:
     Print #UBRptA, "Date "; "XX/XX/XX"; Tab(50); "        Town of Anywhere"
     Print #UBRptA, "Present Reading"; Tab(30); "#######";
     Print #UBRptA, Tab(50); "Option line 2 of setup"
     Print #UBRptA, "Previous Reading"; Tab(30); "#######";
     Print #UBRptA, Tab(50); "Option line 3 of setup"
     Print #UBRptA, "Consumed (Thousands of Gal.) "; Tab(30); "#######";
     Print #UBRptA, Tab(50); "Option line 4 of setup"
     Print #UBRptA, "Rev1name"; Tab(32); "###,###.##";
     Print #UBRptA, Tab(50); "Option line 5"
     Print #UBRptA, "Rev2name"; Tab(32); "###,###.##";
     Print #UBRptA, Tab(50); "Option line 6"
     Print #UBRptA, "Rev3name"; Tab(32); "###,###.##";
     Print #UBRptA, Tab(50); "Option line 7"
     Print #UBRptA, "Rev4name"; Tab(32); "###,###.##";
     Print #UBRptA, Tab(50); "Option line 8"
     Print #UBRptA, "Pastdue Balance Due: "; Tab(32); "###,###.##";
     Print #UBRptA, Tab(50); "Message or Svrc Dates"
     Print #UBRptA, "Total Due"; Tab(32); "$###,###.##";
     Print #UBRptA, Tab(50); "Option line 9"
     Print #UBRptA, Tab(50); "Option Date Due:"; "XX/XX/XX"
     Print #UBRptA,
     Print #UBRptA,
     Print #UBRptA, "####";
     Print #UBRptA, Tab(10); "Customer Name"
     Print #UBRptA, Tab(10); "Address line 1"
     Print #UBRptA, Tab(10); "Address line 2"
     Print #UBRptA, Tab(10); "City"; " "; "NC"; " "; "11111";
     Print #UBRptA, Tab(60); "#####"
     Print #UBRptA,
     Rem condense here lines 19 & 20
     Print #UBRptA, Chr$(15);
     Print #UBRptA, "Text from setup info - 2 lines. Ex- A 10% penalty will be added 30 days after billing. Services will be discontinued if not"
     Print #UBRptA, "paid within 35 days from billing date.  Receipt will not be returned unless requested.  A $10.00 charge on all returned checks."
     Print #UBRptA, Chr$(18);
     Rem uncondense here
     Print #UBRptA, "~"
  Return
...
Private Function ValidDate()
  ...
  ValidDate = False
  If fpPrevDate <> "" And fpCurrDate <> "" Then
    TempDate1 = DateDiff("d", "12/31/1979", fpPrevDate)
    TempDate2 = DateDiff("d", "12/31/1979", fpCurrDate)
    If TempDate1 > TempDate2 Then
      ValidDate = False
      MsgBox "The Previous And Current Dates Must Be In Chronological Order Or Equal", vbOKOnly, "Invalid Date"
    Else
      ValidDate = True
    End If
  Else
  ...
  End If
End Function
Private Sub Prn21LineTxtPlainP(UBCustRec() As NewUBCustRecType, UBBillRec() As UBTransRecType, UBSetUpRec() As UBSetupRecType, UBbill21line() As UBBill21LineType) 'Bill 21
'@#@#@#@##@#@##@# Text Bill for Middletown  Blank 21 line stock
'Middletown Va
  ...
  For MtrCnt = 1 To 7
    ...
    Case MtrWaterOnly, MtrSewerOnly, MtrCombined, MtrTouchRead
      If UBBillRec(1).PrevRead(MtrCnt) < 0 Then
        UBBillRec(1).PrevRead(MtrCnt) = 0
      End If
      If UBBillRec(1).CurRead(MtrCnt) < 0 Then
        UBBillRec(1).CurRead(MtrCnt) = 0
      End If
      WCurrRead& = UBBillRec(1).CurRead(MtrCnt)
      WPrevRead& = UBBillRec(1).PrevRead(MtrCnt)
      WUsageAmt& = UBBillRec(1).CurRead(MtrCnt) - UBBillRec(1).PrevRead(MtrCnt)
      If WUsageAmt& < 0 Then
        MaxMeterAmt& = 10& ^ (Len(Str$(UBBillRec(1).PrevRead(MtrCnt))) - 1)
        WUsageAmt& = (MaxMeterAmt& - UBBillRec(1).PrevRead(MtrCnt)) + UBBillRec(1).CurRead(MtrCnt)
      End If
      Exit For
    End Select
  Next
   ...
   AcctNum = UBBillRec(1).CustAcctNo
     ...
     Print #UBRpt, "Date "; Num2Date$(UBBillRec(1).BillDate); Tab(50); RTrim$(UBbill21line(1).TxtLine1)
     Print #UBRpt, "Present Reading"; Tab(30); Using("#######", WCurrRead&);
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine2)
     Print #UBRpt, "Previous Reading"; Tab(30); Using("#######", WPrevRead&);
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine3)
     Print #UBRpt, "Consumed (Thousands of Gal.) "; Tab(30); Using("#######", WUsageAmt&);
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine4)
     
     If UBBillRec(1).RevAmt(1) <> 0 Then
      Print #UBRpt, UBSetUpRec(1).Revenues(1).RevName; Tab(32); Using$("#####,#.##", UBBillRec(1).RevAmt(1));
     End If
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine5)
     If UBBillRec(1).RevAmt(2) <> 0 Then
      Print #UBRpt, UBSetUpRec(1).Revenues(2).RevName; Tab(32); Using("#####,#.##", UBBillRec(1).RevAmt(2));
     End If
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine6)
     If UBBillRec(1).RevAmt(3) <> 0 Then
      Print #UBRpt, UBSetUpRec(1).Revenues(3).RevName; Tab(32); Using("#####,#.##", UBBillRec(1).RevAmt(3));
     End If
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine7)
     If Middletown Then  'skip rev 4)penalty
        If UBBillRec(1).RevAmt(4) > 0 Then
          tmprev# = UBBillRec(1).RevAmt(4)
        End If
        For PCnt = 6 To 15
          If UBBillRec(1).RevAmt(PCnt) <> 0 Then
            tmprev# = tmprev# + UBBillRec(1).RevAmt(PCnt)
        ...
        Next
        If tmprev# <> 0 Then
          tmprev# = tmprev# + UBBillRec(1).RevAmt(5)
          Print #UBRpt, "Other";
          Print #UBRpt, Tab(32); Using("#####,#.##", tmprev#);
        Else
          If UBBillRec(1).RevAmt(5) <> 0 Then
            Print #UBRpt, UBSetUpRec(1).Revenues(5).RevName; Tab(32); Using("#####,#.##", UBBillRec(1).RevAmt(5));
          End If
        End If
     Else
        For PCnt = 5 To 15
        ...
        Next
        If tmprev# <> 0 Then
          tmprev# = tmprev# + UBBillRec(1).RevAmt(4)
          Print #UBRpt, "Other";
          Print #UBRpt, Tab(32); Using("#####,#.##", tmprev#);
        Else
          If UBBillRec(1).RevAmt(4) <> 0 Then
            Print #UBRpt, UBSetUpRec(1).Revenues(4).RevName; Tab(32); Using("#####,#.##", UBBillRec(1).RevAmt(4));
          End If
        End If
     End If
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine8)
     Print #UBRpt, "Pastdue Balance Due: "; Tab(32); Using("#####,#.##", Previous#);
     
     If MsgORdates = 0 Then
       Print #UBRpt, Tab(50); QPTrim$(Msg1$)
     Else
       Print #UBRpt, Tab(50); "Service: "; PrevDate$; "-"; DateRead$
     End If
     Print #UBRpt, "Total Due"; Tab(32); Using("$#####,#.##", Totalamt#);
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine9)
     Print #UBRpt, Tab(50); RTrim$(UBbill21line(1).TxtLine11); Num2Date$(UBBillRec(1).PastDueDate)
     Print #UBRpt,
     Print #UBRpt,
     ...
     Print #UBRpt, Tab(60); Using("#####", FBillNO& + PrintedCnt)
     Print #UBRpt,
     Rem condense here lines 19 & 20
     Print #UBRpt, Chr$(15);
     Print #UBRpt, QPTrim$(UBbill21line(1).TxtBLine1) '"All bills due upon presentation. A penalty of 10% will be added 30 days after billing date. Services will be discontinued if not"
     Print #UBRpt, QPTrim$(UBbill21line(1).TxtBLine2) '"paid within 35 days from billing date.  Receipt will not be returned unless requested.  A $10.00 charge on all returned checks."
     Print #UBRpt, Chr$(18);
     Rem uncondense here
     Print #UBRpt, "~"
...
End Sub
Private Sub PrnLaserLegal(UBCustRec() As NewUBCustRecType, UBBillRec() As UBTransRecType, UBSetUpRec() As UBSetupRecType)
'''PrnLaserLegal:  'Use billing date and past due date
'Utility Bill Laser(Uses Blank Stock)BAR CODE PRINTABLE
  Zip$ = QPTrim$(UBCustRec(1).ZIPCODE)
  'ZDigit$ = GetZipEDigit$(Zip$)
  'Zip$ = Zip$ + ZDigit$
  '"Route: "; UBCustRec(1).POSTRTE
  lpcnt = 8
  FoundAMtr = False
  ReDim MsgText(0 To 5) As String
  ReDim SFoundMtr(1 To 7) As Boolean
  ...
  ReDim SUsageAmt(1 To 7) As Long
  ReDim SMtrType(1 To 7) As String
  Dim PrnBilFlag As Boolean
  Dim PBYN As String
  PrnBilFlag = True
  
'060619 Deep Run WWWWWWWW
  If blnAllowEBill Then
    PBYN = QPTrim$(UBCustRec(1).PrnBillYN)
    If Len(PBYN) = 0 Then
      PrnBilFlag = True
    Else
      Select Case PBYN
...
'Need to get Penalty calculated based on defaults from setup screen
  If Totalamt# > 0 And Not FinalFlag Then
    If UBCustRec(1).LATEFEE = "Y" Then
        If (TotalTax > 0 And Not PenTaxFlag) And Not SpenTn Then
          CalcPenamt Round(UBBillRec(1).Transamt - TotalTax), Previous, Round(Totalamt - (TotalTax))
        Else
          CalcPenamt UBBillRec(1).Transamt, Previous, Totalamt
        End If
...
'    Else
    ...
    If Len(QPTrim$(UBCustRec(1).POSTRTE)) > 0 Then
      ToPrint$ = ToPrint$ + "~" + "Route: " + UBCustRec(1).POSTRTE
    Else
      ToPrint$ = ToPrint$ + "~ "
    End If
'060619 Deep Run
