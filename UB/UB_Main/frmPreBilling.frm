  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        UBLog "Closed via PreBilling by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
Private Function alloktogo()
  If Val(fptxtRoute1.Text) > 0 Then
    alloktogo = True
  Else
    alloktogo = False
  End If
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpPrebilling
End Sub
...
Private Sub PreBillReport()
  ...
  Dim Pumptest As Double, linechk As Integer, Num2Print As Integer, BillcopyMsg As String
...
'''  Erase UBCustRec, MDateIdx
  ...
  If UBSetUpRec(1).PreByBook = "Y" Then
    ...
    Else
      BookFlag = True
    End If
  ElseIf UBSetUpRec(1).BILLCYCL = "Y" Then 'And UBSetUpRec(1).PreByBook = "N"
  ...
  For cnt = 1 To NumOfRecs
    ...
    If UBBillRec(1).ActiveFlag <> 0 Then
      ...
      Else
        BillTo$ = " C"
      End If
      'GoSub GetWhatBook  'Put this above active flag so non charge will have book
      If BadBookFlag Then
      ...
      If Num2Print = 0 Then BillcopyMsg = "***BILLCopies Set to 0" Else BillcopyMsg = ""
      ...
      Print #UBRpt, BillTo$, BillcopyMsg
      LineCnt = LineCnt + 1
      For FRCnt = 1 To 4
...
'***********************************************************************
        ...
        Else    'it's a nonmetered service
          ...
          Bookconsump(WhatBook).TaxAmt(TRevCnt) = Round#(Bookconsump(WhatBook).TaxAmt(TRevCnt) + UBBillRec(1).TaxAmt(TRevCnt))
          RevTotals(TRevCnt) = Round#(RevTotals(TRevCnt) + UBBillRec(1).RevAmt(TRevCnt))
        'Here would be the place to list rate for nonmetered svc???
        ''' '' Print #UBRpt, tab(14);   Tab(69); Using(fmt$(3), UBBillRec(1).RevAmt(TRevCnt));
          Print #UBRpt, Tab(69); Using(fmt$(3), UBBillRec(1).RevAmt(TRevCnt));
          If UBBillRec(1).ProRatePCT < 100 And ProrateServ(TRevCnt) Then
            Print #UBRpt, "*";
          End If
            ...
            For MCCnt = 1 To 7
              ...
              If (Len(LocMeterType$) > 0) And (LocMeterType$ = UBCustRec(1).serv(TTRevCnt).RMtrType) Then
                ...
                If CurReadAmt& <> PreReadAmt& Then
                 ...
                 '012805 added multiplier for consumption total
                  ConsumpAmt& = ConsumpAmt& * MeterMulti&
    ...
    If HasAPumpCode Then
      ...
      For MPCnt = 1 To 7
        ...
        If PumpMtrOK Then
          ...
          Pumptest = (0# + ReadAmt&) * MeterMulti&
'                  ^This forces basic to convert to a Double
'                   before calculation, traps overflow errors
              If Pumptest# > 2147483647 Then
                '                  ^Max long integer value
                Unload FrmShowPctComp
                If ErrorScrn(1, ThisCustRec&) Then
                  AbortFlag = True
                  GoTo ErrorAbortExit
                End If
              End If

          PumpConsump(WhatPump).Consump = PumpConsump(WhatPump).Consump + Pumptest '(ReadAmt& * MeterMulti&)
        End If
...
'    ShowPctComp cnt, NumOfRecs
  ...
  If DoFuelAdjFlag Then
    Print #UBRpt,
    Print #UBRpt, Tab(10); "Fuel adjustment factor:" + Str$(FuelAdjAmt#)
  End If
  Print #UBRpt, FF$
  ...
  For cnt = 1 To TBooks
    ...
    For ZCnt = 1 To NumofRevs
      ...
      TestTot# = TestTot# + Bookconsump(cnt).Consump(ZCnt)
    Next
    TestTot# = TestTot# + Bookconsump(cnt).CustCnt
    If TestTot# <> 0 Then
...
'*******
      ...
      For RCnt = 1 To NumofRevs
        Print #UBRpt, QPTrim$(RevDesc(RCnt)); Tab(30); Using(fmt$(4), Bookconsump(cnt).Consump(RCnt));
...
SkipThisBook:
  ...
  If Not AbortFlag Then
    If Grpt Then
      Load frmLoadingRpt
      frmLoadingRpt.setwherefrom frmPreBilling
...
Public Sub MakeBillFile(AbortFlag, FuelAdjAmt#, ThisCycle%, ThisBook%)
  ...
  Dim TCnt As Integer, CoveCityFlag As Boolean
 ' BlockClear
...
'  End If
  ...
  If UBSetUp(1).PreByBook = "Y" And ThisBook > 0 Then
    BookFlag = True
  ElseIf UBSetUp(1).BILLCYCL = "Y" Then 'And UBSetUp(1).PreByBook = "N"
    CycleFlag = True
  End If
...
'SunnyBeach 091701
'Rem this out on 6/22/04 Irrigation meters added to replace special code.
'    If SunBchFlag Then
'      GotIRRMtr = False
'      IrrConsp& = 0
'      For IrrMtr = 1 To 7
...
'   Rem this out 6/22/04 Irrigation meters added so no need
