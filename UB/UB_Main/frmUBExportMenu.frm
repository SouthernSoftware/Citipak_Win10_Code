...
ExportThisAccount:
  ...
  Print #UBRpt, QPTrim$(Str$(cnt));
  Print #UBRpt, q$; Address1$;
  Print #UBRpt, q$; Address2$;
  Print #UBRpt, q$; Cty$;
...
Private Sub ImpPostalCass()
  ...
  'If cnt is greater than the total num of customers then bad data
    ...
    Else
      FrmShowPctComp.ShowPctComp 1, 1
      MsgBox "Invalid Information Found - Imported " & Import& & " Accounts.", vbOKOnly, "Procedure Ended"
      GoTo impend
    End If
