...
Private Sub cmdEditPenalty_Click()
  ...
  ReDim MsgText(0 To 5) As String
   If Not FileSize(UBPath$ + "UBPENTRN.DAT") > 0 Then
   'UBPath$ + "UBPENINF.DAT"
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpPenalty
End Sub

Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    If cmdExitPenaltyProcess.Enabled = False Then
      Cancel = True
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        UBLog "Closed via PenaltyMenu by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
  Select Case KeyCode
    Case vbKeyEscape:
      ...
      KeyCode = 0
    Case vbKeyHome
      cmdCalcPenalties.SetFocus
    Case vbKeyEnd
      cmdExitPenaltyProcess.SetFocus
    Case Else:
  End Select
End Sub
...
Private Sub PenaltyReport(rptopt As Integer)
  ...
  If Not Exist(UBPath$ + "UBPENINF.DAT") Then
     frmMsgDialog.RetLabel = "-2"
     FntSize = frmMsgDialog.Label(2).FontSize
     frmMsgDialog.Label(2).FontSize = (FntSize + 2)
     MsgText(0) = "ERROR:"
  ...
  Else
    Load frmLoadingRpt
    frmLoadingRpt.setwherefrom frmUBPenaltyMenu
...
Private Sub PostPenalties()
  ...
  UBLog PWUser$ + "IN: Post Penalty Transactions (PPT)"
  ...
  Next
  Close
  UBLog PWUser$ + "  DONE: Posting Penalty Transactions."
...
ExitPenPost:
  UBLog PWUser$ + " OUT: Penalty Posting." + CrLf$
