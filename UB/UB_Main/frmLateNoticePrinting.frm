...
Begin VB.Form frmLateNoticePrinting 
   ...
   Begin LpLib.fpCombo fpcboPrintOrder 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmLateNoticePrinting.frx":0BF8
   End
   Begin LpLib.fpCombo fpcboBalType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmLateNoticePrinting.frx":0F1B
   End
...
Dim Rteflag As Boolean, AcctBar As Boolean, PostBar As Boolean
Dim CedarBlf As Boolean, Bladen As Boolean, Summerton As Boolean
Dim Wrights As Boolean
...
'depending on which late notice they have selected in setup
  If LNType = 1 Then
    ...
    'do the text
      ...
      ActivateControls Me
    End If
  ElseIf LNType = 8 Or LNType = 9 Or LNType = 10 Then
    PrintLateNotices True
  Else
    DeActivateControls Me
    PrintLateNotices False
...
Private Sub Form_Load()
  ...
  Else
    Summerton = False
  End If
'remmed out Blanden flag
'added wrights 4/18/2008
...
Private Sub PrintLateNotices(Grpt As Boolean) '(NoticeInfo As NoticeInfoType)
  ...
  Dim Print1 As Integer, PrnCnt As Integer, NIfile As Integer, CutOff As String
  ...
  Dim npcnt As Integer, xcnt As Integer
  ...
  If Exist(UBPath$ + "UBLatLet.dat") Then
    ...
    Close
  End If
  If Exist(UBPath$ + "UBLatN21.dat") And LNType = 21 Then
    ReDim Latelet2(1) As UBLateNotice21LineType
    LaLe = FreeFile
    lenlate = Len(Latelet2(1))
    Open UBPath$ + "UBLatN21.dat" For Random Shared As LaLe Len = lenlate
    Get LaLe, 1, Latelet2(1)
    Close
  End If
...
'If lntype = 1 then
  If Grpt Then
    ...
    ElseIf LNType = 9 Then
      ...
      If PostBar = False Then ARptLglLateNotice17.Barcode1.Visible = False
      If AcctBar = False Then ARptLglLateNotice17.Barcode2.Visible = False
      ARptLglLateNotice17.Field39.Visible = False
      ARptLglLateNotice17.startrpt
    ElseIf LNType = 10 Then
      ARptLglLateNotice172.GetName ReportFile$
      If PostBar = False Then ARptLglLateNotice172.Barcode1.Visible = False
      If AcctBar = False Then ARptLglLateNotice172.Barcode2.Visible = False
      ARptLglLateNotice172.Field39.Visible = False
      ARptLglLateNotice172.startrpt
    End If
  Else
...
'Cedar Bluff VA
  ...
  Print #UBRpt,
  Print #UBRpt, Tab(35); "Acct:"; Using("#####", AcctNo&)
  If UBCustRec(1).CurrRevAmts(1) > 0 Then
  ...
  Print #UBRpt,
  Print #UBRpt, Tab(2); QPTrim$(fptxtMessage(0).Text)
  Print #UBRpt, Tab(2); QPTrim$(fptxtMessage(1).Text)
  Print #UBRpt, Tab(2); QPTrim$(fptxtMessage(2).Text)
  Print #UBRpt, Tab(2); QPTrim$(fptxtMessage(3).Text)
  Print #UBRpt, "~"
Return
...
'Late Notice Laser(Uses Blank Stock)BAR CODE PRINTABLE
  ...
  If npcnt <= 8 Then
    For WRevCnt = 1 To 9
    If UBCustRec(1).CurrRevAmts(WRevCnt) <> 0 Then
      ToPrint$ = ToPrint$ + "~" + Left$(UBSetUpRec(1).Revenues(WRevCnt).RevName, 10) + "~" + Using("#####.##", UBCustRec(1).CurrRevAmts(WRevCnt))
    End If
    Next
    For xcnt = (npcnt + 1) To 8 'fill out rest of spaces
        ToPrint$ = ToPrint$ + "~ ~ "
     Next
  Else
    If UBCustRec(1).CurrRevAmts(1) <> 0 Then
    ...
    tmprev# = 0
      ...
      If UBCustRec(1).CurrRevAmts(6) <> 0 Then
        tmprev# = tmprev + UBCustRec(1).CurrRevAmts(6)
      End If
        
      If UBCustRec(1).CurrRevAmts(9) <> 0 Then
        tmprev# = tmprev + UBCustRec(1).CurrRevAmts(9)
      End If
...
Return
PrnBladenLN:
  CutOff$ = QPTrim$(fptxtMessage(0).Text)
  ...
  Print #UBRpt,
  Print #UBRpt, QPTrim$(fptxtMessage(1).Text)
  Print #UBRpt, QPTrim$(fptxtMessage(2).Text)
  Print #UBRpt,
  Print #UBRpt, "~"
Return
Prn21LinetxtBlank:
  Totalamt# = Round#(UBCustRec(1).PrevBalance + UBCustRec(1).CurrBalance)
     Print #UBRpt, "Notice Date: "; LongNDate$;
     Print #UBRpt, Tab(50); RTrim(Latelet2(1).TxtLine1)
     Print #UBRpt, "Pay By Date: "; LongPDate$;
     Print #UBRpt, Tab(50); RTrim(Latelet2(1).TxtLine2)
     Print #UBRpt, RTrim(Latelet2(1).TxtLineH1);
     Print #UBRpt, Tab(50); RTrim(Latelet2(1).TxtLine3)
     Print #UBRpt, RTrim(Latelet2(1).TxtLineH2);
     Print #UBRpt, Tab(50); RTrim(Latelet2(1).TxtLine4)
     
     If UBCustRec(1).CurrRevAmts(1) <> 0 Then
      Print #UBRpt, UBSetUpRec(1).Revenues(1).RevName; Tab(34); Using("#####.##", UBCustRec(1).CurrRevAmts(1));
     End If
     Print #UBRpt, Tab(50); RTrim$(Latelet2(1).TxtLine5)
     If UBCustRec(1).CurrRevAmts(2) <> 0 Then
      Print #UBRpt, UBSetUpRec(1).Revenues(2).RevName; Tab(34); Using("#####.##", UBCustRec(1).CurrRevAmts(2));
     End If
     Print #UBRpt, Tab(50); RTrim$(Latelet2(1).TxtLine6)
     If UBCustRec(1).CurrRevAmts(3) <> 0 Then
      Print #UBRpt, UBSetUpRec(1).Revenues(3).RevName; Tab(34); Using("#####.##", UBCustRec(1).CurrRevAmts(3));
     End If
     Print #UBRpt, Tab(50); RTrim$(Latelet2(1).TxtLine7)
     If UBCustRec(1).CurrRevAmts(4) <> 0 Then
      Print #UBRpt, UBSetUpRec(1).Revenues(4).RevName; Tab(34); Using("#####.##", UBCustRec(1).CurrRevAmts(4));
     End If
     Print #UBRpt, Tab(50); RTrim(Latelet2(1).TxtLine8)
        tmprev# = 0
        For PCnt = 6 To 15
          If UBCustRec(1).CurrRevAmts(PCnt) <> 0 Then
            tmprev# = tmprev# + UBCustRec(1).CurrRevAmts(PCnt)
        ...
        If tmprev# <> 0 Then
          tmprev# = tmprev#
          tmprev# = tmprev# + UBCustRec(1).CurrRevAmts(5)
          Print #UBRpt, "Other";
          Print #UBRpt, Tab(34); Using("#####.##", tmprev#);
        Else
          If UBCustRec(1).CurrRevAmts(5) <> 0 Then
            Print #UBRpt, UBSetUpRec(1).Revenues(5).RevName; Tab(34); Using("#####.##", UBCustRec(1).CurrRevAmts(5));
          End If
        End If
        
     Print #UBRpt, Tab(50); RTrim(Latelet2(1).TxtLine9)
     Print #UBRpt, "Past Balance Due: "; Tab(32); Using("#####,#.##", Totalamt#);
     Print #UBRpt, Tab(50); RTrim(Latelet2(1).TxtLine10)
     Print #UBRpt, Tab(50); RTrim(Latelet2(1).TxtLine11)
     Print #UBRpt,
     Print #UBRpt,
     Print #UBRpt, Using("####", AcctNo);
     ...
     Print #UBRpt, Chr$(15);
     Print #UBRpt, RTrim$(Latelet2(1).TxtBLine1) '"All bills due upon presentation. A penalty of 10% will be added 30 days after billing date. Services will be discontinued if not"
     Print #UBRpt, RTrim$(Latelet2(1).TxtBLine2) '"paid within 35 days from billing date.  Receipt will not be returned unless requested.  A $10.00 charge on all returned checks."
     Print #UBRpt, Chr$(18);
     Rem uncondense here
     Print #UBRpt, "~" '    ToPrint$ = ToPrint$ + "~" + " ~ ~ "
...
DoLNMask:
  ...
  Case 7
'    If Bladen = True Then
'      GoSub PrnBladenLNMask
'    Else
      GoSub PrnStand24L3BxMask
'    End If
  Case 21
      GoSub Print21lineblankmask
  Case Else
    'NO MASK
  End Select
  Close UBRptA
...
PrnBladenLNMask:
  ...
  Print #UBRptA, "~"
Return
Print21lineblankmask:
  DeDate$ = FormatDateTime(Date$, vbLongDate)
     Print #UBRptA, "Notice Date: "; DeDate$;
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine1)
     Print #UBRptA, "Pay By Date: "; DeDate$;
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine2)
     Print #UBRptA, RTrim(Latelet2(1).TxtLineH1);
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine3)
     Print #UBRptA, RTrim(Latelet2(1).TxtLineH2);
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine4)
     Print #UBRptA, "Rev1name"; Tab(32); Using$("#####,#.##", 75#);
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine5)
     Print #UBRptA, "Rev2name"; Tab(32); Using("#####,#.##", 25);
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine6)
     Print #UBRptA, "Rev3name"; Tab(32); Using("#####,#.##", 50#);
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine7)
     Print #UBRptA, "Rev4name"; Tab(32); Using("#####,#.##", 5#);
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine8)
     Print #UBRptA, "Rev5name"; Tab(32); Using("#####,#.##", 4.5);
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine9)
     Print #UBRptA, "Past Balance Due: "; Tab(32); Using("#####,#.##", 159.5);
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine10)
     Print #UBRptA, Tab(50); RTrim(Latelet2(1).TxtLine11)
     Print #UBRptA,
     Print #UBRptA,
     Print #UBRptA, "1121";
     Print #UBRptA, Tab(10); "XXXXXXXXXXXXXXXXXXXXXXXXX"
     Print #UBRptA, Tab(10); "XXXXXXXXXXXXXXXXXXXXXXXXX"
     Print #UBRptA, Tab(10); "XXXXXXXXXXXXXXXXXXXXXXXXX"
     Print #UBRptA, Tab(10); "ANYWHERE          "; " "; "NC"; " "; "11111"
     Print #UBRptA,
     Rem condense here lines 19 & 20
     Print #UBRptA, Chr$(15);
     Print #UBRptA, RTrim$(Latelet2(1).TxtBLine1) '"All bills due upon presentation. A penalty of 10% will be added 30 days after billing date. Services will be discontinued if not"
     Print #UBRptA, RTrim$(Latelet2(1).TxtBLine2) '"paid within 35 days from billing date.  Receipt will not be returned unless requested.  A $10.00 charge on all returned checks."
     Print #UBRptA, Chr$(18);
     Rem uncondense here
     Print #UBRptA, "~" '    ToPrint$ = ToPrint$ + "~" + " ~ ~ "
Return
ExitLatePrint:

End Sub
'Middletown
'Print #UBRpt, Tab(3); "To: "; UBCustRec(1).CustName; "    "; LongNDate$
'Print #UBRpt,
'Print #UBRpt, Tab(3); "Our records indicate your water and sewer bill has not yet been"
'Print #UBRpt, Tab(3); "paid.  Please be advised that if the amount indicated below is not"
'Print #UBRpt, Tab(3); "paid within five (5) working days from the date of this notice, your"
'Print #UBRpt, Tab(3); "water service will be discontinued and there will be a $20.00 charge"
'Print #UBRpt, Tab(3); "to turn the water on again.  Bills must be paid before  NOON  on the"
'Print #UBRpt, Tab(3); "cutoff date below.  AS DETERMINED BY TOWN COUNCIL, THERE WILL BE NO"
'Print #UBRpt, Tab(3); "EXCEPTIONS TO THE CUTOFF DATE.  ALL BILLS MUST BE PAID IN FULL."
'Print #UBRpt,
'Print #UBRpt, Tab(53); "Water"; Tab(63); Using; "####.##"; UBCustRec(1).CurrRevAmts(1)
'Print #UBRpt, Tab(53); "Sewer"; Tab(63); Using; "####.##"; UBCustRec(1).CurrRevAmts(2)
'Print #UBRpt, Tab(53); "Surcharge"; Tab(63); Using; "####.##"; UBCustRec(1).CurrRevAmts(3)
'Print #UBRpt, Tab(3); Using; "####"; AcctNo&;
'Print #UBRpt, Tab(13); UBCustRec(1).CustName; Tab(53); "Misc"; Tab(63); Using; "####.##"; UBCustRec(1).CurrRevAmts(5)
'Print #UBRpt, Tab(13); UBCustRec(1).ADDR1; Tab(53); "Sub Total"; Tab(63); Using; "####.##"; Round#((UBCustRec(1).CurrBalance + UBCustRec(1).PrevBalance) - UBCustRec(1).CurrRevAmts(4))
'Print #UBRpt, Tab(13); Left$(UBCustRec(1).CITY, 20); " "; UBCustRec(1).STATE; " "; Zip$; Tab(53); "Penalty"; Tab(63); Using; "####.##"; UBCustRec(1).CurrRevAmts(4)
'Print #UBRpt, Tab(53); "Total"; Tab(63); Using; "####.##"; Round#(UBCustRec(1).CurrBalance + UBCustRec(1).PrevBalance)
'Print #UBRpt, Tab(3); "Cut Off Date: "; PDate$
'Print #UBRpt, Tab(45); "Patricia Mullins - Town Clerk"
'Print #UBRpt,
'Print #UBRpt,
'
