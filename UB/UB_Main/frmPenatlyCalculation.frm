...
Begin VB.Form frmPenaltyCalculation 
   ...
   Begin LpLib.fpCombo fpcboBalType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmPenatlyCalculation.frx":0BF8
   End
   Begin LpLib.fpCombo fpcboRevenues 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmPenatlyCalculation.frx":0F1B
   End
  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        UBLog "Closed via PenaltyCalc by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
Private Sub Form_Load()
  ...
  If QPTrim$(UBBillSetup(1).ChargeOn) = "Previous Balance" Then
    fpcboBalType.ListIndex = 1
  ElseIf QPTrim$(UBBillSetup(1).ChargeOn) = "Total Balance" Then
  ...
  If QPTrim$(UBBillSetup(1).GreatLess) = "L" Then
    fpcboWhichever.ListIndex = 1
  ElseIf QPTrim$(UBBillSetup(1).GreatLess) = "G" Then
...
Private Function CheckFields()
  ...
  ElseIf Len(QPTrim$(txtDescription.Text)) = 0 Then
    MsgText(3) = "You Have Not Entered a Description"
...
Private Sub PenaltyProcess()
  ...
  Dim Pct2ChgFld  As Integer, TennFlag As Boolean, PolockFlag As Boolean, TalogaFlag As Boolean
  Dim HamFlag As Boolean, TuckFlag As Boolean, CashFlag As Boolean, DrexelFlag As Boolean
  ...
  Dim PenFile As String, PHandle As Integer, CHandle As Integer, cnttx As Integer
  ...
  Dim hand2 As Integer, ExitFlag As Boolean, Nmn As String, Act As Long
  Dim curtax As Double, prevtax As Double, totTax As Double
  UBLog PWUser$ + " IN: Create Penalty File (CPF)"
  On Error GoTo ERList
  'Put this on error code incase balances have funky stuff *&*&*&*&*&
  'will display the acct and name see ERList at bottom of code........
  Pct2ChgFld = 5
...
'    SunSetFlag = True
'  End If
  If InStr(TOWNNAME$, "TALOGA") > 0 Or InStr(TOWNNAME$, "DREXEL") > 0 Then
    TalogaFlag = True
  End If
  ...
  For cnt = 1 To MaxRevsCnt
    ...
    TaxAmt(cnt) = Round(UBSetUpRec(1).Revenues(cnt).TAXRATE)
  Next
...
'  ShowProcessingScrn "Calculating Penalty Charges"
  ...
  For CCnt& = 1 To NumCustRecs&
    ...
    Nmn = QPTrim(UBCustRec(1).CustName)
    Act = CCnt&
    If Not UBCustRec(1).DelFlag Then
      If UBCustRec(1).LATEFEE = "Y" Then
        ...
        ElseIf UBCustRec(1).Status <> "A" Then      'if they are not inactive
          GoTo SkipEm
        End If
' try other way to gracefully get out if strange balance - no work
' used on error instead -- see above....
'        dodo = UBCustRec(1).CurrBalance
'        If dodo < -1000000 Then 'Or UBCustRec(1).PrevBalance < -1000000 Then
'          GoTo ERList
'        End If
''''''''''''''''''''''''
        '05-01-97 fixed bug where CurrBalance+PrevBalance is <= 0
        If Round#(UBCustRec(1).CurrBalance + UBCustRec(1).PrevBalance) > 0 Then
    '&%&%& line below no work if use total bal for min rem out 10-28-04
...
ExitPenalty:
 ...
 ' Erase frm, Form$, Fld
  ...
  If ErFlag = False And PCnt > 0 Then
    MsgBox "Penalty Calculation Is Complete.", vbOKOnly, "Procedure Complete"
    UBLog PWUser$ + "OUT: Create Penalty File."
  Else
    MsgBox "No Records To Process.", vbOKOnly, "Procedure Ended"
    UBLog PWUser$ + "OUT: Penalty File NOT Created."
  End If
...
prevtax = 0
    For cnttx = 1 To 14
      If UBCustRec(1).CurrRevAmts(cnttx) > 0 Then
        curtax = Round(curtax + Round((UBCustRec(1).CurrRevAmts(cnttx) * TaxAmt(cnttx))))
      End If
     
'      If UBCustRec(1).PrevRevAmts(cnttx) > 0 Then
'        prevtax = Round(prevtax + Round((UBCustRec(1).PrevRevAmts(cnttx) * TaxAmt(cnttx))))
'      End If
    Next
'    If UsePrevFlag And Not UseCurrFlag Then
'    totTax# = Round(prevtax)
'    ElseIf UseCurrFlag And Not UsePrevFlag Then
      totTax# = Round(curtax)
'    ElseIf UsePrevFlag And UseCurrFlag Then
'      totTax# = Round(curtax + prevtax)
'    End If
    PenBal# = Round(PenBal# - totTax)
Return
...
ERList:
  ...
  Unload FrmShowPctComp
    ...
    MsgText(0) = "ERROR:"
    MsgText(1) = "Problem with Account Balance!"
    MsgText(2) = "Number: " + Str(Act)
    MsgText(3) = "Name: " + Nmn
    MsgText(4) = "Check the Account then retry,"
    MsgText(5) = "Or Contact Software Support."
    GetOKorNot MsgText(), True
    Close
    Unload Me
End Sub
