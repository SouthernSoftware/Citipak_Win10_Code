  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        UBLog "Closed via RptPaymSum by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpPaymentSummary
End Sub
...
Private Sub PaymentSumReport()
  ...
  Dim taxnum As Long, NonTaxtot As Double, Taxedtot As Double
  ReDim RevenueName$(15)
  ReDim Revenues(1 To 15) As Double
  ReDim NTXRevenues(1 To 15) As Double
  ReDim TXRevenues(1 To 15) As Double
        ...
        For RevCnt = 1 To 15
          If Not TaxExempt Then
            If TaxRates(RevCnt) > 0 Then
              ...
              TXRevenues(RevCnt) = Round#(TXRevenues(RevCnt) + (UBTransRec(1).RevAmt(RevCnt) - Tax#))
              Taxedtot# = Round#(Taxedtot# + (UBTransRec(1).RevAmt(RevCnt) - Tax#))
            Else
              NTXRevenues(RevCnt) = Round#(NTXRevenues(RevCnt) + UBTransRec(1).RevAmt(RevCnt))
              NonTaxtot# = Round#(NonTaxtot# + UBTransRec(1).RevAmt(RevCnt))
            End If
          Else
            NTXRevenues(RevCnt) = Round#(NTXRevenues(RevCnt) + UBTransRec(1).RevAmt(RevCnt))
            NonTaxtot# = Round#(NonTaxtot# + UBTransRec(1).RevAmt(RevCnt))
...
DoDetailedRptHeader3:
  ...
  Print #UBRpt,
  Print #UBRpt, "    Source             NonTaxed Rev      Taxed Rev            Tax      Total Rev"
Return

DoDetailedRptFooter3:
  Print #UBRpt, Dash80$
  For cnt = 1 To MaxRevenue
    Print #UBRpt, Tab(5); RevenueName$(cnt); Tab(23); Using("$##,###,###.##", NTXRevenues(cnt)); Tab(38); Using("$##,###,###.##", TXRevenues(cnt)); Tab(53); Using("$##,###,###.##", TaxAmt(cnt)); Tab(68); Using("$##,###,###.##", Revenues(cnt))
    TotalTrans# = Round#(TotalTrans# + Revenues(cnt))
    TaxTotal# = Round#(TaxTotal# + TaxAmt(cnt))
  Next
  Print #UBRpt, Dash80$
  Print #UBRpt, "Revenue Totals: "; Tab(23); Using("$##,###,###.##", NonTaxtot#); Tab(38); Using("$##,###,###.##", Taxedtot#); Tab(53); Using("$##,###,###.##", TaxTotal#); Tab(68); Using("$##,###,###.##", TotalTrans#)
  Print #UBRpt,
  'Print #UBRpt, "Revenue Grand Total: "; Tab(25);
  Print #UBRpt, "Total Payments: "; Tab(20); Using("######", TransCnt&); Tab; 28; Using("######", taxnum)
  Print #UBRpt, FF$
Return
End Sub
...
SkipEm:
  ...
  If TransCnt& > 0 Then
    Load frmLoadingRpt
    frmLoadingRpt.setwherefrom frmRptPaymSum
...
DoDetailedRptFooter3:
  'ToPrint$ = ""
  For cnt = 1 To MaxRevenue
    Print #UBRpt, RevenueName$(cnt) + "~" + Using("$##,###,###.##", Revenues(cnt)) + "~" + Using("$##,###,###.##", NTXRevenues(cnt)) + "~" + Using("$##,###,###.##", TXRevenues(cnt)) + "~" + Using("$##,###,###.##", TaxAmt(cnt))
    TotalTrans# = Round#(TotalTrans# + Revenues(cnt))
    TaxTotal# = Round#(TaxTotal# + TaxAmt(cnt))
  Next
