...
Private UBDrftCode As String
Private UBLineOnenumber As String
Private UBLineFivenumber As String
Private UBBankAcctNum As String
Private UBRouteNum As String
...
Private Sub OpenDrftCode()
  ...
  Dim CodeTemp As CitiUBDraftCodeType
  Tempfile = FreeFile
  Open "UBDrftCode.dat" For Random Shared As Tempfile Len = Len(CodeTemp) ' Len = lentemp
  Get Tempfile, 1, CodeTemp
  UBDrftCode$ = QPTrim(CodeTemp.DraftCode)
  If Val(UBDrftCode$) = 0 Then UBDrftCode$ = "200"
  UBLineOnenumber$ = CodeTemp.Line1number
  UBLineFivenumber$ = CodeTemp.Line5number
  UBBankAcctNum$ = CodeTemp.BankAcctNum
  UBRouteNum$ = CodeTemp.RouteNum
   If Val(UBLineOnenumber$) = 0 Then UBLineOnenumber$ = ""
  If Val(UBLineFivenumber$) = 0 Then UBLineFivenumber$ = ""
  If Val(UBRouteNum$) > 0 Then QPTrim (UBRouteNum$)
  If Val(UBBankAcctNum$) > 0 Then QPTrim (UBBankAcctNum$)
  Close
End Sub
...
ProcessDraft:
  ...
  If Val(UBRouteNum$) = 0 Then UBRouteNum$ = QPTrim$(UBDraftRec(1).BANKDEST)
...
DraftExit:
  ...
  frmDraftMsg.cmdOKOnly.Enabled = True
Exit Sub
...
DraftProcessStep1:
  ...
  If Len(UBLineOnenumber$) > 0 Then
    UBDraftRecord1(1).Field4 = UBLineOnenumber$
  Else
...
DraftProcessStep2:
  ...
  UBDraftRecord5(1).Field2 = UBDrftCode$
  UBDraftRecord5(1).Field3 = Left$(TOWNNAME$, 16)
  UBDraftRecord5(1).Field4 = "                    "
  If Len(UBLineFivenumber$) > 0 Then
    UBDraftRecord5(1).Field5 = UBLineFivenumber$
  Else
...
'HERE
  ...
  If BalRecFlag Then
    ...
    Else
      nme$ = Left$(nme$, 22)
    End If
    If Val(UBBankAcctNum$) > 0 Then
      CompanyAcct$ = UBBankAcctNum$
    End If
    ...
    UBDraftRecord6(1).Field3 = Left$(UBRouteNum$, 8)
    UBDraftRecord6(1).Field4 = Right$(UBRouteNum$, 1)
    UBDraftRecord6(1).Field5 = Left$(BankAcct$, 17)
    UBDraftRecord6(1).Field6 = BillAmt$   ' All zero's for Prenote
    UBDraftRecord6(1).Field7 = AcctNumber$
    UBDraftRecord6(1).Field8 = UCase$(nme$)
    ...
    Put DraftFileNum, Counter, UBDraftRecord6(1)
    hashh# = hashh# + Val(Left$(UBRouteNum$, 8))
    Number = Number + 1
  End If
...
DraftProcessStep4:
  ...
  UBDraftRecord8(1).Field2 = UBDrftCode$
  UBDraftRecord8(1).Field3 = Trace$
  UBDraftRecord8(1).Field4 = hash$
  If BalRecFlag Then   'Balancing Debits and Credits
    UBDraftRecord8(1).Field5 = TotalAmount$
    UBDraftRecord8(1).Field6 = TotalAmount$
  Else 'debits only
    UBDraftRecord8(1).Field5 = TotalAmount$
    UBDraftRecord8(1).Field6 = "000000000000"
  End If
...
DraftProcessStep5:
  ...
  If BalRecFlag Then  'Balancing Debits and Credits
    UBDraftRecord9(1).Field6 = TotalAmount$
    UBDraftRecord9(1).Field7 = TotalAmount$
  Else 'debits only
    UBDraftRecord9(1).Field6 = TotalAmount$
