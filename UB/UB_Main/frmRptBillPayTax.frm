...
Begin VB.Form frmRptBillPayTax 
   ...
   Begin LpLib.fpCombo fpcboRptOn 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptBillPayTax.frx":0BF8
   End
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptBillPayTax.frx":0EEF
   End
  ...
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      If MsgBox("Are You Sure You Wish To Close The Program?", vbYesNo, "Close?") = vbNo Then
        Cancel = True
      Else
        UBLog "Closed via RptBillPayTax by " + PWUser$
        CitiTerminate
      End If
    End If
  End If
...
Private Sub Form_Load()
  ...
  Me.HelpContextID = hlpBillPaymentTax
End Sub
            ...
            For RCnt = 1 To LastRev
              If TaxRates(RCnt) > 0 Then
                If UBTransRec(1).TransType = TranUtilityBill Then
                  Tax# = Round#(UBTransRec(1).RevAmt(RCnt) * (TaxRates(RCnt)))
                Else
...
'  ViewPrint "UBTAXRPT.RPT", "Transaction Tax Report"
    Load frmLoadingRpt
    frmLoadingRpt.setwherefrom frmRptBillPayTax
