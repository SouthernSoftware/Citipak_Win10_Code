...
Begin VB.Form frmRptSCnsmpExtravaganza 
   BackColor       =   &H008F8265&
   ...
   ClientTop       =   540
   ClientWidth     =   12192
   Icon            =   "frmRptSCnsmpRateCodeB.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   8640
   ...
   Begin LpLib.fpList fplstGCodes 
      ...
      TextTipMultiLine=   0
      ColDesigner     =   "frmRptSCnsmpRateCodeB.frx":08CA
   End
   Begin LpLib.fpList fplstRates 
      ...
      ResizeRowToFont =   0   'False
      TextTipMultiLine=   0
      ColDesigner     =   "frmRptSCnsmpRateCodeB.frx":0C4A
   End
   Begin LpLib.fpCombo fpcboRptType 
      ...
      EditAlignH      =   0
      EditAlignV      =   0
      ColDesigner     =   "frmRptSCnsmpRateCodeB.frx":0F9E
   End
   ...
   End
End
Attribute VB_Name = "frmRptSCnsmpExtravaganza"
...
Private Sub ConsumpUnitStep(RptType)
  ...
  If Not Doall Then
    ...
    ReDim TblBreak(1 To NumOfratesrpt, 10) As Long
    ReDim TblUnitVal(1 To NumOfratesrpt, 10) As Double
    ReDim TotalConsp(1 To NumOfratesrpt, 10) As Double
    ReDim TotalCust(1 To NumOfratesrpt, 10) As Long
  Else
    ...
    ReDim TblBreak(1 To NumOfRates, 10) As Long
    ReDim TblUnitVal(1 To NumOfRates, 10) As Double
    ReDim TotalConsp(1 To NumOfRates, 10) As Double
    ReDim TotalCust(1 To NumOfRates, 10) As Long
  End If
 ...
 ' RCName$ = "Ratecds.lst"
       ...
       For Tnt = 1 To 10
        If UBRateTbls(PCnt).TblBreaks(Tnt).UNITS >= 0 Then
          If UBRateTbls(PCnt).TblBreaks(Tnt).UNITS > 0 Then
            Greater = True
          End If
          If (UBRateTbls(PCnt).TblBreaks(Tnt).UNITS = 0) And (Greater = True) Then
            MaxStep(PCnt) = Tnt
            TblBreak&(PCnt, Tnt) = 99999999
            TblUnitVal#(PCnt, Tnt) = UBRateTbls(PCnt).TblBreaks(Tnt - 1).UNITAMT
            Exit For
          End If
          TblBreak&(PCnt, Tnt) = UBRateTbls(PCnt).TblBreaks(Tnt).UNITS
          TblUnitVal#(PCnt, Tnt) = UBRateTbls(PCnt).TblBreaks(Tnt).UNITAMT
          MaxStep(PCnt) = Tnt
        Else
          MaxStep(PCnt) = Tnt
          TblBreak&(PCnt, Tnt) = 99999999
        ...
        For Tnt = 1 To 10
          If UBRateTbls(RateRec).TblBreaks(Tnt).UNITS >= 0 Then
            If UBRateTbls(RateRec).TblBreaks(Tnt).UNITS > 0 Then
              Greater = True
            End If
            If (UBRateTbls(RateRec).TblBreaks(Tnt).UNITS = 0) And (Greater = True) Then
              MaxStep(RateRec) = Tnt
              TblBreak&(RateRec, Tnt) = 99999999
              TblUnitVal#(RateRec, Tnt) = UBRateTbls(RateRec).TblBreaks(Tnt - 1).UNITAMT
              Exit For
            End If
            TblBreak&(RateRec, Tnt) = UBRateTbls(RateRec).TblBreaks(Tnt).UNITS
            TblUnitVal#(RateRec, Tnt) = UBRateTbls(RateRec).TblBreaks(Tnt).UNITAMT
            MaxStep(RateRec) = Tnt
          Else
            MaxStep(RateRec) = Tnt
            TblBreak&(RateRec, Tnt) = 99999999
  ...
  If RptType = 0 Then
    ...
    ARptSCnsmpRate.FldRptInfo = "(Rates - " + RptInfo$ + ") (Customers - " + RptInfo2$ + ")"
...
SkipThisMtrB:
        ...
        If (TMeterConsp# >= 0) And (ValidCustomer = 1) Then
          ...
          For LL = 1 To MaxStep(RateRec)
            If TMeterConsp# >= TblBreak&(RateRec, LL - 1) And TMeterConsp# <= TblBreak&(RateRec, LL) Then
              TotalConsp#(RateRec, LL) = TotalConsp#(RateRec, LL) + TMeterConsp#
          ...
          If NonUpdated = 1 Then
            TotalConsp#(RateRec, MaxStep(RateRec)) = TotalConsp#(RateRec, MaxStep(RateRec)) + TMeterConsp#
...
ElseIf RptType = 1 Then
  For LL = 1 To MaxStep(RateRec)
    Print #UBRpt, "Step # "; LL;
    Print #UBRpt, Tab(12); "From "; TblBreak&(RateRec, LL - 1); " to "; TblBreak&(RateRec, LL)
    Print #UBRpt, "Consumption = "; Tab(18); Using("#########,#", TotalConsp#(RateRec, LL));
    Print #UBRpt, Tab(29); "  # of Cust = "; Using("#####,#", TotalCust(RateRec, LL));
    If TblUnitVal#(RateRec, LL) > 0 Then
    'ok
    Else
      TblUnitVal#(RateRec, LL) = 0
    ...
    BigTotCust = BigTotCust + TotalCust(RateRec, LL)
    If TotalCust(RateRec, LL) > 0 Then
      Print #UBRpt, Tab(51); Using("###,###,###.##", Round#(TotalConsp#(RateRec, LL) * TblUnitVal#(RateRec, LL))); Tab(68); Using("  ###,###.##", Round#(NMinAMT# * TotalCust(RateRec, LL)))
  ...
  Print #UBRpt, Tab(51); Using("###,###,###.##", BigUTotal#); Tab(67); Using("##,###,###.##", MinGT#)
  GBBigUTotal# = Round#(GBBigUTotal# + BigUTotal#)
  GBMinGT# = Round#(GBMinGT# + MinGT#)
  GBGTMeterConsp# = Round#(GBGTMeterConsp# + GTMeterConsp#)
 ' GBCustTot = GBCustTot + BigTotCust
