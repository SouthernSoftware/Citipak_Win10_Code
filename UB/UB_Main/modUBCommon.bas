...
End Function
Public Function ZipFormat$(Zip$)
  Dim ZipLen As Integer, ZipVal As Integer, DashPos As Integer
  Dim cnt As Integer, Dif As Double, newzip As String
  Do While DashPos
    Zip$ = Left$(Zip$, DashPos - 1) + Mid$(Zip$, DashPos + 1)
    DashPos = InStr(Zip$, "-")
  Loop
  ZipLen = Len(Zip$)
  If ZipLen = 9 Then
    newzip = Mid$(Zip$, 1, 5) + "-" + Mid$(Zip$, 6, 4)
  Else
    newzip = Zip$
  End If
...
Public Sub LoadUBBillLetterFile(UBBillLetterRec() As UBBillLetterType, UBBillLetterlen)
  ...
  End If
End Sub
Public Sub LoadUBBill21LineFile(UBBill21LineRec() As UBBill21LineType, UBBill21Linelen)
  Dim Handle As Integer
  UBBill21Linelen = Len(UBBill21LineRec(1))            'use the length as an error flag
  If Exist(UBPath$ + "UBBil21L.DAT") Then
    Handle = FreeFile
    Open UBPath$ + "UBBil21L.DAT" For Random Shared As Handle Len = UBBill21Linelen    'open data file
    If LOF(Handle) > 0 Then
      Get #Handle, 1, UBBill21LineRec(1)
    End If
    Close Handle
  End If
End Sub
...
ErrorCatch:
  Select Case Err
    Case Is <> 53
      ...
      UBLog ("KillfileD error code is " + Str$(Err) + " .")
      UBTerminate
    Case 53
      Resume ExitFillFile
  End Select
...
Public Function ExistD(FileName$)
  ...
  Else
    ExistD = False
      UBLog ("ExistD NOT-File " + FileName$ + "##@ Does not exist.")
      Kill FileName$
  End If
End Function
Public Function NewExist(FileName$)
    On Local Error GoTo NGExit
    Dim FileHandle As Integer
    Dim FileSize As Long
    FileHandle = FreeFile
...
'  End If
  ...
  Else
   ...
   ' If UCase(FileName$) <> UCase(UBPath$ + "UBCust.Dat") Then  'Added this to see if corrects problem with custfile getting deleted. 10/05/2007 -PS
      UBLog ("Exist NOT-File " + FileName$ + "##@ Does not exist.")
      'Kill FileName$
   ' End If
  End If
'  On Local Error GoTo 0
...
ErrorCatch:
  Select Case Err
    Case Is <> 53
      MsgBox ("File deletion permission denied " + Str$(Err) + " . PLEASE CONTACT SOUTHERN SOFTWARE @ 1-800-842-8190."), vbOKOnly
      UBLog ("Killfile error code is " + Str$(Err) + " .")
      UBTerminate
    Case 53
      Resume ExitFillFile
  End Select
...
Public Function FileSize(FileName$) As Long
  ...
  Else
    FileSize = 0
    UBLog ("Exist NOT-File " + FileName$ + "Filesize returns 0") 'also added 10/05/2007
  End If
End Function
...
'
  ...
  If PrevTranRec& > 0 Then
    Do While PrevTranRec& > 0
      ...
      GoSub GetTransType
      Mid$(Build, 19) = TType$
      Mid$(Build, 47) = Using("########.##", UBTranRec(1).Transamt, True)
...
'      Mid$(Build, 55) = Str$(PrevTranRec&)
      Mid$(Build, 59) = Using("#########.##", UBTranRec(1).RunBalance, True)
...
'WorkOrderRec(1).CompletedDate
  If rptopt <> 1 Then
    ...
    Print #RptHandle, " "
    Print #RptHandle, " "
    Print #RptHandle, "    Work Order#: "; Using("######", TransRecNum&); Tab(30); "Date Issued: "; Num2Date$(WorkOrderRec(1).ENTRYDATE)
  ...
  Else
    ...
    ToPrint$ = ToPrint$ + Using("######", TransRecNum&) + "~"
...
Public Function GetAllowEBills() As Boolean
  Dim UBBillSetUpRec(1) As UBBillSetupType
  Dim UBBillSetuplen As Integer
  Dim blnUseEBill  As Boolean
  Dim Handle As Integer
  blnUseEBill = False
  UBBillSetuplen = Len(UBBillSetUpRec(1))            'use the length as an error flag
  If Exist(UBPath$ + "UBBilSET.DAT") Then
    ...
    Close Handle
  End If
  If UBBillSetUpRec(1).PSNBills = "Y" Then
    blnUseEBill = True
  Else
    blnUseEBill = False
  End If
  GetAllowEBills = blnUseEBill
End Function
