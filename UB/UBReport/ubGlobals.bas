...
Global BillYears() As Integer
Global RevList() As String

Type UBCustRevTotalsType
  CustRec As Long
  CurrBal As Double
  PrevBal As Double
  TotlBal As Double
  RevName As String
  CurrRev(1 To 14) As Double
  PrevRev(1 To 14) As Double
  RevTotal(1 To 14) As Double
End Type

Type UBCustRevenueTotalsType
  CustRec As Long
  CustName As String * 50
  BilledCnt As Integer
  RevTotal(1 To 14) As Double
  PctOfSales As Double
End Type
...
'END OF Quick Screen Form
    ...
    NewNotes      As Integer
    DPCode        As String * 2
    PrnBillYN     As String * 1
    FillPad       As String * 111
    ChkByte       As String * 1
End Type
...
Type BillOutRecType   '
   ...
   PenaltyAmt             As String * 15
   WithPenAmt             As String * 15
   CrLf                   As String * 2
End Type
...
Type BillOutRec3Type   '
   ...
   PastDueDate            As String * 10
   DraftDate              As String * 10
   MsgLine1               As String * 45     'was 25
   MsgLine2               As String * 45     'was 25
   MsgLine3               As String * 45     'was 25
   MsgLine4               As String * 45     'was 25
   PenaltyAmt             As String * 15
   WithPenAmt             As String * 15
   CrLf                   As String * 2
End Type
...
Type UBBadgerV2RecType
  Fill1          As String * 8   ' 1 - 8 fill spaces
  CustName       As String * 20  ' 9 - 28
  ServAddr       As String * 20  ' 29 - 48 lj
  MtrNum1        As String * 10  ' 49 - 58  Radio or ERT# from MtrInfo field
  Fill2          As String * 4   ' 59 - 62 n/a fill spaces
  Status         As String * 1   ' 63 - 63 Acct Status 'A' or 'I'
  MtrServ        As String * 1   ' 64 - 64 Number of meters
  MTRType        As String * 1   ' 65 - 65 W for water meter
  DNI            As String * 2   ' 66 - 67 Dialog ID    'space fill
  MtrSerN        As String * 9   ' 68 - 76 Meter Serial Number
  Fill3          As String * 1   ' 77 - 77 N/A fill space
  HiRead         As String * 9   ' 78 - 86 High reading limit 78-86
  LoRead         As String * 9   ' 87 - 95 Low reading limit  87-95
  CurrRead       As String * 9   ' 96 - 104 Current reading or space fill  96-104
  ReadTime       As String * 8   ' 105 - 112 read time  space fill
  ReadType       As String * 1   ' 113 - 113 space fill
  TamperMode     As String * 1   ' 114 - 114 space fill
  CmntCode       As String * 2   ' 115 - 116 space fill
  RouteNum       As String * 4   ' 117 - 120 space fill
  Account        As String * 15  ' 121 - 135 location  '01-000001
  ReadDate       As String * 8   ' 136 - 143 Read date ddmmyyyy
  ModType        As String * 1   ' 144 - 144 C for manuals Z for radios
  Latitude       As String * 12  ' 145 - 156 space fill
  Fill4          As String * 6   ' 157 - 162 space fill
  MtrRes         As String * 2   ' 163 - 164 test circle
  SEQNUMB        As String * 6   ' 165 - 170 Read sequence number
  MfgModel       As String * 20  ' 171 - 190 space fill
  UserUse        As String * 30  ' 191 - 220 we will use for acct number and meter count
  ReadID         As String * 3   ' 221 - 223 space fill
  ReadCo1        As String * 2   ' 224 - 225 space fill
  ReadCo2        As String * 2   ' 226 - 227 space fill
  ReadCo3        As String * 2   ' 228 - 229 space fill
  MtrLoc         As String * 9   ' 230 - 238 space fill
  MMIReadCode    As String * 2   ' 239 - 240 space fill
  Longitude      As String * 12  ' 241 - 252 space fill
  Fill5          As String * 2   ' 253 - 254 space fill
  CrLf           As String * 2   ' 255 - 256 Carriage return line feed sequence
End Type
...
Type UBBadgerRecType
  ...
  UserField      As String * 30  'we will use for acct number and meter count
...
End Type
'for the new HH type added for Badger Y version3 same as G used diff type so will distinguish the diff
Type UBBadgerV3RecType
  Fill1          As String * 8   ' 1 - 8 fill spaces
  CustName       As String * 20  ' 9 - 28
  ServAddr       As String * 20  ' 29 - 48 lj
  MtrNum1        As String * 10  ' 49 - 58  NEW Radio or ERT# from MtrInfo field
  Fill2          As String * 4   ' 59 - 62 n/a fill spaces
  Status         As String * 1   ' 63 - 63 Acct Status 'A' or 'I'
  MtrServ        As String * 1   ' 64 - 64 Number of meters
  MTRType        As String * 1   ' 65 - 65 W for water meter
  DNI            As String * 2   ' 66 - 67 Dialog ID    'space fill
  MtrSerN        As String * 9   ' 68 - 76 NEW Meter Serial Number
  Fill3          As String * 1   ' 77 - 77 N/A fill space
  HiRead         As String * 9   ' 78 - 86 High reading limit 78-86
  LoRead         As String * 9   ' 87 - 95 Low reading limit  87-95
  CurrRead       As String * 9   ' 96 - 104 Current reading or space fill  96-104
...
'**************************************************************
  ...
  NoteStatus    As String * 1  'T=Temp  P=Perm
End Type
''This is for Performance Meters HH's
Type UBPMHHRouteRecType        'This is 0n file to send to HH and get from HH
  HeaderID     As String * 2    'HeaderID - RI
  CycleNum     As String * 2    'Billing Cycle
  RouteNum     As String * 6    'Book Number
  CrLf         As String * 2    'Carriage return line feed sequence
End Type                        'Total Length 12

Type UBPMHHSendRecType
  RecordID     As String * 2    'RecordID - MI
  AccountNum   As String * 20   'Customer Account Number
  MeterNum     As String * 12   'Meter Serial Number
  SeqNum       As String * 6    'Location w/out book or Read Sequence number
  ReadTypeCode As String * 4    'Water, Gas or other
  HighRead     As String * 10   'High Reading
  LowRead      As String * 10   'Low Reading
  LocatnCode   As String * 4    'Not Required
  InstrCode    As String * 4    'Reader inst - We won't use
  AcctCode     As String * 4    'A for Active, I for Inactive
  Address      As String * 40
  Name         As String * 20   'Full Name
  PrevRead     As String * 10   'Current Reading
  PReadDate    As String * 4    'MMDD Last read date
  NumDials     As String * 2    'This is required, will have to use usercode?
  DecimalPL    As String * 1    'Decimal Point Location Used for Electric meters
  Compound     As String * 10   'This won't use
  UID          As String * 10   'ID information field
  Longitude    As String * 12
  Latitude     As String * 12
  Filler       As String * 10
  CrLf         As String * 2    'Carriage return line feed sequence
End Type                        'Total Length 209

Type UBPMHHGetRecType
  RecordID     As String * 2    'RecordID - MI
  AccountNum   As String * 20   'Customer Account Number
  MeterNum     As String * 12   'Meter Serial Number
  SeqNum       As String * 6    'Location w/out book or Read Sequence number
  ReadTypeCode As String * 4    'Water, Gas or other
  AcctCode     As String * 4    'A for Active, I for Inactive
  Reading      As String * 10   'New Reading
  ReadDate     As String * 8    'Reading Date - YYYYMMDD
  ReadTime     As String * 6    'Time HHMMSS
  TroubleCode  As String * 4    'Not used by us
  Address      As String * 40
  Name         As String * 20   'Full Name
  PrevRead     As String * 10   'Prev Reading
  Consumption  As String * 10
  Compound     As String * 10   'This won't use
  UserID       As String * 10
  UID          As String * 10   'ID information field
  Longitude    As String * 12
  ...
  Filler       As String * 10
  CrLf         As String * 2    'Carriage return line feed sequence
End Type                        'Total Length 212

'this type is a file (UBDrftCode.dat) kept in the citipak folder to indicate what type of code is used in the ub draft file
Type CitiUBDraftCodeType
  DraftCode  As String * 3
  Line1number As String * 10
  Line5number  As String * 10
  BankAcctNum As String * 17
  RouteNum As String * 9
End Type
