  ...
  Open "UBRevInfo.txt" For Output As #2
    ...
    For LopCnt = 1 To RecCnt   'RecCnt
      ...
      If UBCust.DelFlag = 0 Then
        'If UBCust.CurrBalance < 0 Or UBCust.PrevBalance < 0 Then
          If DidCnt > 1 Then
          ...
          CustRevs(DidCnt).PrevBal = Round#(UBCust.PrevBalance)
          CustRevs(DidCnt).TotlBal = Round#(CustRevs(DidCnt).PrevBal + CustRevs(DidCnt).CurrBal)
          For zzCnt = 1 To RevCnt
            CustRevs(DidCnt).CurrRev(zzCnt) = Round#(UBCust.CurrRevAmts(zzCnt))
            CustRevs(DidCnt).PrevRev(zzCnt) = Round#(UBCust.PrevRevAmts(zzCnt))
            CustRevs(DidCnt).RevTotal(zzCnt) = Round#(CustRevs(DidCnt).CurrRev(zzCnt) + CustRevs(DidCnt).PrevRev(zzCnt))
          Next
          DidCnt = DidCnt + 1
        End If
      'End If
    ...
    For LopCnt = 1 To DidCnt - 1
        GotNeg = False
        GotPos = False
        For ChkCnt = 1 To RevCnt
          If CustRevs(LopCnt).CurrRev(ChkCnt) > 0 Then GotPos = True
          If CustRevs(LopCnt).CurrRev(ChkCnt) < 0 Then GotNeg = True
        Next
        If GotPos And GotNeg Then
          Print #2, CustRevs(LopCnt).CustRec, CustRevs(LopCnt).TotlBal
          For zzCnt = 1 To RevCnt
            Print #2, RevList(zzCnt), CustRevs(LopCnt).RevTotal(zzCnt)
          Next
          Print #2,
          PCnt = PCnt + 1
        End If
...
Private Sub RenOldFile()
  On Error Resume Next
  Name "UtilityBillingData\UBDeposit.txt" As "UtilityBillingData\UBDeposit.ORG"
  On Error GoTo 0
End Sub
