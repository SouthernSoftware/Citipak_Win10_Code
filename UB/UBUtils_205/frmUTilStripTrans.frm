...
Begin VB.Form frmUtilStripTrans 
   ...
   End
   Begin fpBtnAtlLibCtl.fpBtn fpBtnRemoveInv 
      Height          =   570
      Left            =   4440
      TabIndex        =   16
      Top             =   8010
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUTilStripTrans.frx":1184
   End
   Begin fpBtnAtlLibCtl.fpBtn btnRemoveSpecific 
      Height          =   570
      Left            =   630
      TabIndex        =   17
      Top             =   8670
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUTilStripTrans.frx":136C
   End
   Begin fpBtnAtlLibCtl.fpBtn fpBtn2 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUTilStripTrans.frx":1555
   End
   Begin fpBtnAtlLibCtl.fpBtn fpBtnRemoveHist 
      Height          =   570
      Left            =   4410
      TabIndex        =   19
      Top             =   8700
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUTilStripTrans.frx":173F
   End
   Begin fpBtnAtlLibCtl.fpBtn fpBtnDeleteInActCust 
      Height          =   630
      Left            =   1260
      TabIndex        =   20
      Top             =   9630
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmUTilStripTrans.frx":1920
   End
...
Private Sub fpBtnDeleteInActCust_Click()
If MsgBox("Delete Inactive customers without balances --but NOT VACANT --Yes to Continue, No to Cancel", vbYesNo) = vbYes Then
      Call DeleteInactiveCusts
    End If
End Sub

Private Sub fpBtnRemoveHist_Click()
    If MsgBox("Remove Transaction History prior to " & txtDate1.Text & " Yes to Continue, No to Cancel", vbYesNo) = vbYes Then
      StripemHistory
    End If
End Sub

Private Sub fpBtnRemoveInv_Click()
  StripemInvalidTrans
End Sub
Private Sub btnRemoveSpecific_Click()
  StripemSpecificTrans
End Sub
...
'      Case TranUtilityBill
      If (UBTranRec(1).TransType = TrType) Or (UBTranRec(1).TransType = TrType + 100) Then
        ...
        CustBook = Val(UBCust(1).Book)
        If CustBook >= FromBook And CustBook <= ThruBook Then
         If operchk = UBTranRec(1).OperatorNumber Then
          Select Case TrType
            Case 1  'Utility Bill
              Removed = Removed + 1
            Case 4   'payment
...
Private Sub StripemNOBALADJ()
  ...
  Dim TrType As String, TrTyp As Integer, CustBook As Integer
  Dim Removed As Long, FromBook As Integer, ThruBook As Integer
  Dim operchk As Integer, CustRec As Long
  Date1 = Date2Num(txtDate1.Text)
  FromBook = Val(fptxtRoute1)
  ThruBook = Val(fptxtRoute2)
  If fpcboTransType.ListIndex <> -1 Then
...
'      Case TranUtilityBill
''      If (UBTranRec(1).TransType = TrType) Or (UBTranRec(1).TransType = TrType + 100) Then
''        If UBTranRec(1).CustAcctNo <= 0 Or UBTranRec(1).CustAcctNo > CustRec Then
''          UBLog "InvAcct1:" & Str(UBTranRec(1).CustAcctNo) & "," & Str(cnt&) & "," & Str(UBTranRec(1).Transamt)
''          GoTo InvAcct1
''        End If
''        Get UBFile3, UBTranRec(1).CustAcctNo, UBCust(1)
...
InvAcct:
  ...
  End If
End Sub
Private Sub StripemInvalidTrans()
  Dim Date1 As Integer, UBTranRecLen As Integer, CustLen As Integer
  Dim UBFile1 As Integer, UBFile2 As Integer, UBFile3 As Integer
  Dim TNumOfRecs As Long, cnt As Long, RCnt As Integer
  Dim TrType As String, TrTyp As Integer, CustBook As Integer
  ...
  End If
End Sub
Private Sub StripemSpecificTrans()
  Dim Date1 As Integer, UBTranRecLen As Integer, CustLen As Integer
  Dim UBFile1 As Integer, UBFile2 As Integer, UBFile3 As Integer
  Dim TNumOfRecs As Long, cnt As Long, RCnt As Integer
  Dim TrType As String, TrTyp As Integer, CustBook As Integer
  ...
  For cnt& = 1 To TNumOfRecs&
    ...
    Get UBFile1, cnt&, UBTranRec(1)
          ...
          If cnt& >= 2372829 And cnt& <= 2372927 Then
            Removed = Removed + 1
           Else
...
Private Sub StripemHistory()
'Remove history prior to a date
'and any accounts that are inactive and do not have a balance but not vacant accts that have location
  Dim Date1 As Integer, UBTranRecLen As Integer, CustLen As Integer
  Dim UBFile1 As Integer, UBFile2 As Integer, UBFile3 As Integer
  Dim TNumOfRecs As Long, cnt As Long, RCnt As Integer
  Dim TrType As String, TrTyp As Integer, CustBook As Integer
  ...
  For cnt& = 1 To TNumOfRecs&
    ...
    Get UBFile1, cnt&, UBTranRec(1)
            ...
            If UBTranRec(1).TransDate < Date1 Then
              If UBTranRec(1).TransType <> 7 Or UBTranRec(1).TransType <> 5 Or UBTranRec(1).TransType <> 9 Or UBTranRec(1).TransType <> 39 Then
              Removed = Removed + 1
            End If
           Else
...
Private Sub DeleteInactiveCusts()
Dim Date1 As Integer, UBTranRecLen As Integer
Dim PrevTranRec As Long, FoundCnt As Integer
Dim UBFile1 As Integer
  Dim UBCustRecLen As Integer, NumOfCustRecs As Long, NumOfRecs As Long
  Dim CustFile As Integer, cnt As Long, TotalBalance#
  ...
  For cnt = 1 To NumOfCustRecs&
    ...
    oktodel = False
    If (Trim(UBCustRec(1).SEARCH) <> "VACANT") Then
    If (Trim(UBCustRec(1).CustName) <> "VACANT") Then
    If UBCustRec(1).Status = "I" Then
      
      PrevTranRec& = UBCustRec(1).LastTrans
      If PrevTranRec& > 0 Then
        Get UBFile1, PrevTranRec&, UBTranRec(1)
        If UBTranRec(1).TransDate >= Date1 Then
          oktodel = False
        Else
          TotalBalance# = Round#(UBCustRec(1).CurrBalance + UBCustRec(1).PrevBalance)
          If (TotalBalance# <> 0) Or (UBCustRec(1).DepositAmt <> 0) Then
            oktodel = False
          Else
            oktodel = True
          End If
   ...
   Next
  Close
  UBLog "Set Inactive Accounts without balances to deleted (not VACANT)"
  MsgBox "Inactive Accounts Deleted.", vbOKOnly
End Sub
