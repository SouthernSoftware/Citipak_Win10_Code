...
Begin VB.Form frmVATaxPPenRateSetUpTbl 
   BackColor       =   &H008F8265&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Personal Penalty Setup Table"
   ClientHeight    =   8760
   ClientLeft      =   30
   ClientTop       =   420
   ClientWidth     =   11655
   Icon            =   "frmVATaxPPenRateSetUpTbl.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPPenRateSetUpTbl.frx":08CA
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPPenRateSetUpTbl.frx":0AA8
   End
   ...
   Begin VB.Label Label5 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "Charge Percent of Bill Amt"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   10.5
         Charset         =   0
   ...
   Begin VB.Label Label1 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "Personal Penalty Rate Tables"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   14.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmVATaxPPenRateSetUpTbl"
...
Private Sub cmdExit_Click()
  ...
  If Exist("C:\CPWork\penmenu.dat") Then
    frmVATaxCalcPenalty.Show
    KillFile "C:\CPWork\penmenu.dat"
  ElseIf Exist("C:\CPWork\pencalc.dat") Then
    Unload Me
  Else
...
Private Sub cmdSave_Click()
  ...
  If Exist(TaxPPenFile) Then
    If TaxMsgWOpts(700, "WARNING: There is a personal penalty calculation file that has not been posted. If you choose to save these changes the unposted file will be deleted.", "F10 Save Anyway", "ESC Don't Save") = "abort" Then
      Unload frmVATaxMsgWOpts
      Exit Sub
    Else
      KillFile TaxPPenFile
      Call TaxMsg(900, "Personal penalty calculation file was deleted successfully.")
      MainLog ("WARNING: User warned that a personal penalty file exists that has not been posted. The user elected to save the new penalty rate anyway and the file was deleted.")
    End If
  End If
    
  OpenTaxPenRateTbls PRHandle, NumOfPRRecs
  If NumOfPRRecs = 1 Then
    Get PRHandle, 1, TblRec
    If TblRec.BillType = "R" Then
      SaveHere = NumOfPRRecs + 1
    ElseIf TblRec.BillType = "P" Then
      SaveHere = 1
    End If
  ElseIf NumOfPRRecs = 2 Then
    SaveHere = RecNum
  ElseIf NumOfPRRecs = 0 Then
    SaveHere = 1
  End If
  TblRec.Cushion = ""
  ...
  For x = 0 To 9
    ...
    If TblRec.TaxFAmt(x + 1) > 0 And TblRec.TaxPAmt(x + 1) = 0 Then
      TblRec.RateType(x + 1) = "F"
    ElseIf TblRec.TaxFAmt(x + 1) = 0 And TblRec.TaxPAmt(x + 1) > 0 Then
      TblRec.RateType(x + 1) = "P"
    ElseIf TblRec.TaxFAmt(x + 1) = 0 And TblRec.TaxPAmt(x + 1) = 0 Then
      TblRec.RateType(x + 1) = "N"
    Else
      Call TaxMsg(700, "Please revise the charge amount values on line " + CStr(x + 1) + ". Either the flat amount or the percentage of bill amount must be saved as zero.")
      fpCurrFTax(x).SetFocus
      Close
      Exit Sub
    End If
  Next x
  ...
  Put PRHandle, SaveHere, TblRec
  
  Close PRHandle
  
  Call Savemsg(900, "The penalty personal rate was saved successfully.")
  
  If Exist("C:\CPWork\penmenu.dat") Then
    frmVATaxCalcPenalty.Show
    KillFile "C:\CPWork\penmenu.dat"
  ElseIf Exist("C:\CPWork\pencalc.dat") Then
    If frmVATaxCalcPenalty.BillType = "R" Then
      frmVATaxCalcPenalty.BillType = "B"
    End If
    Unload Me
  Else
    frmVATaxRateMenu.Show
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPPenRateSetUpTbl", "cmdSave_Click", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      ...
      MainLog ("CitiTaxes.exe terminated via menu bar on frmVATaxPPenRateSetUpTbl.")
      Call Terminate
      End
    End If
  End If
  ...
  If Exist(TaxPenRateTblFile) Then
    ...
    For x = 1 To NumOfPRRecs
      ...
      If TblRec.BillType = "P" Then
        fptxtComment.Text = TblRec.Comment
        RecNum = x
        Exit For
      End If
Deleted:
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPPenRateSetUpTbl", "LoadMe", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub fpCurrFTax_LostFocus(Index As Integer)
  If CDbl(fpCurrFTax(Index).Value) = 0 Then Exit Sub
  
  If CDbl(fpDblPTax(Index).Value) > 0 Then
    If TaxMsgWOpts(700, "Either the 'Charge Flat Amount' or the 'Charge Percent of Bill Amount' must be zero. Press F10 to allow the program to zero out the 'Charge Percent of Bill Amount' or press ESC to review.", "F10 Zero Out", "ESC Review") = "abort" Then
      Exit Sub
    Else
      fpDblPTax(Index).Value = 0
      Exit Sub
    End If
  End If
...
Private Sub fpDblPTax_LostFocus(Index As Integer)
  If CDbl(fpDblPTax(Index).Value) = 0 Then Exit Sub
  
  If CDbl(fpCurrFTax(Index).Value) > 0 Then
    If TaxMsgWOpts(700, "Either the 'Charge Flat Amount' or the 'Charge Percent of Bill Amount' must be zero. Press F10 to allow the program to zero out the 'Charge Flat Amount' or press ESC to review.", "F10 Zero Out", "ESC Review") = "abort" Then
      Exit Sub
    Else
      fpCurrFTax(Index).Value = 0
      Exit Sub
    End If
  End If
...
Private Sub fptxtComment_KeyDown(KeyCode As Integer, Shift As Integer)
  ...
  ElseIf KeyCode = vbKeyUp Then
    If fpDblPTax(9).Enabled = True Then
      fpDblPTax(9).SetFocus
    ElseIf fpCurrFTax(9).Enabled = True Then
      fpCurrFTax(9).SetFocus
    Else
      fpCurrFrom(0).SetFocus
    End If
  End If
...
Public Function Check4Changes() As Boolean
  ...
  If RecNum = 0 Then Exit Function
  OpenTaxPenRateTbls PRHandle, NumOfPRRecs
  Get PRHandle, RecNum, TblRec
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPPenRateSetUpTbl", "Check4Changes", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
