...
Begin VB.Form frmVATaxCustInq 
   BackColor       =   &H008F8265&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tax Customer Inquiry"
   ClientHeight    =   8730
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   11640
   Icon            =   "frmVATaxCustInq.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin LpLib.fpList fpList 
      ...
      TextTipMultiLine=   0
      ColDesigner     =   "frmVATaxCustInq.frx":08CA
   End
   ...
   Begin EditLib.fpMask fptxtZip 
      ...
      OLEDragMode     =   0
   End
   Begin EditLib.fpText fptxtActiveYN 
      Height          =   396
      Left            =   2880
      TabIndex        =   31
      TabStop         =   0   'False
      ...
      OLEDragMode     =   0
   End
   Begin EditLib.fpText fptxtChrgIntYN 
      Height          =   390
      Left            =   2880
      TabIndex        =   32
      TabStop         =   0   'False
      ...
      OLEDragMode     =   0
   End
   Begin EditLib.fpText fptxtLateNotYN 
      Height          =   390
      Left            =   2880
      TabIndex        =   33
      TabStop         =   0   'False
      ...
      OLEDragMode     =   0
   End
   Begin EditLib.fpText fptxtTaxExYN 
      Height          =   390
      Left            =   2880
      TabIndex        =   34
      TabStop         =   0   'False
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdLookup 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxCustInq.frx":0BDA
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdHistory 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxCustInq.frx":0DBC
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxCustInq.frx":0F9A
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdPropDet 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxCustInq.frx":1176
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdMessage 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxCustInq.frx":135C
   End
   ...
   End
End
Attribute VB_Name = "frmVATaxCustInq"
...
Option Explicit
  ...
  Dim WarnOK As Boolean
  Dim BtnFnt As Double
  Dim Town$
Private Sub cmdExit_Click()
  KillFile ("C:\CPWork\custinq.dat")
  
  If Not Exist("C:\CPWork\txrealpyment.dat") And Not Exist("C:\CPWork\txperspyment.dat") And Not Exist("C:\CPWork\prepayrefund.dat") Then
...
Private Sub cmdHistory_Click()
  If GCustNum = 0 Then
    Call TaxMsg(900, "Please load customer data first.")
    Exit Sub
  End If
...
Private Sub cmdPropDet_Click()
  ...
  If fpList.ListIndex <> -1 Then
    ...
    ThisDesc = QPTrim$(fpList.ColText)
    If ThisDesc = "No Real Property Listed" Or ThisDesc = "No Personal Property Listed" Then
      Call TaxMsg(900, "No details available for selection.")
      Exit Sub
    End If
    fpList.Col = 2
    ThisRec = CLng(fpList.ColText)
  Else
    Call TaxMsg(900, "Please make a property selection.")
    Exit Sub
  End If
  
  If InStr(ThisDesc, "Personal") Then
    OpenPersPropFile PHandle, NumOfPersRecs
    Get PHandle, ThisRec, PersPropRec
    Close PHandle
    PersRec = ThisRec
    frmVATaxPersDetail.Show vbModal
  ElseIf InStr(ThisDesc, "Real") Then
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxCustInq", "cmdPropDet", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub Form_Load()
  ...
  If Exist("C:\CPWork\txrealpyment.dat") Or Exist("C:\CPWork\txperspyment.dat") Or Exist("C:\CPWork\prepayrefund.dat") Then
    cmdLookup.Visible = False
    fptxtAcctNum.ControlType = ControlTypeReadOnly
  End If
End Sub
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      KillFile ("C:\CPWork\custinq.dat")
      MainLog ("CitiTaxes.exe terminated via menu bar on frmVATaxCustInq.")
      Call Terminate
      End
    End If
  End If
...
'  On Error GoTo ERRORSTUFF
  ...
  If GCustNum = 0 Then
    Call TaxMsg(900, "ERROR: The customer data could not be loaded. Please try again.")
    Close TCHandle
    Exit Sub
  End If
  ...
  Close TCHandle
  fptxtAcctNum.Text = CStr(TaxCust.Acct)
  fptxtName.Text = QPTrim$(TaxCust.CustName)
  fptxtOpenDate.Text = MakeRegDate(TaxCust.OPENDATE)
  fptxtHomePhone.Text = GetPhoneNum(TaxCust.HPHONE)
  fptxtWorkPhone.Text = GetPhoneNum(TaxCust.WPHONE)
  Call InsertSSNDashes(TaxCust.CSSN)
  
  fptxtSocSec.Text = TaxCust.CSSN
  
  fpCurrBalance = GetCustBalance(GCustNum, -1)
  fptxtDrvrsLic.Text = TaxCust.DrvrsLic
  fptxtAdd1.Text = QPTrim$(TaxCust.Addr1)
  fptxtAdd2.Text = QPTrim$(TaxCust.Addr2)
  fptxtCity.Text = QPTrim$(TaxCust.City)
  fptxtState.Text = TaxCust.State
  fptxtZip.Text = QPTrim$(TaxCust.Zip)
  fptxtActiveYN.Text = TaxCust.Active
'  fptxtChrgPenYN.Text = TaxCust.Penalty
  fptxtChrgIntYN.Text = TaxCust.Interest
  fptxtLateNotYN.Text = TaxCust.LateNotice
  fptxtTaxExYN.Text = TaxCust.TaxExempt
  ...
  If ThisRec > 0 Then
    ...
    Do While ThisRec > 0
      Get RHandle, ThisRec, RealPropRec
      fpList.InsertRow = "Real Property" + Chr(9) + QPTrim$(RealPropRec.RealPin) + Chr(9) + CStr(ThisRec)
      ThisRec = RealPropRec.NextRec
    Loop
  Else
    fpList.InsertRow = "No Real Property Listed"
  End If
  Close RHandle
  ...
  If ThisRec > 0 Then
    ...
    Do While ThisRec > 0
      Get PHandle, ThisRec, PersPropRec
      fpList.InsertRow = "Personal Property" + Chr(9) + QPTrim$(PersPropRec.PropPin) + Chr(9) + CStr(ThisRec)
      ThisRec = PersPropRec.NextRec
    Loop
  Else
    fpList.InsertRow = "No Personal Property Listed"
  End If
  Close PHandle
  TempAcctNum = CLng(fptxtAcctNum.Value)
  
  Exit Sub
  
GetPhoneNum:
  
  ThisPhone$ = ReplaceString(TaxCust.HPHONE, "-", "")
  NewPhone = ""
  ...
  If ThisLen = 10 Then
    For x = 1 To 12
      If x = 4 Or x = 8 Then
        NewPhone = NewPhone + "-"
      Else
        ThisPhone = NewPhone + Mid(ThisPhone, x, 1)
      End If
    Next x
  ...
  fptxtHomePhone.Text = NewPhone
  
  ThisPhone$ = ReplaceString(TaxCust.WPHONE, "-", "")
  NewPhone = ""
  ThisLen = Len(ThisPhone)
  If ThisLen = 10 Then
    For x = 1 To 12
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxCustInq", "LoadCust", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxCustInq", "Check4ValidCustNum", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Public Sub Clearscreen()
  ...
  fptxtWorkPhone.Text = "000-000-0000"
  fptxtSocSec.Text = "000-00-0000"
  fptxtDrvrsLic.Text = ""
  fptxtAdd1.Text = ""
  fptxtAdd2.Text = ""
  fptxtCity.Text = ""
  ...
  fptxtZip.Text = ""
  fpCurrBalance = 0
  fptxtActiveYN.Text = "N"
'  fptxtChrgPenYN.Text = "N"
  fptxtChrgIntYN.Text = "N"
  fptxtLateNotYN.Text = "N"
  fptxtTaxExYN.Text = "N"
  If Not Exist("C:\CPWork\txrealpyment.dat") And Not Exist("C:\CPWork\txperspyment.dat") And Not Exist("C:\CPWork\prepayrefund.dat") Then
    GCustNum = 0
  End If
