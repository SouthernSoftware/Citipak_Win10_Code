...
Public Sub ClearInUsePRReg(PWcnt As Integer)
  ...
  If Exist("CitiPass.dat") Then
    ...
    Close CitiPassFile
    Call SetToGo
'    KillFile ("C:\passtemp.dat")'2/14/08
    KillFile (PassP$)
  End If
End Sub
...
'  Open "c:\PassTemp.dat" For Random Shared As Tempfile ' Len = lentemp
  Open PassP$ For Random Shared As Tempfile '2/14/08
...
Public Sub ClearInUseTaxReg(PWcnt As Integer)
  Dim NumPassRecs As Integer, Cnt As Integer, CitiPassFile As Integer
  If PWcnt = 0 Then Exit Sub
  If PWcnt = -3 Then Exit Sub 'sosoft password
  If Exist("CitiPass.dat") Then
