...
Private Sub cmdPost_Click()
  ...
  If Not Exist("TAXCPR" + Oper$ + ".DAT") Then Exit Sub
...
SkipThisRec:
  ...
  For x = 1 To 5
    KillFile ("TAXCPR" + Oper$ + ".DAT")
    KillFile ("TAXLOP" + Oper$ + ".DAT")
  Next x
  
  If Exist("TAXCPR" + Oper$ + ".DAT") = True Or Exist("TAXLOP" + Oper$ + ".DAT") = True Then '11/30/06
    One = 1
    AHandle = FreeFile
    Open "payposterror" + Oper$ + ".dat" For Output As AHandle
    Print #AHandle, One
    Close AHandle
    ...
    MainLog ("ERROR: A payment error has taken place because payment batch files were not deleted. User was notified.")
  Else
    Call Savemsg(900, "Posting has completed successfully.")
    MainLog ("Payment post completed successfully.")
  End If
...
PrePay:
  ...
  PayTranRec.CustPin = TaxCustRec.PIN
  PayTranRec.DiscXDate = TaxTranRec.DiscXDate
  PayTranRec.RealPin = " " 'QPTrim$(TaxTranRec.RealPin)'1/29/08
  PayTranRec.PersPin = " " 'QPTrim$(TaxTranRec.PersPin)'1/29/08
  PayTranRec.Posted2GL = "N"
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxPayPost", "cmdPost_Click", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmTaxPayOperEntry", "LoadMe", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
