...
Begin VB.Form Form1 
   ...
   Begin VB.Label Label1 
      Alignment       =   2  'Center
      Caption         =   "NC Taxes Transaction Checker"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
Private Sub Command1_Click()
  ...
  Else
    ...
    Label4.Caption = "Removing Transactions"
    Call CheckTransactions
'    Call RelinkTransactions
    Label3.Caption = ""
    Label4.Caption = "Processing Complete."
  End If
End Sub
...
Private Sub CheckTransactions()
  ...
  Dim InterAmt As Double
  Dim FirstTR As Long
  Dim LastTR As Long
  CustCnt = 0
  gotCust = False
  Dim Belong As Double
  ...
  Get #1, 140934, TaxTR
  'Print TaxTR.Revenue.CollectionPd
  TaxTR.Revenue.CollectionPd = 1.18
  Put #1, 140934, TaxTR
...
'  LastTR = TRRecCnt
'
'  For WhatTR = TRRecCnt To 1 Step -1
'    Get #1, WhatTR, TaxTR
'    If TaxTR.TranType <> 4 Then
'      FirstTR = WhatTR + 1
'      Exit For
'    End If
'  Next
'
'  For WhatTR = FirstTR To LastTR
'    Get #1, WhatTR, TaxTR
'    Belong = TaxTR.BelongTo
'    InterAmt = TaxTR.Revenue.Interest
'    Get #1, Belong, TaxTR
'    TaxTR.Revenue.Interest = TaxTR.Revenue.Interest - InterAmt
'    Put #1, Belong, TaxTR
'  Next
'
'  Close
'  Name TaxTransFile As "TXTR.001"
'
'  Open "TXTR.001" For Random As #1 Len = TRLen
'  Open TaxTransFile For Random As #2 Len = TRLen
'
'  For WhatTR = 1 To (FirstTR - 1)
'    Get #1, WhatTR, TaxTR
'    Put #2, WhatTR, TaxTR
'  Next
  ...
  'GoTo DownHere 'jump over this for
  ''This checks to see if any of the transaction amount is "0"
  'and builds a list.
...
'  TaxYear = 2005
  If PrevTranRec& > 0 Then
    Do While PrevTranRec& > 0
      ...
      If TaxTran.TaxYear <> TaxYear Then
        'GoTo MoveAlong 'if we only want to get
      End If
