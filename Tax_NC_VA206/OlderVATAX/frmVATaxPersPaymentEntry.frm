...
Begin VB.Form frmVATaxPersPaymentEntry 
   ...
   Caption         =   "Personal Property Tax Payment Entry"
   ClientHeight    =   8760
   ClientLeft      =   30
   ClientTop       =   420
   ClientWidth     =   11655
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdCash 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPersPaymentEntry.frx":0F7E
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdCheck 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPersPaymentEntry.frx":1159
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdCharge 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPersPaymentEntry.frx":1335
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdDist 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPersPaymentEntry.frx":1512
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPersPaymentEntry.frx":16ED
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPersPaymentEntry.frx":18C9
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdInfo 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPersPaymentEntry.frx":1AA5
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdDrawer 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPersPaymentEntry.frx":1C80
   End
   ...
   Begin VB.Label Label8 
      ...
      Caption         =   "Personal Payment  Entry"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   13.5
         Charset         =   0
...
Option Explicit
  ...
  Dim TempBillList() As PersPayListType
...
Public Sub cmdBills_Click()
  ...
  If GetCustPersBalance(GCustNum, -1) = 0 Then
...
'  If EditFlag = True Then Exit Sub 'critical
  ...
  ThisCust = 0
  If BillCnt > 0 Or Exist(TempPersBillRecs) Then 'BillCnt is a temporary value representing
...
Private Sub cmdDist_Click()
  ...
  Dim TPayRec As PersPayListType
  Dim PayRec As PersPayListType
...
'      If PayRec.CustRec = GCustNum Then
    ...
    Next x
    Close PHandle
  ElseIf Exist(TempPersBillRecs) Then
    ReDim BillTrans(1 To 1) As Long
    ReDim BillDate(1 To 1) As Integer
    BillCnt = 0
...
Private Sub cmdExit_Click()
  ...
  KillFile TempPersBillRecs 'TempBillRecs is the filename for the temporary file
...
Private Sub cmdSave_Click()
  ...
  If VerifyPayList = False Then 'this routine looks to see if this customer's records
  'match up with the transaction records being saved
  ...
  TaxPayRec.TotPaid = OldRound(TaxPayRec.TotPaid + CDbl(fpCurrOpt3Paid.Value))
  ...
  KillFile TempPersBillRecs 'get rid of all temporary files and records in
  ...
  DontExit = False
  Call Savemsg(900, "This personal tax payment has been saved successfully.")
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      ...
      MainLog ("CitiTaxes.exe terminated via menu bar on frmVATaxPersPaymentEntry.")
      Call Terminate
      End
    End If
  End If
...
Private Sub fpCurrDisc_LostFocus()
  ...
  If CDbl(fpCurrDisc.Value) > MaxDisc Then
    Call TaxMsg(800, "The maximum personal discount allowed for this customer is " + QPTrim$(Using$("$##,##0.00", MaxDisc)) + ". The program will reset the discount to the maximum allowed.")
    fpCurrDisc = MaxDisc
...
'  fptxtPayDate = Date
  ...
  GCustNum = 0
  GPayNum = 0
  ReDim TempBillList(1 To 1) As PersPayListType
  TempBillListCnt = 0
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPersPaymentEntry", "Check4ValidCustNum", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Public Sub EnterEditChk()
  ...
  Select Case BegBalCheck(CustNum, ONum$, ThisRec, ThisBillType)
    Case 1 'normal first time transaction for this customer
      ...
      FindStr = FindCustInBatchFile(CustNum, "P")
      If FindStr <> "0" Then
...
Private Sub LoadHerUpWOEdit()
  ...
  DiscYN = False
  ThisBalance = GetCustPersBalance(GCustNum, -1)
  ...
  ElseIf ThisBalance < 0 Then
    Call TaxMsg(900, "This customer has a personal balance of -" + QPTrim$(Using$("$##,##0.00", Abs(ThisBalance))) + ".")
...
'        PersOwed# = OldRound(PersOwed# - TransRec.DiscAmt) '8/23/07
  ...
  Call GetMaxDisc
  If MaxDisc > 0 Then
    If GetCustPersBalance(GCustNum, ThisTaxYear) > 0 Then
      ...
      Else
        ...
        Call TaxMsg(900, "The total personal amount owed including the discount will be " + QPTrim$(Using$("$##,##0.00", Dif)) + ".")
      End If
    Else
...
Private Function Check4Discounts() As Integer
  ...
  Dim ListRec As PersPayListType
  ...
  If PayTally = CDbl(fpCurrTotOwed.Value) Then
    If GetCustPersBalance(GCustNum, TaxYear) > PayTally Then 'if true than old balance exists
  ...
  If InSave = True And CDbl(fpCurrTotWDisc.Value) < CDbl(fpCurrTotOwed.Value) Then
    If GetCustPersBalance(GCustNum, TaxYear) > 0 Then 'if true than old balance exists
      DiscOK = 2 'discount allowed but warn that some old balance is outstanding
    Else
      DiscOK = 1 'discount allowed...no warnings necessary
    End If
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPersPaymentEntry", "Check4Discounts", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub UPDateListOfPayments()
  ...
  If BillCnt = 0 And GetCustPersBalance(GCustNum, -1) <= 0 Then 'customer
...
''    If OldRound(CDbl(fpCurrPersPaid.Value) + CDbl(fpCurrDisc.Value)) > CDbl(fpCurrPersOwed.Value) Then
...
''      Else
''        frmVATaxMsg.Label1.Caption = "The customer is attempting to overpay Personal (plus Discount) while underpaying Machine Tools. Please only allow overpayment for a revenue if all other revenue obligations have been met."
''      End If
'      frmVATaxMsg.Label1.Top = 600
'      frmVATaxMsg.Show vbModal
'      CheckOverPay = True
...
'      CheckOverPay = True
'      Exit Function
'    ElseIf CDbl(fpCurrMHOwed.Value) > CDbl(fpCurrMHOwed.Value) Then
'      fpCurrMTOwed.BackColor = &H8080FF
'      fpCurrMTPaid.BackColor = &H8080FF
...
'      CheckOverPay = True
'      Exit Function
'    ElseIf CDbl(fpCurrIntOwed.Value) > CDbl(fpCurrIntOwed.Value) Then
'      fpCurrMTOwed.BackColor = &H8080FF
'      fpCurrMTPaid.BackColor = &H8080FF
...
'      CheckOverPay = True
'      Exit Function
'    ElseIf CDbl(fpCurrPenOwed.Value) > CDbl(fpCurrPenOwed.Value) Then
'      fpCurrMTOwed.BackColor = &H8080FF
'      fpCurrMTPaid.BackColor = &H8080FF
...
'      CheckOverPay = True
'      Exit Function
'    ElseIf CDbl(fpCurrOpt1Owed.Value) > CDbl(fpCurrPenOwed.Value) Then
'      fpCurrMTOwed.BackColor = &H8080FF
'      fpCurrMTPaid.BackColor = &H8080FF
'      fpCurrPenPaid.BackColor = &H8080FF
'      fpCurrPenOwed.BackColor = &H8080FF
...
Private Sub PrintReceipt()
  ...
  Print #RHandle, "PERSONAL PROPERTY TAX PAYMENT"
...
'  Call ReFigure
'
'  MainLog ("frmVATaxPersPaymentEntry: Customer, " + fptxtName.Text + ", is eligible for a discount of " + QPTrim$(Using$("$#,##0.00", ThisDiscAmt)) + " and the user allowed the discount to apply.")
'
'  Exit Sub
'
'AddCol:
...
'  PrePayAmt = CDbl(fpCurrPrePay.Value)
  ...
  ReDim Preserve TempBillList(1 To BillCnt) As PersPayListType
  TempBillListCnt = 0
  For x = 1 To BillCnt
...
'     WhatsLeft = OldRound(WhatsLeft - TempBillList(x).DiscAmt) 'remmed on 1/25/07
     For y = 1 To 10
...
PlayedOut:
  ...
  fpCurrChngDue.Value = OldRound(CDbl(fpCurrTotRecd.Value) - CDbl(fpCurrTotPaid.Value)) ' + CDbl(fpCurrDisc.Value)) took out 8/9/06
  If CDbl(fpCurrChngDue.Value) < 0 Then
    fpCurrChngDue = 0
  End If
...
Private Sub RemoveDiscount()
  Dim x As Integer
  Dim TempPayRec As PersPayListType
  Dim THandle As Integer
...
Private Sub OverRideDist()
  ...
  If CDbl(fpCurrMTOwed.Value) > CDbl(fpCurrMTPaid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic machine tools payment of " + QPTrim$(Using("$##,##0.00", TempMTPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new machine tools amount of " + fpCurrMTPaid.Text + ".")
    End If
  End If
  ...
  If CDbl(fpCurrMCOwed.Value) > CDbl(fpCurrMCPaid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic merchant capital payment of " + QPTrim$(Using("$##,##0.00", TempMCPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new merchant capital amount of " + fpCurrMCPaid.Text + ".")
    End If
  End If
  ...
  If CDbl(fpCurrFEOwed.Value) > CDbl(fpCurrFEPaid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic farm equipment payment of " + QPTrim$(Using("$##,##0.00", TempFEPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new farm equipment amount of " + fpCurrFEPaid.Text + ".")
    End If
  End If
  ...
  If CDbl(fpCurrPersOwed.Value) > OldRound(CDbl(fpCurrPersPaid.Value) + CDbl(fpCurrDisc.Value)) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic personal payment of " + QPTrim$(Using("$##,##0.00", TempPersPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new personal amount of " + fpCurrPersPaid.Text + ".")
    End If
  End If
...
'  OptRev = QPTrim$(fptxtOpt1.Text)
  If CDbl(fpCurrMHOwed.Value) > CDbl(fpCurrMHPaid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic mobile home payment of " + QPTrim$(Using("$##,##0.00", TempMHPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new mobile home amount of " + fpCurrMHPaid.Text + ".")
    End If
  End If
...
'  OptRev = QPTrim$(fptxtOpt3.Text)
  If CDbl(fpCurrPenOwed.Value) > CDbl(fpCurrPenPaid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic penalty payment of " + QPTrim$(Using("$##,##0.00", TempPenPaid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new penalty amount of " + fpCurrPenPaid.Text + ".")
    End If
  End If
  ...
  If CDbl(fpCurrOpt1Owed.Value) > CDbl(fpCurrOpt1Paid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic " + Opt1Desc + " payment of " + QPTrim$(Using("$##,##0.00", TempOpt1Paid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new penalty amount of " + fpCurrOpt1Paid.Text + ".")
    End If
  End If
...
Next2:
  ...
  If CDbl(fpCurrOpt2Owed.Value) > CDbl(fpCurrOpt2Paid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic " + Opt2Desc + " payment of " + QPTrim$(Using("$##,##0.00", TempOpt2Paid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new " + Opt2Desc + " amount of " + fpCurrOpt2Paid.Text + ".")
    End If
  End If
...
Next3:
  ...
  If CDbl(fpCurrOpt3Owed.Value) > CDbl(fpCurrOpt3Paid.Value) Then
    ...
    Else
      Unload frmVATaxMsgWOpts
      fpCurrDisc.Value = 0
      MainLog ("WARNING: User warned that overriding the automatic " + Opt3Desc + " payment of " + QPTrim$(Using("$##,##0.00", TempOpt3Paid)) + " would eliminate the discount of " + QPTrim$(Using$("$##,##0.00", TempDisc)) + " but the user continued with the new " + Opt3Desc + "  amount of " + fpCurrOpt3Paid.Text + ".")
    End If
  End If
...
Private Sub GetMaxDisc()
  ...
  MaxDisc = 0
'  If TaxMasterRec.DiscPXDate > Date2Num(fptxtPayDate.Text) Then '7/13/06 Then
'    DiscYN = True
'  End If
  
  ThisTaxYear = TaxMasterRec.PTaxYear ' 7/13/06
...
'    If TaxTRec.BillType = "P" And TaxTRec.TaxYear = ThisTaxYear And DiscPXDate >= Date2Num(fptxtPayDate.Text) Then
    If TaxTRec.BillType = "P" And TaxTRec.TaxYear = ThisTaxYear And TaxTRec.DiscXDate > 0 And DiscPXDate <= TaxTRec.DiscXDate Then 'changed from above 7.26.06
...
'      Balance# = OldRound(Balance# - (TaxTRec.Revenue.RevOpt3Pd + TaxTRec.PPTRADisc + TaxTRec.DiscAmt))
      Balance = TaxTRec.Amount 'added 2/9/07 'remmed out 8/23/07
      If Balance# > 0 Then 'save which transaction the discount is applied to
...
'  DumpPenny = Disc1
'  PaySeq(x, 2) = OldRound(PaySeq(x, 2) - Disc2)
'  DumpPenny = OldRound(DumpPenny + Disc2)
'  PaySeq(x, 3) = OldRound(PaySeq(x, 3) - Disc3)
'  DumpPenny = OldRound(DumpPenny + Disc3)
'  PaySeq(x, 4) = OldRound(PaySeq(x, 4) - Disc4)
'  DumpPenny = OldRound(DumpPenny + Disc4)
'  PaySeq(x, 5) = OldRound(PaySeq(x, 5) - Disc5)
'  DumpPenny = OldRound(DumpPenny + Disc5)
'  PaySeq(x, 8) = OldRound(PaySeq(x, 8) - Disc6)
'  DumpPenny = OldRound(DumpPenny + Disc6)
'  PaySeq(x, 9) = OldRound(PaySeq(x, 9) - Disc7)
'  DumpPenny = OldRound(DumpPenny + Disc7)
'  PaySeq(x, 10) = OldRound(PaySeq(x, 10) - Disc8)
'  DumpPenny = OldRound(DumpPenny + Disc8)
'  DiscApplied = True
'  If DumpPenny <> TempBillList(x).DiscAmt Then
'    PaySeq(x, 1) = PaySeq(x, 1) + OldRound(Abs(DumpPenny - TempBillList(x).DiscAmt))
'  End If
'
'  Return
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPersPaymentEntry", "GetRcpInfo", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Function Check4ValidPaidEntries() As Boolean '8/12/05
  ...
  If CDbl(fpCurrPersPaid.Value) > CDbl(fpCurrPersOwed.Value) Then
    Call TaxMsg(800, "The amount entered for personal paid cannot be more than the amount owed for personal. Press F9 to correct this problem.")
    fpCurrPersPaid.SetFocus
    Check4ValidPaidEntries = False
    Exit Function
  End If
  If CDbl(fpCurrMTPaid.Value) > CDbl(fpCurrMTOwed.Value) Then
    Call TaxMsg(800, "The amount entered for machine tools paid cannot be more than the amount owed for machine tools. Press F9 to correct this problem.")
  ...
  If CDbl(fpCurrMCPaid.Value) > CDbl(fpCurrMCOwed.Value) Then
    Call TaxMsg(800, "The amount entered for merchant capital paid cannot be more than the amount owed for merchant capital. Press F9 to correct this problem.")
  ...
  If CDbl(fpCurrFEPaid.Value) > CDbl(fpCurrFEOwed.Value) Then
    Call TaxMsg(800, "The amount entered for farm equipment paid cannot be more than the amount owed for farm equipment. Press F9 to correct this problem.")
  ...
  If CDbl(fpCurrMHPaid.Value) > CDbl(fpCurrMHOwed.Value) Then
    Call TaxMsg(800, "The amount entered for mobile homes paid cannot be more than the amount owed for mobile homes. Press F9 to correct this problem.")
    fpCurrMHPaid.SetFocus
    Check4ValidPaidEntries = False
    Exit Function
  End If
...
Private Function VerifyPayList() As Boolean
  ...
  OpenPersPayListFile PHandle, OperNum
  NumOfPRecs = LOF(PHandle) / Len(PayRec)
  Do While NextRec > 0
