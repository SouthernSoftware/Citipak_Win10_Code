...
Begin VB.Form frmVATaxPenRateSetUpTbl 
   BackColor       =   &H008F8265&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Penalty Rate Setup Table"
   ClientHeight    =   8760
   ClientLeft      =   30
   ClientTop       =   420
   ClientWidth     =   11655
   Icon            =   "frmVATaxPenRateSetUpTbl.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPenRateSetUpTbl.frx":08CA
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxPenRateSetUpTbl.frx":0AA8
   End
   ...
   Begin VB.Label Label1 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "Real Penalty Rate Tables"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   14.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmVATaxPenRateSetUpTbl"
...
Private Sub cmdSave_Click()
  ...
  If Exist(TaxRPenFile) Then
    If TaxMsgWOpts(700, "WARNING: There is a real penalty calculation file that has not been posted. If you choose to save these changes the unposted file will be deleted.", "F10 Save Anyway", "ESC Don't Save") = "abort" Then
      Unload frmVATaxMsgWOpts
      Exit Sub
    Else
      KillFile TaxRPenFile
      Call TaxMsg(900, "Real penalty calculation file was deleted successfully.")
      MainLog ("WARNING: User warned that a real penalty file exists that has not been posted. The user elected to save the new real rate anyway and the file was deleted.")
    End If
  End If
  ...
  For x = 0 To 9
    ...
    If CDbl(fpCurrFrom(x).Value) > CDbl(fpCurrTo(x).Value) Then
      Call TaxMsg(700, "ERROR: The 'From Real Bill Amount' value is greater than the 'To Real Bill Amt' value. Please correct these values.")
      fpCurrFrom(x).SetFocus
      Close
      Exit Sub
    End If
  ...
  Call Savemsg(900, "The penalty real rate was saved successfully.")
  
  If Exist("C:\CPWork\penmenu.dat") Then
    frmVATaxCalcPenalty.Show
    KillFile "C:\CPWork\penmenu.dat"
  ElseIf Exist("C:\CPWork\pencalc.dat") Then
    If frmVATaxCalcPenalty.BillType = "P" Then
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPenRateSetUpTbl", "cmdSave_Click", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      ...
      MainLog ("CitiTaxes.exe terminated via menu bar on frmVATaxPenRateSetUpTbl.")
      Call Terminate
      End
    End If
  End If
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPenRateSetUpTbl", "LoadMe", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxPenRateSetUpTbl", "Check4Changes", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
