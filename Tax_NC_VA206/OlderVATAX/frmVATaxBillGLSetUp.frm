...
Begin VB.Form frmVATaxBillGLSetUp 
   BackColor       =   &H008F8265&
   ...
   ClientTop       =   435
   ClientWidth     =   11640
   Icon            =   "frmVATaxBillGLSetUp.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   ScaleHeight     =   8730
   ...
   Begin LpLib.fpList fpListYear 
      ...
      TextTipMultiLine=   0
      ColDesigner     =   "frmVATaxBillGLSetUp.frx":08CA
   End
   ...
   Begin fpBtnAtlLibCtl.fpBtn cmdExit 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxBillGLSetUp.frx":0B56
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdSave 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxBillGLSetUp.frx":0D35
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdGLList 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxBillGLSetUp.frx":0F12
   End
   Begin fpBtnAtlLibCtl.fpBtn cmdGLTest 
      ...
      DropShadowColor =   0
      Redraw          =   -1  'True
      ButtonDesigner  =   "frmVATaxBillGLSetUp.frx":10F1
   End
   ...
   End
End
Attribute VB_Name = "frmVATaxBillGLSetUp"
...
MoveOn:
  ...
  Call Savemsg(900, "Your Real Bill Setup Data has been saved successfully.")
  If Exist("C:\CPWork\revrglbill.dat") Then
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxBillGLSetUp", "cmdSave_Click", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
  If ((UnloadMode = vbFormControlMenu)) Then
    ...
    Else
      Unload frmVATaxGLList
      ClearInUse PWcnt
      MainLog ("CitiTaxes.exe terminated via menu bar on frmVATaxBillGLSetUp.")
      Call Terminate
      End
    End If
  End If
...
Private Sub Form_Load()
  ...
  MainLog ("User opened frmVATaxBillGLSetUp.")
  Call LoadMe
End Sub
...
Private Sub LoadMe()
  ...
  One = 1
  AHandle = FreeFile
  Open "C:\CPWork\taxrbillGL.dat" For Output As AHandle
  Print #AHandle, One
  Close AHandle
  
  OpenRTaxGLInterBill RRHandle
  If Exist(TxRGLInterBill) Then
    Get RRHandle, 1, RevRec
    For x = 1 To 51
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxBillGLSetUp", "LoadMe", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxBillGLSetUp", "fpListYear_Click", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxBillGLSetUp", "LogSaves", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxBillGLSetUp", "Check4Changes", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
ERRORSTUFF:
   Select Case ErrorMessage(Err.Number, Err.Description, Err.Source, "frmVATaxBillGLSetup", "VerifyGLNum", Erl)
     Case emrExitProc:
       Resume Proc_Exit
     Case emrResume:
       Resume
...
'  If QPTrim$(fptxtPenCredit.Text) = "" Then Exit Sub
  ThatLen = Fund + Dept + Detail
  If ThatLen = 0 Then Exit Sub
'  fptxtPenCredit.Text = ReplaceString(fptxtPenCredit.Text, "-", "")
'  ThisLen = Len(QPTrim$(fptxtPenCredit.Text))
  ChkGL$ = ReplaceString(ChkGL, "-", "")
  ThisLen = Len(QPTrim$(ChkGL$))
  If ThisLen <> ThatLen Then
    If TaxMsgWOpts(750, "The GL number entered is not the same length as the other GL numbers saved. If you wish to review this entry press ESC. Otherwise, press F10 to continue.", "F10 Continue Anyway", "ESC Review") = "abort" Then
...
'      Exit Sub
    End If
  End If
'  fptxtPenCredit.Text = AddDashesToGLNumber(fptxtPenCredit.Text, Fund, Dept, Detail)
...
'  If QPTrim$(fptxtPenDebit.Text) = "" Then Exit Sub
  ThatLen = Fund + Dept + Detail
  If ThatLen = 0 Then Exit Sub
'  fptxtPenDebit.Text = ReplaceString(fptxtPenDebit.Text, "-", "")
'  ThisLen = Len(QPTrim$(fptxtPenDebit.Text))
  ChkGL$ = ReplaceString(ChkGL, "-", "")
  ThisLen = Len(QPTrim$(ChkGL$))
  If ThisLen <> ThatLen Then
    If TaxMsgWOpts(750, "The GL number entered is not the same length as the other GL numbers saved. If you wish to review this entry press ESC. Otherwise, press F10 to continue.", "F10 Continue Anyway", "ESC Review") = "abort" Then
...
'      Exit Sub
    End If
  End If
'  fptxtPenDebit.Text = AddDashesToGLNumber(fptxtPenDebit.Text, Fund, Dept, Detail)
