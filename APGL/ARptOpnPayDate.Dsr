VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} ARptOpnPays2 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Open Payables "
   ClientHeight    =   6120
   ClientLeft      =   36
   ClientTop       =   264
   ClientWidth     =   10620
   Icon            =   "ARptOpnPayDate.dsx":0000
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   _ExtentX        =   18733
   _ExtentY        =   10795
   SectionData     =   "ARptOpnPayDate.dsx":08CA
End
Attribute VB_Name = "ARptOpnPays2"
...
'    ' or user defined.
    Fields("InvRec").Value = arr(0)
    Fields("Vendor").Value = arr(1)
    Fields("VendName").Value = arr(2)
    Fields("InvNum").Value = arr(3)
    Fields("InvDate").Value = arr(4)
    Fields("DueDate").Value = arr(5)
    Fields("PO").Value = arr(6)
    Fields("InvAmt").Value = arr(7)
    Fields("DistAcct").Value = arr(8)
    Fields("Desc").Value = arr(9)
    Fields("DistAmt").Value = arr(10)
    Fields("Fund").Value = arr(11)
...
Private Sub ActiveReport_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
  ...
  If Shift = 4 Then
    ...
    If KeyCode = vbKeyE Then
      ...
      DoEvents
      MsgBox "File - OpenPayRptD.xls, created in the Citipak Directory.", vbOKOnly
    End If
    If KeyCode = vbKeyT Then
      ...
      Screen.MousePointer = vbDefault
      DoEvents
      MsgBox "File - OpenPayRptD.txt, created in the Citipak Directory.", vbOKOnly
    End If
  End If
End Sub
...
Private Sub ExportReport(x As Integer)
  ...
  Select Case x
    Case 1   '"Excel"
        ...
        oEXL.FileName = outfile & "OpenPayRptD.xls"
        oEXL.Export Me.Pages
    Case 2   '"Text"
        Set oTXT = New ActiveReportsTextExport.ARExportText
        oTXT.FileName = outfile & "OpenPayRptD.txt"
        oTXT.PageDelimiter = ";"
        oTXT.TextDelimiter = ","
