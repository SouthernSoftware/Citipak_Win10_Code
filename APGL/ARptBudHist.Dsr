VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} ARptBudHist 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Budget History Report"
   ClientHeight    =   4380
   ClientLeft      =   36
   ClientTop       =   264
   ClientWidth     =   9180
   Icon            =   "ARptBudHist.dsx":0000
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   _ExtentX        =   16193
   _ExtentY        =   7726
   SectionData     =   "ARptBudHist.dsx":08CA
End
Attribute VB_Name = "ARptBudHist"
...
'    ' or user defined.
    Fields("Acct").Value = arr(0)
    Fields("Typ").Value = arr(1)
    Fields("Date").Value = arr(2)
    Fields("Desc").Value = arr(3)
    Fields("Ref").Value = arr(4)
    Fields("Debit").Value = arr(5)
    Fields("Credit").Value = arr(6)
    Fields("Src").Value = arr(7)
    Fields("BalEDeb").Value = arr(8)
    Fields("BalECred").Value = arr(9)
...
Private Sub Detail_Format()
  If Me.Fields("Desc").Value = "Budget Balance" Then
...
Private Sub ActiveReport_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
  ...
  If Shift = 4 Then
    ...
    If KeyCode = vbKeyE Then
      ...
      DoEvents
      MsgBox "File - BudHist.xls, created in the Citipak Directory.", vbOKOnly
    End If
    If KeyCode = vbKeyT Then
      ...
      Screen.MousePointer = vbDefault
      DoEvents
      MsgBox "File - BudHist.txt, created in the Citipak Directory.", vbOKOnly
    End If
  End If
...
Private Sub ExportReport(x As Integer)
  ...
  Select Case x
    Case 1   '"Excel"
        ...
        oEXL.FileName = outfile & "BudHist.xls"
        oEXL.Export Me.Pages
    Case 2   '"Text"
        Set oTXT = New ActiveReportsTextExport.ARExportText
        oTXT.FileName = outfile & "BudHist.txt"
        oTXT.PageDelimiter = ";"
        oTXT.TextDelimiter = ","
...
'  If Me.Fields("Typ").Value = "D" Then
'    GroupFooter2.Visible = False
'  Else
'    GroupFooter2.Visible = True
'  End If
'
'  If Me.Field3.DataValue = "" And Me.Field4.DataValue = "" Then
...
Private Sub GroupFooter1_Format()
  If Me.BalCred.DataValue = "" And Me.BalCred.DataValue = "" Then
    GroupFooter1.Visible = False
  Else
'    If Me.BalDeb.DataValue = 0 And Me.BalCred.DataValue = 0 Then
'      GroupFooter1.Visible = False
'    Else
